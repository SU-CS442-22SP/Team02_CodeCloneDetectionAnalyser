    public static ISimpleChemObjectReader createReader(URL url, String urlString, String type) throws CDKException {
        ...
        if (cor == null) {
            throw new CDKException(GT._underscore("Could not determine file format"));
        }
        if (cor instanceof MDLV2000Reader) {
            try {
                ...
                while ((line = in.readLine()) != null) {
                    if (line.equals("$$$$")) {
                        String message = GT._underscore("It seems you opened a mol or sdf" + " file containing several molecules. " + "Only the first one will be shown");
                        JOptionPane.showMessageDialog(null, message, GT._underscore("sdf-like file"), JOptionPane.INFORMATION_underscoreMESSAGE);
                        break;
                    }
                }
