            ...
            if (message == null) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (contentId.startsWith(Constant.PREFIX_underscorePERSON)) {
                        ...
                        if (notify_underscoreid.equals("")) {
                            notify_underscoreid = u.generate().toString();
                            header.setType(Constant.MESSAGETYPE_underscoreNOTIFY);
                            pstmt = conn.prepareStatement(drawOutRecvSql(header, notify_underscoreid));
                            pstmt.executeUpdate();
                            String notify_underscorecontent = header.getNotifyContent();
                            if (notify_underscorecontent == null) notify_underscorecontent = "接收到新的esb消息，但未定义通知消息内容!";
                            saveClobMessage(pstmt, conn, rs, notify_underscorecontent, notify_underscoreid);
                        }
                        savePersonInfo(pstmt, conn, recv, notify_underscoreid);
                    } else {
                        DataHandler dh = a.getDataHandler();
                        InputStream is = dh.getInputStream();
                        String attid = u.generate().toString();
                        sql = "insert into message_underscorerecv_underscoreattachment(ATTACHMENTID," + "VERSION,MRECVID,BUSS_underscoreID,ATTACHMENT) values('" + attid + "',0,'" + id + "','" + contentId + "',empty_underscoreblob())";
                        pstmt = conn.prepareStatement(sql);
                        pstmt.executeUpdate();
                        sql = "select attachment from message_underscorerecv_underscoreattachment" + " where attachmentid = '" + attid + "' for update";
                        pstmt = conn.prepareStatement(sql);
