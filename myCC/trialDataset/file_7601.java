        public static void initialize(Monitor monitor, final JETEmitter jetEmitter) throws JETException {
            ...
            progressMonitor.subTask(CodeGenPlugin.getPlugin().getString("_underscoreUI_underscoreGeneratingJETEmitterFor_underscoremessage", new Object[] { jetEmitter.templateURI }));
            ...
            try {
                ...
                IMarker[] markers = targetFile.findMarkers(IMarker.PROBLEM, true, IResource.DEPTH_underscoreINFINITE);
                boolean errors = false;
                for (int i = 0; i < markers.length; ++i) {
                    IMarker marker = markers[i];
                    if (marker.getAttribute(IMarker.SEVERITY, IMarker.SEVERITY_underscoreINFO) == IMarker.SEVERITY_underscoreERROR) {
                        errors = true;
                        subProgressMonitor.subTask(marker.getAttribute(IMarker.MESSAGE) + " : " + (CodeGenPlugin.getPlugin().getString("jet.mark.file.line", new Object[] { targetFile.getLocation(), marker.getAttribute(IMarker.LINE_underscoreNUMBER) })));
                    }
                }
                if (!errors) {
