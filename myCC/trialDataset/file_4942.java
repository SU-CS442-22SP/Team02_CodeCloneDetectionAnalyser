    private static Collection<String> crossCheckFromOBOFile(String category) throws Exception {
        ...
        URL url = new URL(OboUtils.PSI_underscoreMI_underscoreOBO_underscoreLOCATION + "?revision=" + revision);
        log.debug("url " + url);
        ...
        while ((inputLine = in.readLine()) != null) {
            ...
            if (inputLine.startsWith("[Term]")) {
                ...
                mi = temp.split("\\s+")[1];
                miCounter++;
            } else if (inputLine.contains("is_underscoreobsolete: true")) {
                obsoleteCounter++;
                ...
                psiTerm++;
                if (category.equalsIgnoreCase(OboCategory.PSI_underscoreMI_underscoreSLIM)) miCol.add(mi);
