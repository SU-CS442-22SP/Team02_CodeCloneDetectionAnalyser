    public static void copy(File from_underscorefile, File to_underscorefile) throws IOException {
        if (!from_underscorefile.exists()) abort("FileCopy: no such source file: " + from_underscorefile.getName());
        if (!from_underscorefile.isFile()) abort("FileCopy: can't copy directory: " + from_underscorefile.getName());
        if (!from_underscorefile.canRead()) abort("FileCopy: source file is unreadable: " + from_underscorefile.getName());
        ...
        if (to_underscorefile.exists()) {
            if (!to_underscorefile.canWrite()) abort("FileCopy: destination file is unwriteable: " + to_underscorefile.getName());
        } else {
