    public void removeRecipe(Recipe recipe) throws Exception {
        ...
        try {
            int id = -1;
            conn = getConnection();
            pst1 = conn.prepareStatement("SELECT recipe_underscoreid FROM recipes WHERE name = ? AND instructions = ? ");
            ...
            pst2 = conn.prepareStatement("DELETE FROM ingredients WHERE recipe_underscoreid = ? ");
            pst3 = conn.prepareStatement("DELETE FROM recipes WHERE recipe_underscoreid = ? ");
            pst2.setInt(1, id);
            pst3.setInt(1, id);
            pst2.executeUpdate();
