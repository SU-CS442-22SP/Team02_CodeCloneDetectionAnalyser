    private boolean authenticate(Module module) throws Exception {
        ...
        Util.writeASCII(out, RSYNCD_underscoreAUTHREQD + b64 + "\n");
        String reply = Util.readLine(in);
        if (reply.indexOf(" ") < 0) {
            Util.writeASCII(out, AT_underscoreERROR + ": bad response\n");
            if (remoteVersion < 25) Util.writeASCII(out, RSYNCD_underscoreEXIT + "\n");
            socket.close();
        ...
        if (!module.users.contains(user)) {
            Util.writeASCII(out, AT_underscoreERROR + ": user " + user + " not allowed\n");
            if (remoteVersion < 25) Util.writeASCII(out, RSYNCD_underscoreEXIT + "\n");
            socket.close();
        ...
        while ((line = secrets.readLine()) != null) {
            if (line.startsWith(user + ":")) {
                ...
                if (hash.equals(response)) {
                    ...
                    Util.writeASCII(out, AT_underscoreERROR + ": auth failed on module " + module.name + "\n");
                    if (remoteVersion < 25) Util.writeASCII(out, RSYNCD_underscoreEXIT + "\n");
                    socket.close();
                    secrets.close();
