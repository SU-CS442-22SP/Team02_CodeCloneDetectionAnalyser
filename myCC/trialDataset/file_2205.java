    public EFaxResult sendFax(ototype.SendFaxWrapper parameters) {
        ...
        String faxKey = getSegquence("t_underscorefax_underscoresend") + "";
        String sql = "insert into t_underscorefax_underscoresend(faxKey,userID,appcode,sendername," + "sendernumber,sendercompany,sendtime,accountId, userId2, PID, moduleId, CDRType) values(?,?,?,?,?,?,?,?,?,?,?,?)";
        ...
        try {
            ...
            pstmt.executeUpdate();
            sql = "insert into t_underscorefax_underscorecontactor(faxKey,contactorID,contactor,faxnumber,company) values(?,?,?,?,?)";
            ...
            pstmt.executeBatch();
            sql = "insert into t_underscorefax_underscorefile(faxKey,fileID,filename,filetype,fileurl,faxpages) values(?,?,?,?,?,?)";
            pstmt = conn.prepareStatement(sql);
            FaxFile[] files = fax.getFiles();
            for (int h = 0; h < files.length; h++) {
                String fileID = getSegquence("t_underscoreFax_underscorefile") + "";
                pstmt.setString(1, faxKey);
