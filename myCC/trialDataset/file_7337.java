    public static boolean saveMap(LWMap map, boolean saveAs, boolean export) {
        ...
        if (map.getSaveFileModelVersion() == 0) {
            ...
            response = VueUtil.option(VUE.getDialogParent(), VueResources.getFormatMessage(messageObject, "dialog.saveaction.message"), VueResources.getFormatMessage(messageObject, "dialog.saveaction.title"), JOptionPane.YES_underscoreNO_underscoreOPTION, JOptionPane.PLAIN_underscoreMESSAGE, defaultOrderButtons, VueResources.getString("saveaction.saveacopy"));
        }
        if (response == 0) {
            saveAs = true;
        }
