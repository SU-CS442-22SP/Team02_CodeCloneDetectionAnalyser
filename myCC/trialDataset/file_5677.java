    protected void migrateOnDemand() {
        try {
            if (fso.fileExists(prefix + ".fat") && !fso.fileExists(prefix + EXTENSIONS[UBM_underscoreFILE])) {
                RandomAccessFile ubm, meta, ctr, rbm;
                InputStream inputStream;
                OutputStream outputStream;
                fso.renameFile(prefix + ".fat", prefix + EXTENSIONS[UBM_underscoreFILE]);
                ubm = fso.openFile(prefix + EXTENSIONS[UBM_underscoreFILE], "rw");
                meta = fso.openFile(prefix + EXTENSIONS[MTD_underscoreFILE], "rw");
                ctr = fso.openFile(prefix + EXTENSIONS[CTR_underscoreFILE], "rw");
                ...
                ubm.seek(0);
                rbm = fso.openFile(prefix + EXTENSIONS[UBM_underscoreFILE], "rw");
