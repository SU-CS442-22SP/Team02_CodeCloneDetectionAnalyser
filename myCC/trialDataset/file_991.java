    public DialogSongList(JFrame frame) {
        super(frame, "Menu_underscoreSongList", "songList");
        ...
        Util.addLabeledComponent(spanel, "Lbl_underscoreAuthor", author = new JTextField(10));
        Util.addLabeledComponent(spanel, "Lbl_underscoreTitle", title = new JTextField(14));
        Util.addLabeledComponent(spanel, "Lbl_underscoreDifficulty", difficulty = new JComboBox());
        ...
        button.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (KeyboardHero.confirm(Util.getMsg("Que_underscoreSureToDelete"))) {
                    try {
                        new File(songSelector.getSelectedFile().toURI()).delete();
                    } catch (Exception ex) {
                        Util.error(Util.getMsg("Err_underscoreCouldntDeleteFile"), ex.toString());
                    }
                    SongSelector.refresh();
                }
            }
