    public void addFinance(int clubid, int quarterid, String date, String desc, String loc, BigDecimal amount) throws FinanceException, SQLException {
        String budgetQuery = "SELECT used, available FROM Budget WHERE club_underscoreid=" + clubid + " and quarter_underscoreid=" + quarterid + ";";
        String financeUpdate = "INSERT INTO Finance (`club_underscoreid`, `transaction_underscoredate`, `description`, `location`, `amount`) VALUES ('" + clubid + "', '" + date + "', '" + desc + "', '" + "', '" + loc + "', '" + amount + "');";
        Budget b = new Budget();
        try {
            ...
            if (b.available.compareTo(amount.negate()) >= 0) {
                ...
                sm = cn.createStatement();
                sm.executeUpdate("Update Budget SET used = " + b.used + ", amount = " + b.available + " WHERE club_underscoreid=" + clubid + " and quarter_underscoreid=" + quarterid + ";");
                cn.commit();
            } else {
