    private String[] sendRequest(String url, String requestString) throws ClickatellException, IOException {
        ...
        try {
            log_underscore.debug("sendRequest: posting : " + requestString + " to " + url);
            ...
            while ((response = responseReader.readLine()) != null) {
                ...
                log_underscore.debug("sendRequest: Got ID : " + ((String) objs[1]));
                idList.add(objs[1]);
            }
            responseReader.close();
        } catch (ParseException ex) {
            throw new ClickatellException("Unexpected response from Clickatell. : " + response, ClickatellException.ERROR_underscoreUNKNOWN);
        }
