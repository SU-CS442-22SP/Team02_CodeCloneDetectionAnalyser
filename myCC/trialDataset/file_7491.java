    public boolean gerarTutorialPage() {
        try {
            ...
            srcCss3.close();
            destCss3.close();
            FileChannel srcCss4 = new FileInputStream(local + "/bin/style/layout_underscoreie.css").getChannel();
            FileChannel destCss4 = new FileOutputStream(cssDir + "/layout_underscoreie.css").getChannel();
            destCss4.transferFrom(srcCss4, 0, srcCss4.size());
            srcCss4.close();
            destCss4.close();
            FileChannel srcCss5 = new FileInputStream(local + "/bin/style/elementos_underscoreie.css").getChannel();
            FileChannel destCss5 = new FileOutputStream(cssDir + "/elementos_underscoreie.css").getChannel();
            destCss5.transferFrom(srcCss5, 0, srcCss5.size());
            srcCss5.close();
            destCss5.close();
            FileChannel srcCss6 = new FileInputStream(local + "/bin/style/estilo-cap-lic-mid_underscoreie.css").getChannel();
            FileChannel destCss6 = new FileOutputStream(cssDir + "/estilo-cap-lic-mid_underscoreie.css").getChannel();
            ...
            JOptionPane.showMessageDialog(null, "Web Site gerado com sucesso", "\\o/", JOptionPane.INFORMATION_underscoreMESSAGE);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            System.err.println("Site não gerado");
            JOptionPane.showMessageDialog(null, "Web Site não gerado corretamente", "Ops...", JOptionPane.ERROR_underscoreMESSAGE);
            return false;
        }
    }
