    private void copyfile(File srcFile, File dstDir) throws FileNotFoundException, IOException {
        if (srcFile.isDirectory()) {
            ...
            for (i = 0; i < srcFile.length() - BLOCK_underscoreLENGTH; i += BLOCK_underscoreLENGTH) {
                synchronized (this) {
                    inChannel.transferTo(i, BLOCK_underscoreLENGTH, outChannel);
                    copiedSize += BLOCK_underscoreLENGTH;
                }
            }
            synchronized (this) {
