    public void actualizar() throws SQLException, ClassNotFoundException, Exception {
        ...
        try {
            ...
            int off_underscorecodi = 0;
            try {
                off_underscorecodi = Integer.parseInt(oficinafisica);
            } catch (Exception e) {
            }
            ...
            ms.execute();
            ms.close();
            String insertOfifis = "INSERT INTO BZSALOFF (FOSANOEN, FOSNUMEN, FOSCAGCO, OFS_underscoreCODI)" + "VALUES (?,?,?,?)";
            ms = conn.prepareStatement(insertOfifis);
            ms.setInt(1, fzaanoe);
            ms.setInt(2, fzanume);
            ms.setInt(3, fzacagc);
            ms.setInt(4, off_underscorecodi);
            ms.execute();
            ms.close();
