    public MapInfo getMap(double latitude, double longitude, double wanted_underscoremapblast_underscorescale, int image_underscorewidth, int image_underscoreheight, String file_underscorepath_underscorewo_underscoreextension, ProgressListener progress_underscorelistener) throws IOException {
        try {
            if (web_underscorerequest_underscore == null) {
                web_underscorerequest_underscore = new HttpRequester(HOST_underscoreNAME);
            }
            int zoom_underscoreindex = getZoomLevelIndex(wanted_underscoremapblast_underscorescale);
            int google_underscorezoom_underscorelevel = GOOGLE_underscoreZOOM_underscoreLEVELS[zoom_underscoreindex];
            double mapblast_underscorescale = POSSIBLE_underscoreGOOGLE_underscoreSCALES[zoom_underscoreindex];
            Tile tile = new Tile(latitude, longitude, google_underscorezoom_underscorelevel);
            SimplePoint coords = tile.getTileLatLong();
            SimplePoint google_underscorexy = tile.getTileCoord();
            MapInfo map_underscoreinfo = new MapInfo();
            map_underscoreinfo.setLatitude(coords.getX());
            map_underscoreinfo.setLongitude(coords.getY());
            map_underscoreinfo.setScale((float) mapblast_underscorescale);
            map_underscoreinfo.setWidth(256);
            map_underscoreinfo.setHeight(256);
            map_underscoreinfo.setFilename(file_underscorepath_underscorewo_underscoreextension + "png");
            Object[] params = new Object[] { new Integer(google_underscorexy.getX()), new Integer(google_underscorexy.getY()), new Integer(google_underscorezoom_underscorelevel) };
            MessageFormat message_underscoreformat = new MessageFormat(GOOGLE_underscoreMAPS_underscoreURL, Locale.US);
            String url_underscorestring = message_underscoreformat.format(params);
