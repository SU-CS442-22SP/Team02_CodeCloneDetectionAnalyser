    public String getNextObjectId() throws SQLException {
        ...
        String s1 = "SELECT nextserial FROM serials WHERE tablename = 'SERVER_underscoreOIDS'";
        String s2;
        try {
            ...
            if (!rs.next()) {
                s2 = "insert into serials (tablename,nextserial) values ('SERVER_underscoreOIDS', " + (nextserial) + ")";
            } else {
                nextserial = rs.getLong(1) + 1;
                s2 = "update serials set nextserial=" + (nextserial) + " where tablename='SERVER_underscoreOIDS'";
            }
