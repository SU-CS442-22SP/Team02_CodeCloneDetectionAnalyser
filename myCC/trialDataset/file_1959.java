    ...
    public User createUser(User bean) throws SitoolsException {
        ...
        if (!User.isValid(bean)) {
            throw new SitoolsException("CREATE_underscoreUSER_underscoreMALFORMED");
        }
        Connection cx = null;
        try {
            cx = ds.getConnection();
            cx.setAutoCommit(false);
            PreparedStatement st = cx.prepareStatement(jdbcStoreResource.CREATE_underscoreUSER);
            int i = 1;
            ...
            try {
                ...
                e1.printStackTrace();
                throw new SitoolsException("CREATE_underscoreUSER ROLLBACK" + e1.getMessage(), e1);
            }
            e.printStackTrace();
            throw new SitoolsException("CREATE_underscoreUSER " + e.getMessage(), e);
        } finally {
            closeConnection(cx);
        }
