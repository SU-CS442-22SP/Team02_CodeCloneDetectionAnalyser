    private void LoadLoginInfo() {
        m_underscorePwdList.removeAllElements();
        String szTemp = null;
        int iIndex = 0;
        int iSize = m_underscoreUsrList.size();
        for (int i = 0; i < iSize; i++) m_underscorePwdList.add("");
        try {
            if ((m_underscoreUsrList.size() > 0) && m_underscorebSavePwd) {
                ...
                if (file.exists()) {
                    ...
                    while (br.available() > 0) {
                        md.reset();
                        md.update(((String) m_underscoreUsrList.get(iLine)).getBytes());
                        ...
                        if (iSize > 0) {
                            ...
                            m_underscorePwdList.setElementAt(new String(cPwd), iLine);
                        }
                        iLine++;
                    }
                }
