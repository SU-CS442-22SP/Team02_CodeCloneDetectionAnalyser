    public InputLattice(String file, int type, boolean convertJoinMeet) throws FileNotFoundException, IOException {
        ...
        switch(type) {
            ...
            case STRING:
                ...
                break;
            case URL:
                URL url = new URL("http", Config.HOST, Config.PORT, "/" + Config.HASSE + "/" + Config.URL_underscoreSTREAMER + file);
        ...
        in.wordChars('^', '_underscore');
        in.wordChars('*', '.');
        while (in.nextToken() != StreamTokenizer.TT_underscoreEOF) {
            if (in.ttype == StreamTokenizer.TT_underscoreWORD || in.ttype == StreamTokenizer.TT_underscoreNUMBER || in.ttype == '"') {
                if (in.ttype == StreamTokenizer.TT_underscoreNUMBER) {
