    public String getNextSequence(Integer id) throws ApplicationException {
        ...
        synchronized (lock) {
            synchronized (dbConn) {
                try {
                    preStat = dbConn.prepareStatement("SELECT PREFIX,SUFFIX,START_underscoreNO,LENGTH,CURRENT_underscoreNO,INCREMENT FROM FORM_underscoreRECORD WHERE ID=?");
                    ...
                    throw new ApplicationException("errors.framework.get_underscorenext_underscoreseq", e);
                } finally {
                    try {
                        rs.close();
                    } catch (Exception ignore) {
                ...
                if (!noRecordMatch && formReferenceID != null) {
                    try {
                        ...
                        do {
                            preStat = dbConn.prepareStatement("UPDATE FORM_underscoreRECORD SET CURRENT_underscoreNO=? WHERE ID=?");
