    protected PredicateAnnotationRecord generatePredicateAnnotationRecord(PredicateAnnotationRecord par, String miDescriptor) {
        ...
        String hashKey = annotClass + CLASS_underscoreSIG_underscoreSEPARATOR_underscoreSTRING + methodName;
        PredicateAnnotationRecord gr = _underscoregeneratedPredicateRecords.get(hashKey);
        if (gr != null) {
            _underscoresharedAddData.cacheInfo.incCombinePredicateCacheHit();
            return gr;
        } else {
            _underscoresharedAddData.cacheInfo.incCombinePredicateCacheMiss();
        }
        String predicateClass = ((_underscorepredicatePackage.length() > 0) ? (_underscorepredicatePackage + ".") : "") + annotClass + "Pred";
        ClassFile predicateCF = null;
        File clonedFile = new File(_underscorepredicatePackageDir, annotClass.replace('.', '/') + "Pred.class");
        if (clonedFile.exists() && clonedFile.isFile() && clonedFile.canRead()) {
            ...
            try {
                _underscoretemplatePredicateClassFile.write(baos);
        ...
        if (predicateMI == null) {
            ...
            if ((par.combineMode == Combine.Mode.OR) || (par.combineMode == Combine.Mode.XOR) || (par.combineMode == Combine.Mode.IMPLIES)) {
                il.insertInstr(new GenericInstruction(Opcode.ICONST_underscore0), predicateMI.getCodeAttributeInfo());
            } else {
                il.insertInstr(new GenericInstruction(Opcode.ICONST_underscore1), predicateMI.getCodeAttributeInfo());
            }
            boolean res;
            res = il.advanceIndex();
            assert res == true;
            ...
            for (PredicateAnnotationRecord memberPAR : memberPARs) {
                ++memberCount;
                il.insertInstr(new GenericInstruction(Opcode.ALOAD_underscore0), predicateMI.getCodeAttributeInfo());
                res = il.advanceIndex();
                assert res == true;
                int curStack = 1;
                if (memberPAR.passArguments) {
                    if (par.passArguments) {
                        il.insertInstr(new GenericInstruction(Opcode.ALOAD_underscore1), predicateMI.getCodeAttributeInfo());
                        res = il.advanceIndex();
                        assert res == true;
                        curStack += 1;
                    }
            ...
            if (par.combineMode == Combine.Mode.XOR) {
                ...
                res = il.advanceIndex();
                assert res == true;
                WideBranchInstruction br2 = new WideBranchInstruction(Opcode.GOTO_underscoreW, il.getIndex() + 1);
                ...
                assert res == true;
                BranchInstruction br1 = new BranchInstruction(Opcode.IF_underscoreICMPEQ, jumpIndex);
        ...
        _underscoregeneratedPredicateRecords.put(hashKey, gr);
        return gr;
    }
