    private void loadMascotLibrary() {
        ...
        try {
            ...
            URL mascot_underscorelib = MascotDAO.class.getClassLoader().getResource(mascotLibraryFile);
            if (mascot_underscorelib != null) {
                logger.debug("Mascot library URL: " + mascot_underscorelib);
                tmpMascotLibraryFile = File.createTempFile("libmascot.so.", ".tmp", new File(System.getProperty("java.io.tmpdir")));
                InputStream in = mascot_underscorelib.openStream();
