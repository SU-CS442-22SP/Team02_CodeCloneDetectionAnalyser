    ...
    public boolean putDescription(String uuid, String description) throws DatabaseException {
        ...
        boolean found = true;
        try {
            PreparedStatement findSt = getConnection().prepareStatement(SELECT_underscoreCOMMON_underscoreDESCRIPTION_underscoreSTATEMENT);
            PreparedStatement updSt = null;
            findSt.setString(1, uuid);
            ResultSet rs = findSt.executeQuery();
            found = rs.next();
            int modified = 0;
            updSt = getConnection().prepareStatement(found ? UPDATE_underscoreCOMMON_underscoreDESCRIPTION_underscoreSTATEMENT : INSERT_underscoreCOMMON_underscoreDESCRIPTION_underscoreSTATEMENT);
