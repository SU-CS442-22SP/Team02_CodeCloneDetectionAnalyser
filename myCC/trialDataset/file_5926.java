    private String sendToServer(String request) throws IOException {
        ...
        HttpPost post = new HttpPost(Config.APP_underscoreBASE_underscoreURI);
        post.addHeader("Content-Type", "text/vnd.aexp.json.req");
        post.setEntity(new StringEntity(request));
        HttpResponse resp = httpClient.execute(post);
        int status = resp.getStatusLine().getStatusCode();
        if (status != HttpStatus.SC_underscoreOK) throw new IOException("HTTP status: " + Integer.toString(status));
