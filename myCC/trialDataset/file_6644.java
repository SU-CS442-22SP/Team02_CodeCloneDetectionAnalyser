    ...
    public void execute() throws BuildException {
        final String GC_underscoreUSERNAME = "google-code-username";
        final String GC_underscorePASSWORD = "google-code-password";
        ...
        try {
            ...
            cfg.loadFromXML(fin);
            fin.close();
            if (!cfg.containsKey(GC_underscoreUSERNAME)) throw new BuildException("undefined " + GC_underscoreUSERNAME + " in " + this.config);
            if (!cfg.containsKey(GC_underscorePASSWORD)) throw new BuildException("undefined " + GC_underscorePASSWORD + " in " + this.config);
            HttpClient client = new HttpClient();
            post = new PostMethod("https://" + projectName + ".googlecode.com/files");
            post.addRequestHeader("User-Agent", getClass().getName());
            post.addRequestHeader("Authorization", "Basic " + Base64.encode(cfg.getProperty(GC_underscoreUSERNAME) + ":" + cfg.getProperty(GC_underscorePASSWORD)));
