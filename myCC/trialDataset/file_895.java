    public boolean saveTemplate(Template t) {
        try {
            ...
            if (rset.next()) {
                ...
                int templateid = Integer.parseInt(executeMySQLGet("SELECT LAST_underscoreINSERT_underscoreID()"));
                t.setID(templateid);
                ...
                try {
                    while (iter.hasNext()) {
                        ...
                        pstmt.execute();
                        f.setID(Integer.parseInt(executeMySQLGet("SELECT LAST_underscoreINSERT_underscoreID()")));
                    }
                } catch (SQLException e) {
                    conn.rollback();
                    conn.setAutoCommit(true);
