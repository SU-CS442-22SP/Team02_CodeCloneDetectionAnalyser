    public int addRecipe(Recipe recipe) throws Exception {
        ...
        try {
            conn = getConnection();
            pst1 = conn.prepareStatement("INSERT INTO recipes (name, instructions, category_underscoreid) VALUES (?, ?, ?)");
            pst1.setString(1, recipe.getName());
            pst1.setString(2, recipe.getInstructions());
            pst1.setInt(3, recipe.getCategoryId());
            if (pst1.executeUpdate() > 0) {
                pst2 = conn.prepareStatement("SELECT recipe_underscoreid FROM recipes WHERE name = ? AND instructions = ? AND category_underscoreid = ?");
