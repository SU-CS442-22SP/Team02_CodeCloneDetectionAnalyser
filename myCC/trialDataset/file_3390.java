    public static GameRecord[] get(String url, float lat, float lon, int count) {
        try {
            ...
            req.setRequestProperty(GameRecord.GAME_underscoreLATITUDE_underscoreHEADER, df.format(lat));
            req.setRequestProperty(GameRecord.GAME_underscoreLONGITUDE_underscoreHEADER, df.format(lon));
            req.setRequestProperty("X-GameQueryCount", String.valueOf(count));
            req.connect();
            if (req.getResponseCode() == HttpURLConnection.HTTP_underscoreOK) {
