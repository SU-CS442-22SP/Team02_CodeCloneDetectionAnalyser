    public void _underscoresaveWebAsset(ActionRequest req, ActionResponse res, PortletConfig config, ActionForm form, User user, String subcmd) throws WebAssetException, Exception {
        long maxsize = 50;
        long maxwidth = 3000;
        long maxheight = 3000;
        long minheight = 10;
        ...
        try {
            ...
            _underscorecheckUserPermissions(folder, user, PERMISSION_underscoreWRITE);
            ...
            for (int k = 0; k < countFiles; k++) {
                ...
                if (fileName.length() > 0) {
                    ...
                    if ((FileFactory.existsFileName(folder, fileName))) {
                        ...
                        _underscoresetFilePermissions(folder, file, user);
                        fileCounter += 1;
                        if ((subcmd != null) && subcmd.equals(com.dotmarketing.util.Constants.PUBLISH)) {
                            try {
                                PublishFactory.publishAsset(file, httpReq);
                                if (fileCounter > 1) {
                                    SessionMessages.add(req, "message", "message.file_underscoreasset.save");
                                } else {
