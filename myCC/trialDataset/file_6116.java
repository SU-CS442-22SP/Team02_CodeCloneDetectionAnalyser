    public void importarHistoricoDeCotacoesDosPapeis(File[] pArquivosTXT, boolean pApagarDadosImportadosAnteriormente, Andamento pAndamento) throws FileNotFoundException, SQLException {
        if (pApagarDadosImportadosAnteriormente) {
            ...
            String sql = "TRUNCATE TABLE TMP_underscoreTB_underscoreCOTACAO_underscoreAVISTA_underscoreLOTE_underscorePDR";
            stmtLimpezaInicialDestino.executeUpdate(sql);
            sql = "TRUNCATE TABLE TMP_underscoreTB_underscoreCOTACAO_underscoreOUTROS_underscoreMERCADOS";
            stmtLimpezaInicialDestino.executeUpdate(sql);
        }
        final int TAMANHO_underscoreDO_underscoreREGISTRO = 245;
        long TAMANHO_underscoreDOS_underscoreMETADADOS_underscoreDO_underscoreARQUIVO = 2 * TAMANHO_underscoreDO_underscoreREGISTRO;
        ...
        int quantidadeEstimadaDeRegistros = (int) ((tamanhoDosArquivos - (pArquivosTXT.length * TAMANHO_underscoreDOS_underscoreMETADADOS_underscoreDO_underscoreARQUIVO)) / TAMANHO_underscoreDO_underscoreREGISTRO);
        String sqlMercadoAVistaLotePadrao = "INSERT INTO TMP_underscoreTB_underscoreCOTACAO_underscoreAVISTA_underscoreLOTE_underscorePDR(DATA_underscorePREGAO, CODBDI, CODNEG, TPMERC, NOMRES, ESPECI, PRAZOT, MODREF, PREABE, PREMAX, PREMIN, PREMED, PREULT, PREOFC, PREOFV, TOTNEG, QUATOT, VOLTOT, PREEXE, INDOPC, DATVEN, FATCOT, PTOEXE, CODISI, DISMES) VALUES(:DATA_underscorePREGAO, :CODBDI, :CODNEG, :TPMERC, :NOMRES, :ESPECI, :PRAZOT, :MODREF, :PREABE, :PREMAX, :PREMIN, :PREMED, :PREULT, :PREOFC, :PREOFV, :TOTNEG, :QUATOT, :VOLTOT, :PREEXE, :INDOPC, :DATVEN, :FATCOT, :PTOEXE, :CODISI, :DISMES)";
        ...
        stmtDestinoMercadoAVistaLotePadrao.setExecuteBatch(COMANDOS_underscorePOR_underscoreLOTE);
        String sqlOutrosMercados = "INSERT INTO TMP_underscoreTB_underscoreCOTACAO_underscoreOUTROS_underscoreMERCADOS(DATA_underscorePREGAO, CODBDI, CODNEG, TPMERC, NOMRES, ESPECI, PRAZOT, MODREF, PREABE, PREMAX, PREMIN, PREMED, PREULT, PREOFC, PREOFV, TOTNEG, QUATOT, VOLTOT, PREEXE, INDOPC, DATVEN, FATCOT, PTOEXE, CODISI, DISMES) VALUES(:DATA_underscorePREGAO, :CODBDI, :CODNEG, :TPMERC, :NOMRES, :ESPECI, :PRAZOT, :MODREF, :PREABE, :PREMAX, :PREMIN, :PREMED, :PREULT, :PREOFC, :PREOFV, :TOTNEG, :QUATOT, :VOLTOT, :PREEXE, :INDOPC, :DATVEN, :FATCOT, :PTOEXE, :CODISI, :DISMES)";
        OraclePreparedStatement stmtDestinoOutrosMercados = (OraclePreparedStatement) conDestino.prepareStatement(sqlOutrosMercados);
        stmtDestinoOutrosMercados.setExecuteBatch(COMANDOS_underscorePOR_underscoreLOTE);
        ...
        try {
            for (File arquivoTXT : pArquivosTXT) {
                ...
                int vDATA_underscorePREGAO;
                try {
                    in = new Scanner(new FileInputStream(arquivoTXT), Constantes.CONJUNTO_underscoreDE_underscoreCARACTERES_underscoreDOS_underscoreARQUIVOS_underscoreTEXTO_underscoreDA_underscoreBOVESPA.name());
                    String registro;
                    numeroDoRegistro = 0;
                    while (in.hasNextLine()) {
                        ++numeroDoRegistro;
                        registro = in.nextLine();
                        if (registro.length() != TAMANHO_underscoreDO_underscoreREGISTRO) throw new ProblemaNaImportacaoDeArquivo();
                        if (registro.startsWith("01")) {
                            stmtDestinoMercadoAVistaLotePadrao.clearParameters();
                            stmtDestinoOutrosMercados.clearParameters();
                            vDATA_underscorePREGAO = Integer.parseInt(registro.substring(2, 10).trim());
                            ...
                            stmtDestino.setIntAtName("DATA_underscorePREGAO", vDATA_underscorePREGAO);
