    private void addQDInformation() {
        try {
            ...
            if (qdDate > this._underscoreqdFileDate) {
                this._underscoreqdFileDate = qdDate;
                for (int ii = 0; ii < this._underscoreprojectInfo.size(); ii++) {
                    Information info = getInfo(ii);
                    if (info != null) {
                        info._underscoreqdValue = null;
                    }
                }
                Reader reader = null;
                if (_underscorelocal == true) {
                ...
                try {
                    ...
                    while ((line = file.readLine()) != null) {
                        if (line.startsWith("pg ")) {
                            this._underscoreqdDate = Long.parseLong(line.substring(3), 16);
                            this._underscoreqdDate = (this._underscoreqdDate + 946684800) * 1000;
                            ...
                            while ((line.length() > 0) && ((pos = line.indexOf(' ')) > 0)) {
                                ...
                                if (info._underscoreqdValue == null) {
                                    info._underscoreqdValue = value;
                                }
                            }
                        }
                    }
