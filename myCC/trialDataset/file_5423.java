    public void exe2(String[] args) {
        ...
        try {
            ...
            rs = stmt.executeQuery("SELECT * FROM MIS.MAP_underscorePUR0101 WHERE NOT EXISTS (SELECT 1 FROM MIS.RSCMGOOD@MIS030DB@DEVU01 WHERE GOODCD = PUM_underscoreCODE OR GOODCD = NEW_underscorePUM_underscoreCODE)");
            pstmt = con2.prepareStatement("INSERT INTO MIS.RSCMGOOD ( GOODCD,GOODFLAG,GOODNM,GOODHNGNM,GOODENGNM,GOODSPEC,GOODMODEL,ASETFLAG,LRGCD,MDLCD,SMLCD,EDICD,PRODCMPYCD,CMT,FSTRGSTRID,FSTRGSTDT,LASTUPDTRID,LASTUPDTDT,APPINSTDATA,MNGTFLAG) " + "VALUES ( ?,SUBSTR(?,1,1),?,?,?,?,NULL,'1',substr(?,2,2),substr(?,4,3),NULL,NULL,NULL,'OCS �����빰ǰ','MISASIS',TO_underscoreDATE('20111231','YYYYMMDD'),'MISASIS',TO_underscoreDATE('20111231','YYYYMMDD'),NULL,'N')");
            int count = 0;
            String goodcd = null;
            String goodnm = null;
            while (rs.next()) {
                count++;
                goodcd = rs.getString("PUM_underscoreCODE").toUpperCase();
                goodnm = rs.getString("PUM_underscoreHNAME");
                ...
                pstmt.setString(5, rs.getString("PUM_underscoreENAME"));
