    public static void putWithUserSettings(String from, String to, String renameTo, boolean binary, IProgressMonitor monitor) {
        ...
        int ftpqueuesize = PrefPageOne.getIntValue(CONSTANTS.PREF_underscoreFTPQUEUE);
        if (synchedSet.size() >= ftpqueuesize) {
            JobHandler.aquireFTPLock();
            try {
                ftpClient = new FTPClient();
                ftpClient.setRemoteAddr(InetAddress.getByName(PrefPageOne.getValue(CONSTANTS.PREF_underscoreHOST)));
                ftpClient.setControlPort(PrefPageOne.getIntValue(CONSTANTS.PREF_underscoreFTPPORT));
                ftpClient.connect();
                try {
                    Thread.sleep(1500);
                ...
                ftpClient.login((PrefPageOne.getValue(CONSTANTS.PREF_underscoreUSERNAME)), FTPUtils.decrypt(PrefPageOne.getValue(CONSTANTS.PREF_underscorePASSWORD)));
                try {
                    Thread.sleep(1000);
                } catch (InterruptedException e) {
                    e.printStackTrace();
