    ...
    protected void service(final HttpServletRequest req, final HttpServletResponse res) throws ServletException, IOException {
        ...
        if (null == path || "".equals(path)) res.sendRedirect(req.getServletPath() + "/"); else if ("/chart".equals(path)) {
        ...
        } else if (path.startsWith("/log_underscore")) {
            ...
            if (null == provider) {
                ...
                res.sendError(HttpServletResponse.SC_underscoreNOT_underscoreFOUND);
            } else {
            ...
            res.setDateHeader("Expires", new Date().getTime() + YEAR_underscoreIN_underscoreSECONDS * 1000L);
            res.setHeader("Cache-Control", "max-age=" + YEAR_underscoreIN_underscoreSECONDS);
