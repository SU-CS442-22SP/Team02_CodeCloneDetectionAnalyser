    private void updateIngredients(Recipe recipe, int id) throws Exception {
        ...
        try {
            conn = getConnection();
            pst1 = conn.prepareStatement("DELETE FROM ingredients WHERE recipe_underscoreid = ?");
            pst1.setInt(1, id);
            if (pst1.executeUpdate() >= 0) {
                pst2 = conn.prepareStatement("INSERT INTO ingredients (recipe_underscoreid, name, amount, measure_underscoreid, shop_underscoreflag) VALUES (?,?,?,?,?)");
