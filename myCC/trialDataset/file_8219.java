    ...
    public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
        ...
        if (currentMessage == null) {
            ...
            if (m.isChunked()) {
                ...
                List<String> encodings = m.getHeaders(HttpHeaders.Names.TRANSFER_underscoreENCODING);
                encodings.remove(HttpHeaders.Values.CHUNKED);
                if (encodings.isEmpty()) {
                    m.removeHeader(HttpHeaders.Names.TRANSFER_underscoreENCODING);
                }
            ...
            if (maxContentLength != -1 && (localFile.length() > (maxContentLength - chunk.getContent().readableBytes()))) {
                ...
                if (chunk.isLast()) {
                    this.out.flush();
                    this.out.close();
                    currentMessage.setHeader(HttpHeaders.Names.CONTENT_underscoreLENGTH, String.valueOf(localFile.length()));
