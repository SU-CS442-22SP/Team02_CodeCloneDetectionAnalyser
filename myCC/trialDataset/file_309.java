    private String getAuthCookie(boolean invalidate) {
        if (resources.getBoolean(R.bool.dev)) {
            return "dev_underscoreappserver_underscorelogin=get_underscoreview@localhost.devel:false:18580476422013912411";
        } else {
            try {
                ...
                String authToken = accountsManager.getAuthToken(account, "ah", null, act, null, null).getResult().get(AccountManager.KEY_underscoreAUTHTOKEN).toString();
                ...
                HttpGet httpget = new HttpGet("http://" + resources.getString(R.string.host) + "/_underscoreah/login?auth=" + authToken);
