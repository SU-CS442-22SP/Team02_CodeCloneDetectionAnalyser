    public boolean authenticate(String userName, String loginPassword) {
        ...
        env.put(Context.PROVIDER_underscoreURL, "ldap://" + server + ":" + port + "/" + baseDn);
        env.put(Context.SECURITY_underscoreAUTHENTICATION, "simple");
        if ("ldapAuthBind".equals(type)) {
            env.put(Context.SECURITY_underscorePRINCIPAL, bindDn);
            env.put(Context.SECURITY_underscoreCREDENTIALS, loginPassword);
            try {
        ...
        env.put(Context.SECURITY_underscorePRINCIPAL, userDn);
        env.put(Context.SECURITY_underscoreCREDENTIALS, password);
        DirContext ctx = null;
        NamingEnumeration<SearchResult> results = null;
        try {
            ctx = new InitialDirContext(env);
            SearchControls controls = new SearchControls();
            controls.setSearchScope(SearchControls.SUBTREE_underscoreSCOPE);
