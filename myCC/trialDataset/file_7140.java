    private List<String> getSignatureResourceNames(URL url) throws IOException, ParserConfigurationException, SAXException, TransformerException, JAXBException {
        ...
        while (null != (zipEntry = zipInputStream.getNextZipEntry())) {
            if ("_underscorerels/.rels".equals(zipEntry.getName())) {
                break;
            }
        }
        if (null == zipEntry) {
            LOG.debug("no _underscorerels/.rels relationship part present");
        ...
        for (CTRelationship packageRelationship : packageRelationshipList) {
            if (OOXMLSignatureVerifier.DIGITAL_underscoreSIGNATURE_underscoreORIGIN_underscoreREL_underscoreTYPE.equals(packageRelationship.getType())) {
        ...
        String dsOriginRels = dsOriginSegment + "_underscorerels/" + dsOriginName + ".rels";
        ...
        for (CTRelationship dsoRelationship : dsoRelationshipList) {
            if (OOXMLSignatureVerifier.DIGITAL_underscoreSIGNATURE_underscoreREL_underscoreTYPE.equals(dsoRelationship.getType())) {
