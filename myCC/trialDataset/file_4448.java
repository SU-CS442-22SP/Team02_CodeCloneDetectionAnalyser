    public void launchJob(final String workingDir, final AppConfigType appConfig) throws FaultType {
        ...
        HttpServletRequest req = (HttpServletRequest) mc.getProperty(HTTPConstants.MC_underscoreHTTP_underscoreSERVLETREQUEST);
        String clientDN = (String) req.getAttribute(GSIConstants.GSI_underscoreUSER_underscoreDN);
        if (clientDN != null) {
        ...
        try {
            status.setCode(GramJob.STATUS_underscorePENDING);
            status.setMessage("Launching executable");
            status.setBaseURL(new URI(AppServiceImpl.tomcatURL + jobID));
        } catch (MalformedURIException mue) {
            logger.error("Cannot convert base_underscoreurl string to URI - " + mue.getMessage());
            throw new FaultType("Cannot convert base_underscoreurl string to URI - " + mue.getMessage());
        }
        if (!AppServiceImpl.dbInUse) {
            ...
            String sqlStmt = "insert into job_underscorestatus(job_underscoreid, code, message, base_underscoreurl, " + "client_underscoredn, client_underscoreip, service_underscorename, start_underscoretime, last_underscoreupdate) " + "values ('" + jobID + "', " + status.getCode() + ", " + "'" + status.getMessage() + "', " + "'" + status.getBaseURL() + "', " + "'" + clientDN + "', " + "'" + remoteIP + "', " + "'" + serviceName + "', " + "'" + time + "', " + "'" + time + "');";
            try {
        ...
        if (AppServiceImpl.drmaaInUse) {
            ...
            try {
                ...
                logger.error(ex);
                status.setCode(GramJob.STATUS_underscoreFAILED);
            ...
            status.setCode(GramJob.STATUS_underscoreACTIVE);
            ...
            try {
                ...
                GSSCredential gssCred = new GlobusGSSCredentialImpl(globusCred, GSSCredential.INITIATE_underscoreAND_underscoreACCEPT);
        ...
        new Thread() {
            ...
            public void run() {
                ...
                if (AppServiceImpl.drmaaInUse || !AppServiceImpl.globusInUse) {
                    ...
                    status.setCode(GramJob.STATUS_underscoreSTAGE_underscoreOUT);
                ...
                if (!AppServiceImpl.dbInUse) {
                    ...
                    String sqlStmt = "insert into job_underscoreoutput(job_underscoreid, std_underscoreout, std_underscoreerr) " + "values ('" + jobID + "', " + "'" + outputs.getStdOut().toString() + "', " + "'" + outputs.getStdErr().toString() + "');";
                    Statement stmt = null;
                    try {
                    ...
                    for (int i = 0; i < outputFile.length; i++) {
                        sqlStmt = "insert into output_underscorefile(job_underscoreid, name, url) " + "values ('" + jobID + "', " + "'" + outputFile[i].getName() + "', " + "'" + outputFile[i].getUrl().toString() + "');";
                        try {
                            stmt = conn.createStatement();
                            stmt.executeUpdate(sqlStmt);
                        } catch (SQLException e) {
                            status.setCode(GramJob.STATUS_underscoreFAILED);
                            status.setMessage("Cannot update output_underscorefile DB after finish - " + e.getMessage());
                            logger.error(e);
                            try {
                                updateStatusInDatabase(jobID, status);
                            } catch (SQLException se) {
                ...
                if (terminatedOK()) {
                    status.setCode(GramJob.STATUS_underscoreDONE);
