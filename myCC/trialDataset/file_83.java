    ...
    protected void processDownloadAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (documentElement.getLocalName().equals("null")) {
            response.sendError(HttpServletResponse.SC_underscoreUNAUTHORIZED);
            return;
        }
        InputStream is = null;
        try {
            ...
            if (StringUtils.equals(sourceType, "cifsSource")) {
                ...
                response.sendError(HttpServletResponse.SC_underscoreINTERNAL_underscoreSERVER_underscoreERROR);
                return;
            }
            if (StringUtils.isBlank(mimeType) && StringUtils.isBlank(encoding)) {
                response.setContentType(Definitions.MIMETYPE_underscoreBINARY);
            } else if (StringUtils.isBlank(encoding)) {
                response.setContentType(mimeType);
            } else {
                response.setContentType(mimeType + ";charset=" + encoding);
            }
            if (request.getParameterMap().containsKey(Definitions.REQUEST_underscorePARAMNAME_underscoreDOWNLOAD)) {
