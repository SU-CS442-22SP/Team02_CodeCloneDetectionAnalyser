    public String getPassword(URI uri) {
        if (_underscoregetPassword(uri) != null) return _underscoregetPassword(uri);
        String result = null;
        try {
            String sUri = scrubURI(uri);
            URL url = new URL(TEMP_underscorePASSWORD_underscoreSERVICE_underscoreURL + "?SID=" + sessionId + "&ruri=" + URLEncoder.encode(sUri, "UTF-8"));
