        ...
        if ((Boolean) info.get("submit")) {
            try {
                ...
                if (ex.getMessage().toLowerCase().contains("unable")) {
                    JOptionPane.showMessageDialog(null, "Database problem occured, please try again later", "Error", JOptionPane.ERROR_underscoreMESSAGE);
                    passed = true;
                    testVar = false;
                } else {
                    passed = Boolean.parseBoolean(ex.getMessage());
                    JOptionPane.showMessageDialog(null, "Invallid userdata, try again", "Invallid userdata", JOptionPane.ERROR_underscoreMESSAGE);
                }
            }
        } else {
            if (new File(userdataFile).exists()) {
                ...
                testVar = false;
                JOptionPane.showMessageDialog(null, "No userdata was entered\nNo tests will be executed until you enter them ", "Warning", JOptionPane.ERROR_underscoreMESSAGE);
            }
            passed = true;
        }
        while (!passed) {
            ...
            if ((Boolean) info.get("submit")) {
                ...
                if (new File(userdataFile).exists()) {
                    ...
                    testVar = false;
                    JOptionPane.showMessageDialog(null, "No userdata was entered\nNo tests will be executed untill u enter them ", "Warning", JOptionPane.ERROR_underscoreMESSAGE);
                }
                passed = true;
            }
        }
