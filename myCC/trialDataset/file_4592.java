    public static void toZip(File zippedFile, File[] filesToZip, String zipComment, boolean savePath, int compressionLevel) throws IOException, FileNotFoundException, ZipException {
        if (zippedFile != null && filesToZip != null) {
            ...
            if (ZIP_underscoreNO_underscoreCOMPRESSION <= compressionLevel && compressionLevel <= ZIP_underscoreMAX_underscoreCOMPRESSION) out.setLevel(compressionLevel); else out.setLevel(ZIP_underscoreMAX_underscoreCOMPRESSION);
            ...
            out.close();
        } else throw new ZipException(MAIN_underscoreRESOURCE_underscoreBUNDLE.getString("default.ZipException.text"));
    }
