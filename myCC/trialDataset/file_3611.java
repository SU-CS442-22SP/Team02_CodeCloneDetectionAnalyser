    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String myFile = (request.getQueryString() == null) ? "empty" : PublicEncryptionFactory.digestString(parsedQueryString).replace('\\', '_underscore').replace('/', '_underscore');
        myFile = Config.getStringProperty("PATH_underscoreTO_underscoreTITLE_underscoreIMAGES") + myFile + ".png";
        ...
        if (!file.exists() || (request.getParameter("nocache") != null)) {
            ...
            file.createNewFile();
            try {
                String font_underscorefile = "/WEB-INF/fonts/arial.ttf";
                if (request.getParameter("font") != null) {
                    font_underscorefile = "/WEB-INF/fonts/" + request.getParameter("font");
                }
                font_underscorefile = ctx.getRealPath(font_underscorefile);
                float size = 20.0f;
                ...
                Font font = Font.createFont(Font.TRUETYPE_underscoreFONT, new FileInputStream(font_underscorefile));
                font = font.deriveFont(size);
                BufferedImage buffer = new BufferedImage(1, 1, BufferedImage.TYPE_underscoreINT_underscoreRGB);
                Graphics2D g2 = buffer.createGraphics();
                if (request.getParameter("aa") != null) {
                    g2.setRenderingHint(RenderingHints.KEY_underscoreANTIALIASING, RenderingHints.VALUE_underscoreANTIALIAS_underscoreON);
                } else {
                    g2.setRenderingHint(RenderingHints.KEY_underscoreANTIALIASING, RenderingHints.VALUE_underscoreANTIALIAS_underscoreOFF);
                }
                ...
                buffer = new BufferedImage(width, height - offSet, BufferedImage.TYPE_underscoreINT_underscoreARGB);
