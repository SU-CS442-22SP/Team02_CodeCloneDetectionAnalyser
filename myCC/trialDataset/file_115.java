    ...
    protected Set<DataRecord> retrieveRecords(DataTemplate template) {
        ...
        String select = prefix + " SELECT ?resource WHERE { { ?resource rdf:type " + "<http://dbpedia.org/class/yago/" + StringUtils.toCamelCase(query) + "> } UNION { ?resource skos:subject <http://dbpedia.org/resource/Category:" + query.replaceAll(" ", "_underscore") + "> } }";
        Document doc = null;
