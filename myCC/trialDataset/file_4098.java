    ...
    public DataUpdateResult<Record> updateRecord(String authToken, Record record, Filter filter, Field sourceField, InputModel inputmodel) throws DataOperationException {
        ...
        if (record != null) {
            ...
            try {
                ...
                recordUpdateResult.setMessage(messages.server_underscorerecord_underscoreupdate_underscoresuccess(""));
                recordUpdateResult.setSuccessful(true);
                long userId = getSignedOnUser(authToken).getUserId();
                AuditTrailManager.updateAuditTrail(connection, AuditTrailManager.createAuditTrailEvent(record, userId, AuditableEvent.EVENTYPE_underscoreUPDATE), authToken, session);
                    ...
                    while (rItr.hasNext()) {
                        ...
                        if (r.getRecordattributeid() > 0) {
                            if (r.getArchivedtimestamp() == null) {
                                ...
                                AuditTrailManager.updateAuditTrail(connection, AuditTrailManager.createAuditTrailEvent(r, userId, AuditableEvent.EVENTYPE_underscoreUPDATE), authToken, session);
                            } else {
                                ...
                                updateRecordAttribute.executeUpdate();
                                AuditTrailManager.updateAuditTrail(connection, AuditTrailManager.createAuditTrailEvent(r, userId, AuditableEvent.EVENTYPE_underscoreDELETE), authToken, session);
                            }
                        } else {
                            ...
                            if (result.next()) {
                                ...
                                AuditTrailManager.updateAuditTrail(connection, AuditTrailManager.createAuditTrailEvent(r, userId, AuditableEvent.EVENTYPE_underscoreCREATE), authToken, session);
                            }
                        }
                    }
                }
                ...
                if (!updated && connection != null) {
                    try {
                        ...
                        LogUtility.log(Level.SEVERE, session, messages.log_underscoredb_underscorerollback(""), e1, authToken);
                        e1.printStackTrace();
                    }
                }
                recordUpdateResult.setFailed(true);
                if (updated) {
                    recordUpdateResult.setMessage(messages.server_underscorerecord_underscoreupdate_underscoreretrieve(""));
                    recordUpdateResult.setException(e);
                    LogUtility.log(Level.SEVERE, session, messages.server_underscorerecord_underscoreupdate_underscoreretrieve(""), e, authToken);
                } else {
                    recordUpdateResult.setMessage(messages.server_underscorerecord_underscoreupdate_underscorefail(""));
                    recordUpdateResult.setException(new PersistenceException(e));
                    LogUtility.log(Level.SEVERE, session, messages.server_underscorerecord_underscoreupdate_underscorefail(""), e, authToken);
                }
                ...
                try {
                    ...
                    LogUtility.log(Level.SEVERE, session, messages.log_underscoredb_underscoreclose(""), e, authToken);
                }
            }
        }
