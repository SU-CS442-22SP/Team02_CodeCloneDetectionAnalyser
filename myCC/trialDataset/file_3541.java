            ...
            try {
                ...
                while (j.hasNext() && !stop && !cancel) {
                    ...
                    if (exists(list, name)) {
                        ...
                        if (myImagesBehaviour.equals(TLanguage.getString("TIGImportDBDialog.REPLACE_underscoreIMAGES"))) {
                        ...
                        if (myImagesBehaviour.equals(TLanguage.getString("TIGImportDBDialog.ADD_underscoreIMAGES"))) {
                            ...
                            if (aux.size() != 0) {
                                while (aux.size() != 0) {
                                    fileCount++;
                                    aux = TIGDataBase.imageSearchByName(name.substring(0, name.lastIndexOf('.')) + "_underscore" + fileCount);
                                }
                                pathDst = pathDst + name.substring(0, name.lastIndexOf('.')) + '_underscore' + fileCount + name.substring(name.lastIndexOf('.'), name.length());
                                name = name.substring(0, name.lastIndexOf('.')) + '_underscore' + fileCount + name.substring(name.lastIndexOf('.'), name.length());
                            } else {
                                pathDst = pathDst.concat(name);
                            }
                        }
                        String pathThumbnail = (pathDst.substring(0, pathDst.lastIndexOf("."))).concat("_underscoreth.jpg");
