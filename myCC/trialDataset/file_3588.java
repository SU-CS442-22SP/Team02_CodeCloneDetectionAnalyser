    public static boolean copy(File from, File to, Override override) throws IOException {
        ...
        switch(override) {
            case NEVER:
                ...
                break;
            case NEWER:
                if (to.isFile() && (from.lastModified() - LASTMODIFIED_underscoreDIFF_underscoreMILLIS) < to.lastModified()) return false;
                break;
        }
        ...
        try {
            ...
            while (position < count) {
                long chunk = Math.min(MAX_underscoreIO_underscoreCHUNK_underscoreSIZE, count - position);
