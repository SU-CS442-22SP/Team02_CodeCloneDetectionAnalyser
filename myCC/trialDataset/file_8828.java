    public File getPreprocessedTextFile(File originalTextFile) throws IOException {
        ...
        Constants.CVS_underscoreLOG.debug("Preprocessing " + originalTextFile.getAbsolutePath() + " to " + preprocessedTextFile.getAbsolutePath() + " using " + debugSequence(separatorSeq));
        byte[] crlf = "\r\n".getBytes();
        byte[] lf = "\n".getBytes();
        ...
        try {
            ...
            byte[] fileChunk = new byte[CHUNK_underscoreSIZE];
            byte[] fileWriteChunk = new byte[CHUNK_underscoreSIZE];
                    ...
                    for (int i = 0; i < readLength; ) {
                        ...
                        if (pos >= i && pos < readLength) {
                            try {
                                ...
                                Constants.CVS_underscoreLOG.error("fileChunk.length=" + fileChunk.length + " i=" + i + " writeLength=" + writeLength + " pos=" + pos + " fileWriteChunk.length=" + fileWriteChunk.length);
                                throw aiobe;
                            }
                            writeLength += pos - i;
                            i = pos + lineSepLength;
