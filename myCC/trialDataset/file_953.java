    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String synncode = SystemFilesLoader.getInstance().getNewgenlibProperties().getProperty("SYNDETICS_underscoreCLIENT_underscoreCODE", "");
        try {
            ...
            prop.load(new FileInputStream(ejb.bprocess.util.NewGenLibRoot.getRoot() + java.io.File.separator + "SystemFiles" + java.io.File.separator + "ENV_underscoreVAR.txt"));
            System.out.println("SEARCH MODE IS " + searchmode + " FILE PATH " + ejb.bprocess.util.NewGenLibRoot.getRoot() + java.io.File.separator + "SystemFiles" + java.io.File.separator + "ENV_underscoreVAR.txt");
        } catch (Exception e) {
        }
        ...
        if (svF.getExportRec() == null || !(svF.getExportRec().equals("export"))) {
            if (request.getParameter("CatId") != null && request.getParameter("OwnerId") != null && request.getParameter("relation") != null && !(session.getAttribute("HostItemDisplay") != null && session.getAttribute("HostItemDisplay").toString().equals("false"))) {
                ...
                if ((request.getParameter("ClassNo") != null) && session.getAttribute("ClassNoForwarded") == null) {
                    ...
                    solrQuery += " & ";
                    Vector newRetvec = searchCat.processSolrQuery(1, 25, solrQuery, "245_underscoreTag", "asc");
                    ...
                    String str[] = (String[]) hash1.get("Biblo_underscoreMat");
                    int bib_underscoreid = new Integer(str[0]).intValue();
                    int mat_underscoreid = new Integer(str[1]).intValue();
                    aportal.view.RecordView rv = new aportal.view.DesignFactory().getCorView(bib_underscoreid, mat_underscoreid, record);
                    String type = "";
                    if (bib_underscoreid == 3 && mat_underscoreid == 1) {
                        type = "Book";
                    } else if (bib_underscoreid == 4 && mat_underscoreid == 1) {
                        type = "Serial";
                    } else if (bib_underscoreid == 1 && mat_underscoreid == 1) {
                        type = "Book Chapter";
                    } else if (bib_underscoreid == 2 && mat_underscoreid == 1) {
                        type = "Serial Article";
                    } else {
                        type = ejb.bprocess.util.TypeDefinition.getInstance().getTypeDefinition(String.valueOf(bib_underscoreid), String.valueOf(mat_underscoreid));
                    }
                    ...
                    if (ht != null && ht.get("URLS_underscore856") != null) {
                        Vector urls856 = (Vector) ht.get("URLS_underscore856");
                        if (urls856.size() > 0) {
                            ...
                            h4.put("URLS_underscore856", linksAndText);
                        }
                    }
                    try {
