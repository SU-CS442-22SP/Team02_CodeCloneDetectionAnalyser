            ...
            if (blGo) {
                ...
                try {
                    ...
                    if (blExecutePOS) {
                        ...
                        String strSSLDebug = "";
                        if (info != null) {
                            debug = info.get(DBSettings.MAIN_underscoreDEBUG).compareTo("1") == 0;
                            if (debug) log("       * Debug Mode is ON"); else log("       * Debug Mode is OFF");
                            if (info.get(DBSettings.POS_underscoreSSLENABLED).compareTo("1") == 0) {
                                strSSL = "-Djavax.net.ssl.keyStore=" + info.get(DBSettings.POS_underscoreSSLKEYSTORE) + " -Djavax.net.ssl.keyStorePassword=pscafe -Djavax.net.ssl.trustStore=" + info.get(DBSettings.POS_underscoreSSLTRUSTSTORE) + " -Djavax.net.ssl.trustStorePassword=pscafe";
                                log("       * Using SSL");
                                debug("            " + strSSL);
                                if (info.get(DBSettings.POS_underscoreSSLDEBUG).compareTo("1") == 0) {
