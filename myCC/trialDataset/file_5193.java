    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String storedKey = CLIENT_underscoreSESSION_underscoreKEYS.get(req.getRemoteAddr());
        if (sessionKey != null && storedKey != null && sessionKey.equals(storedKey)) validSession = true;
        DataStore ds = DataStore.getConnection();
        if (IPV6_underscoreDETECTED) {
            boolean doneWarning;
            synchronized (SJQServlet.class) {
                doneWarning = IPV6_underscoreWARNED;
                if (!IPV6_underscoreWARNED) IPV6_underscoreWARNED = true;
            }
        ...
        if (clntProto == null || Integer.parseInt(clntProto) != SJQ_underscorePROTO) throw new RuntimeException("Server is speaking protocol '" + SJQ_underscorePROTO + "', but client is speaking protocol '" + clntProto + "'; install a client version that matches the server protocol version!");
        ...
        if (cmd.equals("pop")) {
            if (!validClient) {
                ...
                if (RangeInterpreter.inRange(now.get(Calendar.DAY_underscoreOF_underscoreWEEK), 1, 7, offDay) || RangeInterpreter.inRange(now.get(Calendar.HOUR_underscoreOF_underscoreDAY), 0, 23, offHour)) {
                    ...
                    if (t == null) bw.write("null"); else {
                        ...
                        String objType = null;
                        try {
                            if (jobj != null) objType = jobj.getString(Task.JSON_underscoreOBJ_underscoreTYPE);
                        ...
                        if (obj == null || jobj == null) {
                            ...
                            try {
                                long ratio = calcRatio(jobj.getString(Task.JSON_underscoreOBJ_underscoreID), jobj.getString(Task.JSON_underscoreNORECORDING));
            ...
            try {
                ...
                if (pwd.length() > 0 && pwd.equals(userPwd)) {
                    ...
                    CLIENT_underscoreSESSION_underscoreKEYS.put(req.getRemoteAddr(), Integer.toString(key));
