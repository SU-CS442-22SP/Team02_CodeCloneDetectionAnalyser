    private void weightAndPlaceClasses() {
        int rows = getRows();
        for (int curRow = _underscoremaxPackageRank; curRow < rows; curRow++) {
            xPos = getHGap() / 2;
            BOTLObjectSourceDiagramNode[] rowObject = getObjectsInRow(curRow);
            for (int i = 0; i < rowObject.length; i++) {
                if (curRow == _underscoremaxPackageRank) {
                    ...
                    if (nUplinks > 0) {
                        float average_underscorecol = 0;
                        for (int j = 0; j < uplinks.size(); j++) {
                            average_underscorecol += ((BOTLObjectSourceDiagramNode) (uplinks.elementAt(j))).getColumn();
                        }
                        average_underscorecol /= nUplinks;
                        rowObject[i].setWeight(average_underscorecol);
                    } else {
            ...
            for (int i = 0; i < pos.length; i++) {
                ...
                if ((i > _underscorevMax) && (rowObject[pos[i]].getUplinks().size() == 0) && (rowObject[pos[i]].getDownlinks().size() == 0)) {
                    if (getColumns(rows - 1) > _underscorevMax) {
                        rows++;
                    }
