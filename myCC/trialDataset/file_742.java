    public final void close() throws IOException {
        ...
        try {
            ...
            packStream.write(FLAT_underscorePACK_underscoreHEADER.getBytes(Charsets.ISO_underscore8859_underscore1));
            structBufferWriter.flush();
            structBufferWriter.close();
            int headerSize = structBuffer.size() + nbFiles.length();
            packStream.write(Integer.toString(headerSize).getBytes(Charsets.ISO_underscore8859_underscore1));
            packStream.write('\0');
            packStream.write(nbFiles.getBytes(Charsets.ISO_underscore8859_underscore1));
            structBuffer.writeTo(packStream);
            structBufferWriter = null;
            structBuffer = null;
            FileInputStream in = new FileInputStream(tmpFile);
            try {
                byte[] buffer = new byte[FILE_underscoreCOPY_underscoreBUFFER_underscoreLEN];
                int read;
