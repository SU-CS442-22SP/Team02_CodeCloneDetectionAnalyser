    private void SaveLoginInfo() {
        ...
        try {
            if (m_underscorebSavePwd) {
                ...
                iSize = m_underscorePwdList.size();
                for (int iIndex = 0; iIndex < iSize; iIndex++) {
                    md.reset();
                    md.update(((String) m_underscoreUsrList.get(iIndex)).getBytes());
                    byte[] DESUSR = md.digest();
                    byte alpha = 0;
                    for (int i = 0; i < DESUSR.length; i++) alpha += DESUSR[i];
                    String pwd = (String) m_underscorePwdList.get(iIndex);
                    if (pwd.length() > 0) {
