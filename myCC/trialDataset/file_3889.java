    public void notifyIterationEnds(final IterationEndsEvent event) {
        ...
        source = new File("loads_underscoreout.txt");
        if (source.exists()) {
            File destination = new File(Controler.getIterationFilename("loads_underscoreout.txt"));
            try {
                IOUtils.copyFile(source, destination);
            } catch (FileNotFoundException e) {
                log.info("WARNING: Could not copy loads_underscoreout.txt to its iteration directory.");
            } catch (IOException e) {
                log.info("WARNING: Could not copy loads_underscoreout.txt to its iteration directory.");
            }
            destination = new File("loads_underscorein.txt");
            if (!IOUtils.renameFile(source, destination)) {
                log.info("WARNING: Could not move loads_underscoreout.txt to loads_underscorein.txt.");
            }
        }
