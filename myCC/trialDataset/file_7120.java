    private boolean doStudentCreditUpdate(Double dblCAmnt, String stuID) throws Exception {
        ...
        try {
            ...
            if (hasStudentCredit()) {
                stmt = conn.createStatement();
                if (stmt.executeUpdate("UPDATE " + strPOSPrefix + "studentcredit set credit_underscoreamount = credit_underscoreamount + " + round2Places(dblCAmnt) + " WHERE credit_underscoreactive = '1' and credit_underscorestudentid = '" + stuId + "'") == 1) {
                    stmt.close();
                    stmt = conn.createStatement();
                    if (stmt.executeUpdate("UPDATE " + strPOSPrefix + "studentcredit set credit_underscorelastused = NOW() where credit_underscoreactive = '1' and credit_underscorestudentid = '" + stuId + "'") == 1) {
                        stmt.close();
                        stmt = conn.createStatement();
                        if (stmt.executeUpdate("INSERT into " + strPOSPrefix + "studentcredit_underscorelog ( scl_underscorestudentid, scl_underscoreaction, scl_underscoredatetime ) values( '" + stuId + "', '" + round2Places(dblCAmnt) + "', NOW() )") == 1) {
                            stmt.close();
                            blOk = true;
                        } else {
                ...
                stmt = conn.createStatement();
                if (stmt.executeUpdate("insert into " + strPOSPrefix + "studentcredit (credit_underscoreamount,credit_underscoreactive,credit_underscorestudentid,credit_underscorelastused) values('" + round2Places(dblCAmnt) + "','1','" + stuId + "', NOW())") == 1) {
                    stmt.close();
                    stmt = conn.createStatement();
                    if (stmt.executeUpdate("insert into " + strPOSPrefix + "studentcredit_underscorelog ( scl_underscorestudentid, scl_underscoreaction, scl_underscoredatetime ) values( '" + stuId + "', '" + round2Places(dblCAmnt) + "', NOW() )") == 1) {
                        stmt.close();
