        ...
        try {
            ...
            if (action.equals("test")) {
                try {
                    ...
                    out = res.getWriter();
                    List l = DBSpectrumPeer.executeQuery("select SPECTRUM_underscoreID from SPECTRUM limit 1");
                ...
                String query = "select distinct MOLECULE.MOLECULE_underscoreID from MOLECULE, SPECTRUM where SPECTRUM.MOLECULE_underscoreID = MOLECULE.MOLECULE_underscoreID and SPECTRUM.REVIEW_underscoreFLAG =\"true\" order by MOLECULE.DATE desc;";
                ...
                JasperReport jasperReport = (JasperReport) JRLoader.loadObject(relativepath + "/reports/" + (yearly ? "yearly" : "monthly") + "_underscorereport_underscore" + type + ".jasper");
                ...
                ResultSet rs = null;
                if (type == 1) {
                    rs = stmt.executeQuery("select YEAR(DATE) as YEAR, " + (yearly ? "" : " MONTH(DATE) as MONTH, ") + "AFFILIATION_underscore1, AFFILIATION_underscore2, MACHINE.NAME as NAME, count(*) as C, sum(WISHED_underscoreSPECTRUM like '%13C%' or WISHED_underscoreSPECTRUM like '%variable temperature%' or WISHED_underscoreSPECTRUM like '%ID sel. NOE%' or WISHED_underscoreSPECTRUM like '%solvent suppression%' or WISHED_underscoreSPECTRUM like '%standard spectrum%') as 1_underscoreD, sum(WISHED_underscoreSPECTRUM like '%H,H-COSY%' or WISHED_underscoreSPECTRUM like '%NOESY%' or WISHED_underscoreSPECTRUM like '%HMQC%' or WISHED_underscoreSPECTRUM like '%HMBC%') as 2_underscoreD, sum(OTHER_underscoreWISHED_underscoreSPECTRUM!='') as SPECIAL, sum(OTHER_underscoreNUCLEI!='') as HETERO, sum(PROCESS='self') as SELF, sum(PROCESS='robot') as ROBOT, sum(PROCESS='worker') as OPERATOR from (SAMPLE join TURBINE_underscoreUSER using (USER_underscoreID)) join MACHINE on MACHINE.MACHINE_underscoreID=SAMPLE.MACHINE where YEAR(DATE)>=" + yearstart + " and YEAR(DATE)<=" + yearend + " and LOGIN_underscoreNAME<>'testuser' group by YEAR, " + (yearly ? "" : "MONTH, ") + "AFFILIATION_underscore1, AFFILIATION_underscore2, MACHINE.NAME");
                } else if (type == 2) {
                    rs = stmt.executeQuery("select YEAR(DATE) as YEAR, " + (yearly ? "" : " MONTH(DATE) as MONTH, ") + "MACHINE.NAME as NAME, count(*) as C, sum(WISHED_underscoreSPECTRUM like '%13C%' or WISHED_underscoreSPECTRUM like '%variable temperature%' or WISHED_underscoreSPECTRUM like '%ID sel. NOE%' or WISHED_underscoreSPECTRUM like '%solvent suppression%' or WISHED_underscoreSPECTRUM like '%standard spectrum%') as 1_underscoreD, sum(WISHED_underscoreSPECTRUM like '%H,H-COSY%' or WISHED_underscoreSPECTRUM like '%NOESY%' or WISHED_underscoreSPECTRUM like '%HMQC%' or WISHED_underscoreSPECTRUM like '%HMBC%') as 2_underscoreD, sum(OTHER_underscoreWISHED_underscoreSPECTRUM!='') as SPECIAL, sum(OTHER_underscoreNUCLEI!='') as HETERO, sum(PROCESS='self') as SELF, sum(PROCESS='robot') as ROBOT, sum(PROCESS='worker') as OPERATOR from (SAMPLE join TURBINE_underscoreUSER using (USER_underscoreID)) join MACHINE on MACHINE.MACHINE_underscoreID=SAMPLE.MACHINE group by YEAR, " + (yearly ? "" : "MONTH, ") + "MACHINE.NAME");
                }
                ...
                crit.addJoin(DBCanonicalNamePeer.MOLECULE_underscoreID, DBSpectrumPeer.MOLECULE_underscoreID);
                crit.addJoin(DBSpectrumPeer.SPECTRUM_underscoreTYPE_underscoreID, DBSpectrumTypePeer.SPECTRUM_underscoreTYPE_underscoreID);
                ...
                Criteria crit = new Criteria();
                crit.addJoin(DBMoleculePeer.MOLECULE_underscoreID, DBSpectrumPeer.MOLECULE_underscoreID);
                crit.add(DBSpectrumPeer.REVIEW_underscoreFLAG, "true");
                ...
                while (true) {
                    Statement stmt = conn.createStatement();
                    ResultSet rs = stmt.executeQuery("select HOSE_underscoreCODE, VALUE, SYMBOL from HOSE_underscoreCODES where CONDITION_underscoreTYPE='m' and WITH_underscoreRINGS=0 and SYMBOL='" + req.getParameter("symbol") + "' limit " + (i * 1000) + ", 1000");
                    int m = 0;
                    while (rs.next()) {
            ...
            if (action.equals("exportlastinputs")) {
                ...
                if (user != null) {
                    List l = NmrshiftdbUserPeer.executeQuery("SELECT LAST_underscoreDOWNLOAD_underscoreDATE FROM TURBINE_underscoreUSER  where LOGIN_underscoreNAME=\"" + user.getUserName() + "\";");
                    ...
                    if (((new Date().getTime() - lastDownloadDate.getTime()) / 3600000) < 24) {
                        ...
                        NmrshiftdbUserPeer.executeStatement("UPDATE TURBINE_underscoreUSER SET LAST_underscoreDOWNLOAD_underscoreDATE=NOW() where LOGIN_underscoreNAME=\"" + user.getUserName() + "\";");
                        Vector<String> parameters = new Vector<String>();
                        String query = "select distinct MOLECULE.MOLECULE_underscoreID from MOLECULE, SPECTRUM where SPECTRUM.MOLECULE_underscoreID = MOLECULE.MOLECULE_underscoreID and SPECTRUM.REVIEW_underscoreFLAG =\"true\" and SPECTRUM.USER_underscoreID=" + user.getUserId() + " order by MOLECULE.DATE desc;";
            ...
            if (action.equals("exportspec")) {
                ...
                if (format.equals("pdf") || format.equals("rtf")) {
                    ...
                    Fop fop = fopFactory.newFop(format.equals("rtf") ? MimeConstants.MIME_underscoreRTF : MimeConstants.MIME_underscorePDF, foUserAgent, out2);
                ...
                if (format.indexOf("exsection") == 0) {
                    ...
                    Criteria crit = new Criteria();
                    crit.add(DBSpectrumPeer.USER_underscoreID, spectrum.getUserId());
