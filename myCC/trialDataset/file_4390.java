    private Long getNextPkValueForEntityIncreaseBy(String entityName, int count, int increasePkBy) {
        ...
        String where = "where eoentity_underscorename = '" + entityName + "'";
        if (false) {
                ...
                for (int tries = 0; tries < count; tries++) {
                    try {
                        ...
                        if (hasNext) {
                            ...
                            con.createStatement().executeUpdate("update pk_underscoretable set pk_underscorevalue = " + (pk + increasePkBy) + " " + where);
                        } else {
                            pk = maxIdFromTable(entityName);
                            con.createStatement().executeUpdate("insert into pk_underscoretable (eoentity_underscorename, pk_underscorevalue) values ('" + entityName + "', " + (pk + increasePkBy) + ")");
                        }
                        con.commit();
                        return new Long(pk);
