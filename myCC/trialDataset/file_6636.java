    public Vector downSync(Vector v) throws SQLException {
        ...
        try {
            ...
            PreparedStatement update = con.prepareStatement("update cont_underscoreContact set owner=?,firstname=?," + "lastname=?,nickname=?,title=?,organization=?,orgunit=?," + "emailaddr=?,homeph=?,workph=?,cellph=?,im=?,imno=?," + "fax=?,homeaddr=?,homelocality=?,homeregion=?," + "homepcode=?,homecountry=?,workaddr=?,worklocality=?," + "workregion=?,workpcode=?,workcountry=?,website=?," + "wapsite=?,comments=?,birthday=?,syncstatus=?,dirtybits=? " + "where OId=? and syncstatus=?");
            PreparedStatement insert = con.prepareStatement("insert into cont_underscoreContact (owner,firstname,lastname," + "nickname,title,organization,orgunit,emailaddr,homeph," + "workph,cellph,im,imno,fax,homeaddr,homelocality," + "homeregion,homepcode,homecountry,workaddr,worklocality," + "workregion,workpcode,workcountry,website,wapsite," + "comments,birthday,syncstatus,dirtybits,quicklist) " + "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?," + "?,?,?,?,?,?,?,?)");
            PreparedStatement insert1 = con.prepareStatement(DBUtil.getQueryCurrentOID(con, "cont_underscoreContact", "newoid"));
            PreparedStatement delete1 = con.prepareStatement("delete from  cont_underscoreContact_underscoreGroup_underscoreRel where externalcontact=?");
            PreparedStatement delete2 = con.prepareStatement("delete from  cont_underscoreContact where OId=? " + "and (syncstatus=? or syncstatus=?)");
