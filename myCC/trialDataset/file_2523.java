    private static void download(String urlString) throws IOException {
        ...
        long packageLength = fileLength / THREAD_underscoreCOUNT;
        long leftLength = fileLength % THREAD_underscoreCOUNT;
        String fileName = Utils.decodeURLFileName(url);
        RandomAccessFile file = new RandomAccessFile(fileName, "rw");
        System.out.println("File: " + fileName + ", Size: " + Utils.calSize(fileLength));
        CountDownLatch latch = new CountDownLatch(THREAD_underscoreCOUNT + 1);
        long pos = 0;
        for (int i = 0; i < THREAD_underscoreCOUNT; i++) {
