    public HttpResponse fetch(HttpServletRequest request) throws IOException {
        ...
        if (Helper.useProxy()) {
            ...
            httpclient.getParams().setParameter(ConnRoutePNames.DEFAULT_underscorePROXY, proxy);
        }
        httpclient.getCookieSpecs().register("easy", csf);
        httpclient.getParams().setParameter(ClientPNames.COOKIE_underscorePOLICY, "easy");
        String currentRemoteGAEHost = Helper.getRemoteServer();
        try {
            HttpUriRequest httpRequest = createRequest(request);
            addHeader(request, httpRequest);
            HttpResponse response = httpclient.execute(httpRequest);
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_underscoreFORBIDDEN) {
