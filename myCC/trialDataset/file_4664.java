    private static URL searchForBundle(String name, String parent) throws MalformedURLException {
        ...
        try {
            ...
            url = new URL(REFERENCE_underscorePROTOCOL, null, fileLocation.toURL().toExternalForm());
            reference = true;
        }
        if (!reference) {
            URL baseURL = url;
            if (url.getProtocol().equals(REFERENCE_underscorePROTOCOL)) {
                reference = true;
                String baseSpec = url.getFile();
                if (baseSpec.startsWith(FILE_underscoreSCHEME)) {
        ...
        if (reference) {
            ...
            if (result != null) url = new URL(REFERENCE_underscorePROTOCOL, null, FILE_underscoreSCHEME + result); else return null;
        }
        try {
