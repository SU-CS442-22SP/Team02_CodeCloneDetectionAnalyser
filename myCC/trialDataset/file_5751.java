    public Document getKmlStream(String streetname, String number, String neighbourhood, String city, String state) throws RotaException {
        StringBuffer urlsb = new StringBuffer(resourceBundle.getString(Constants.URL_underscoreSEARCH));
        DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
        InputStream in = null;
        HttpURLConnection httpConnection = null;
        Document doc = null;
        dbf.setValidating(false);
        String proxy = resourceBundle.getString(Constants.PROXY_underscoreHOST);
        String port = resourceBundle.getString(Constants.PROXY_underscorePORT);
        try {
            String address = String.format("%s+%s+%s+%s+%s", URLEncoder.encode(streetname.trim(), Constants.URL_underscoreENCODING), URLEncoder.encode(number.trim(), Constants.URL_underscoreENCODING), URLEncoder.encode(neighbourhood.trim(), Constants.URL_underscoreENCODING), URLEncoder.encode(city.trim(), Constants.URL_underscoreENCODING), URLEncoder.encode(state.trim(), Constants.URL_underscoreENCODING));
            DocumentBuilder df = dbf.newDocumentBuilder();
            urlsb.append(address);
            urlsb.append(resourceBundle.getString(Constants.GOOGLE_underscoreTYPE_underscoreOUTPUT));
            urlsb.append(resourceBundle.getString(Constants.SENSOR));
            urlsb.append(resourceBundle.getString(Constants.GOOGLE_underscoreKEY));
            urlsb.append(resourceBundle.getString(Constants.GOOGLE_underscoreKEY_underscoreVALUE));
