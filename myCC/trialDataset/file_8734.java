    private void handleNodeUp(long eventID, long nodeID, String eventTime) {
        ...
        if (eventID == -1 || nodeID == -1) {
            log.warn(EventConstants.NODE_underscoreUP_underscoreEVENT_underscoreUEI + " ignored - info incomplete - eventid/nodeid: " + eventID + "/" + nodeID);
            return;
        }
        ...
        try {
            ...
            if (openOutageExists(dbConn, nodeID)) {
                ...
                PreparedStatement outageUpdater = dbConn.prepareStatement(OutageConstants.DB_underscoreUPDATE_underscoreOUTAGES_underscoreFOR_underscoreNODE);
                ...
                log.warn("\'" + EventConstants.NODE_underscoreUP_underscoreEVENT_underscoreUEI + "\' for " + nodeID + " no open record.");
            }
            try {
                dbConn.commit();
