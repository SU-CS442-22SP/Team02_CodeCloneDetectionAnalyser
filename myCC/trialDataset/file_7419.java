    public void removeStadium(String name, String city) throws StadiumException {
        ...
        int id = findStadiumBy_underscoreN_underscoreC(name, city);
        if (id == -1) throw new StadiumException("No such stadium");
        try {
            conn.setAutoCommit(false);
            PreparedStatement stm = conn.prepareStatement(Statements.SELECT_underscoreSTAD_underscoreTRIBUNE);
            stm.setInt(1, id);
            ...
            stm = conn.prepareStatement(Statements.DELETE_underscoreSTADIUM);
            stm.setInt(1, id);
            stm.executeUpdate();
