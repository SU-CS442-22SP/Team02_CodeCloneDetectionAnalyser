    public void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        if (request.getParameter("acao").equals("confirmar")) {
            ...
            RequestDispatcher dispatcher = getServletContext().getRequestDispatcher(Pagina.PAGINA_underscoreAPRESENTACAO);
            ...
            servletResponse.sendRedirect(Pagina.PAGINA_underscoreAPRESENTACAO);
        } else if (request.getParameter("acao").equals("preparar")) {
            Empresa objeto = new Empresa();
            request.setAttribute("objeto", objeto);
            RequestDispatcher dispatcher = getServletContext().getRequestDispatcher(Pagina.PAGINA_underscoreEMPRESA_underscoreCADASTRAR);
            dispatcher.forward(request, response);
            ...
            if (!emailEncontrado && !cnpjEncontrado) {
                ...
                corpo.append("<a href='" + Constantes.URL + Constantes.URI_underscoreCONFIRMAR + "&&uuid=" + empresa.getUuid() + "'");
                corpo.append("target='_underscoreblank'>" + Constantes.URL + Constantes.URI_underscoreCONFIRMAR + "&&uuid=" + empresa.getUuid() + "</a></p>");
                ...
                corpo.append(" <a href='mailto:suporte@appestoque.com.br?subject=[Cadastro]Mensagem por engano&&body='target='_underscoreblank'>n�o � minha conta</a>.</p>");
                corpo.append("<p style='font-family: 'Helvetica Neue', Arial, Helvetica, sans-serif; margin-top: 5px; font-size: 10px; color: #888888'>");
                ...
                corpo.append(" <a href='mailto:suporte@appestoque.com.br' target='_underscoreblank'>Suporte do Appestoque</a>.</p>");
                corpo.append("</div>");
                corpo.append("</body>");
                corpo.append("</html>");
                Util.enviarEmail(email, Constantes.ASSUNTO_underscoreCADASTRO, corpo);
                response.sendRedirect(Pagina.PAGINA_underscoreAPRESENTACAO);
            } else {
            ...
            RequestDispatcher dispatcher = getServletContext().getRequestDispatcher(Pagina.PAGINA_underscoreMENU);
            dispatcher.forward(request, response);
            ...
            try {
                ...
                for (int temp = 0; temp < 3; temp++) {
                    ...
                    if (nNode.getNodeType() == Node.ELEMENT_underscoreNODE) {
            ...
            try {
                ...
                QRCode qr = null;
                qr = new QRCode();
                qr.setTypeNumber(Mode.MODE_underscore8BIT_underscoreBYTE);
            ...
            try {
                ...
                for (int s = 0; s < nodeLst.getLength(); s++) {
                    ...
                    if (fstNode.getNodeType() == Node.ELEMENT_underscoreNODE) {
