    public void addFile(File fileobj, boolean delete) {
        ...
        if (currFileName != null) {
            ...
            int deleteFiles = JOptionPane.CANCEL_underscoreOPTION;
            ...
            } else if (!f.exists() || JOptionPane.OK_underscoreOPTION == (deleteFiles = JOptionPane.showConfirmDialog(this, "File" + f.getName() + " Already Exists! Do You Wish To Overwrite That File?" + (f.getName().toLowerCase().endsWith(".exp") ? "\nOverwriting An Expression File Will Delete All Files Which Previously Required The Orginal File" : "")))) {
                try {
                    if (deleteFiles == JOptionPane.OK_underscoreOPTION && f.getName().toLowerCase().endsWith(".exp")) {
            ...
            } else if (fileobj.getName().toLowerCase().endsWith(".ds_underscorestore")) {
                message = "";
