    public void testResolveURL() throws Exception {
        ...
        try {
            ...
            for (int i = 2; i < urls.length; i++) {
                if (urls[i] instanceof String) {
                    ...
                    uriResolver.register(new URL(url), new File("./src/test/java/" + url.replace(":", "_underscore").replace("/", "_underscore") + ".schema.json"));
        ...
        for (int i = 2; i < urls.length; i++) {
            if (urls[i] instanceof String) {
                String url = (String) urls[i];
                uriResolver2.register(new URL(url), new File("./src/test/java/" + url.replace(":", "_underscore").replace("/", "_underscore") + ".schema.json"));
            } else if (urls[i] instanceof File) {
