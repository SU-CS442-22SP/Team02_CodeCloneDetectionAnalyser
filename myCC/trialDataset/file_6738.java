    public FlatFileFrame() {
        ...
        String formatted_underscoredate = formatter.format(new Date());
        this.createdOnText = new JTextField(formatted_underscoredate);
            ...
            public void actionPerformed(ActionEvent ev) {
                ...
                try {
                    ...
                    if (!domain.equals(usersdomain)) {
                        int s = JOptionPane.showConfirmDialog(null, "If you are not the original author, you may wish to switch the current domain name " + domain + " to \nyour domain name " + usersdomain + ".  Would you like to do this?\n (If you'll be using this domain often, you may want to set it in your preferences.)", "Potential WWW name-space clash!", JOptionPane.YES_underscoreNO_underscoreCANCEL_underscoreOPTION);
                        if (s == JOptionPane.YES_underscoreOPTION) {
                            setNickname(usersdomain + "." + name);
                            executeTransfer();
                        }
                        if (s == JOptionPane.NO_underscoreOPTION) {
                            executeTransfer();
                        }
                    } else {
                        executeTransfer();
        ...
        this.formatTable.setAutoResizeMode(JTable.AUTO_underscoreRESIZE_underscoreOFF);
                ...
                try {
                    ...
                    while ((strLine = br.readLine()) != null) {
                        if (line <= (ignorePreHdrLines + numhdr + ignorePostHdrLines)) {
                            ...
                            String[] putative_underscorecols = strLine.split(FD);
                            System.out.println("The number of potential columns is " + putative_underscorecols.length);
                            String FMT = "";
                            while (formatTable.getColumnCount() > putative_underscorecols.length) {
                                TableColumn tcol = formatTable.getColumnModel().getColumn(0);
                                formatTable.removeColumn(tcol);
                            }
                            for (int i = 0; i < putative_underscorecols.length; i++) {
                                String fmt = "";
                                try {
                                    Double dummy = new Double(putative_underscorecols[i]);
                                    fmt = "%f";
                                } catch (Exception err) {
                                    fmt = "%s";
                                }
        ...
        this.addSources.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ev) {
                ...
                File[] files = null;
                if (option == JFileChooser.APPROVE_underscoreOPTION) {
