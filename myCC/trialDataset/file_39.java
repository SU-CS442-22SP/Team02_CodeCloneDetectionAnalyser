        ...
        if (ServletFileUpload.isMultipartContent(rr.getRequest())) {
            ...
            while (iter.hasNext()) {
                ...
                if (!item.isFormField()) {
                    ...
                    DataBuffer buffer = new DataBuffer(0, EGEConstants.BUFFER_underscoreTEMP_underscorePATH);
                    ...
                    zipFile = new File(EGEConstants.BUFFER_underscoreTEMP_underscorePATH + File.separator + newTemp + EZP_underscoreEXT);
                    fos = new FileOutputStream(zipFile);
                    ior.compressData(buffDir, fos);
                    ins = new FileInputStream(zipFile);
                    File szipFile = new File(EGEConstants.BUFFER_underscoreTEMP_underscorePATH + File.separator + newTemp + ZIP_underscoreEXT);
                    ...
                    try {
                        ...
                        response.setContentType(APPLICATION_underscoreOCTET_underscoreSTREAM);
                        String fN = item.getName().substring(0, item.getName().lastIndexOf("."));
                        if (isComplex) {
                            String fileExt;
                            if (cpath.getOutputDataType().getMimeType().equals(APPLICATION_underscoreMSWORD)) {
                                fileExt = DOCX_underscoreEXT;
                            } else {
                                fileExt = ZIP_underscoreEXT;
                            }
            ...
            response.sendError(HttpServletResponse.SC_underscoreBAD_underscoreREQUEST);
            return;
        }
    }
