    public static final boolean updateFDT(final String currentVersion, final String updateURL, boolean shouldUpdate, boolean noLock) throws Exception {
        ...
        if (p.getProperty("totalRead_underscorerst") != null) {
            p.remove("totalRead_underscorerst");
        }
        if (p.getProperty("totalWrite_underscorerst") != null) {
            p.remove("totalWrite_underscorerst");
        }
        if (p != null && p.size() > 0) {
        ...
        try {
            tmpUpdateFile = File.createTempFile("fdt_underscoreupdate_underscoretmp", ".jar");
            ...
            urlConnection.setConnectTimeout(URL_underscoreCONNECTION_underscoreTIMEOUT);
            urlConnection.setReadTimeout(URL_underscoreCONNECTION_underscoreTIMEOUT);
            ...
            if (shouldUpdate) {
                try {
                    ...
                    final File bkpJar = new File(parentDir.getPath() + File.separator + "fdt_underscore" + Config.FDT_underscoreFULL_underscoreVERSION + ".jar");
