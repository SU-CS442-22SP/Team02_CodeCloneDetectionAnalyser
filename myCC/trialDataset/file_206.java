    @TestTargets({ @TestTargetNew(level = TestLevel.PARTIAL_underscoreCOMPLETE, notes = "Verifies that the ObjectInputStream constructor calls checkPermission on security manager.", method = "ObjectInputStream", args = { InputStream.class }) })
    public void test_underscoreObjectInputStream2() throws IOException {
        ...
        class TestObjectInputStream_underscorereadFields extends ObjectInputStream {

            TestObjectInputStream_underscorereadFields(InputStream s) throws StreamCorruptedException, IOException {
                super(s);
            }
        ...
        class TestObjectInputStream_underscorereadUnshared extends ObjectInputStream {

            TestObjectInputStream_underscorereadUnshared(InputStream s) throws StreamCorruptedException, IOException {
                super(s);
            }
        ...
        String filename = "SecurityPermissionsTest_underscore" + id;
        ...
        assertTrue("ObjectInputStream(InputStream) ctor must not call checkPermission on security manager on a class which neither overwrites methods readFields nor readUnshared", !s.called);
        s.reset();
        new TestObjectInputStream_underscorereadFields(new FileInputStream(f));
        ...
        s.reset();
        new TestObjectInputStream_underscorereadUnshared(new FileInputStream(f));
