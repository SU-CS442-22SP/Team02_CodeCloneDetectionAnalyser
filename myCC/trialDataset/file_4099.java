    ...
    public DataUpdateResult<Record> archiveRecord(String authToken, Record record, Filter filter, Field sourceField, InputModel inputmodel) throws DataOperationException {
        ...
        if (record != null) {
            ...
            try {
                ...
                recordUpdateResult.setMessage(messages.server_underscorerecord_underscoredelete_underscoresuccess(""));
                ...
                if (recordArchived > 0) AuditTrailManager.updateAuditTrail(connection, AuditTrailManager.createAuditTrailEvent(record, userId, AuditableEvent.EVENTYPE_underscoreDELETE), authToken, getSession());
                ...
                if (recordAttributes != null && recordAttributes.size() > 0) {
                    ...
                    while (rItr.hasNext()) {
                        ...
                        if (recordAttribArchived > 0) AuditTrailManager.updateAuditTrail(connection, AuditTrailManager.createAuditTrailEvent(r, userId, AuditableEvent.EVENTYPE_underscoreDELETE), authToken, getSession());
                    }
                }
                ...
                if (!archived && connection != null) {
                    try {
                        ...
                        LogUtility.log(Level.SEVERE, getSession(), messages.log_underscoredb_underscorerollback(""), e1, authToken);
                        e1.printStackTrace();
                    }
                }
                recordUpdateResult.setFailed(true);
                if (archived) {
                    recordUpdateResult.setMessage(messages.server_underscorerecord_underscoredelete_underscoreretrieve(""));
                    recordUpdateResult.setException(e);
                    LogUtility.log(Level.SEVERE, getSession(), messages.server_underscorerecord_underscoredelete_underscoreretrieve(""), e, authToken);
                } else {
                    recordUpdateResult.setMessage(messages.server_underscorerecord_underscoredelete_underscorefail(""));
                    recordUpdateResult.setException(new PersistenceException(e));
                    LogUtility.log(Level.SEVERE, getSession(), messages.server_underscorerecord_underscoredelete_underscorefail(""), e, authToken);
                }
                ...
                try {
                    ...
                    LogUtility.log(Level.SEVERE, getSession(), messages.log_underscoredb_underscoreclose(""), e, authToken);
                }
            }
        }
