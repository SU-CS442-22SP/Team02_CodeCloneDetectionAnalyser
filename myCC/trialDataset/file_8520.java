    public ActionForward perform(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        MessageResources errorMessages = MessageResources.getMessageResources("com.apelon.apps.dts.treebrowser.resources.error_underscoremessages");
        ...
        if (logonUtilities.checkSession(session, status, reportingBean, errorMessages)) {
            ...
            if (namespace == null) {
                reportingBean.setReportingHtml(errorMessages.getMessage("classify_underscore3"));
                session.setAttribute("reportingBean", reportingBean);
                status = "classify_underscorefail";
                return (mapping.findForward(status));
            }
            String[] roleMods = request.getParameterValues("some_underscoreor_underscoreall");
            String[] roleGrps = request.getParameterValues("role_underscoregroup");
            ...
            try {
                ...
                reportingBean.setReportingHtml(errorMessages.getMessage("classify_underscore1") + e.getMessage());
                session.setAttribute("reportingBean", reportingBean);
                status = "classify_underscorefail";
            }
        }
