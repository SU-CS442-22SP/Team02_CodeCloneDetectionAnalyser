    public void run() {
        ...
        try {
            ...
            for (int j = 0; j < connections.getLength(); j++) {
                ...
                for (int i = 0; i < attributes.getLength(); i++) {
                    ...
                    if (frequency.equalsIgnoreCase("onchange")) {
                        ...
                        Task task2 = new Task(-1, Task.GET_underscoreATTRIBUTE, server);
                        ...
                        Task task = new Task(frequency2, Task.GET_underscoreATTRIBUTE, server);
                        MBeanAttribute att = new MBeanAttribute(mbean, attributename);
                        task.setAttribute(att);
                        task.setMbean(mbean);
                        if (tasks.get(taskname) != null) {
            ...
            if (MonitorServer.contains(tabName)) {
                JOptionPane.showMessageDialog(null, "This panel is already open, stoping creating of panel.", "Panel already exists", JOptionPane.ERROR_underscoreMESSAGE);
                return;
            }
