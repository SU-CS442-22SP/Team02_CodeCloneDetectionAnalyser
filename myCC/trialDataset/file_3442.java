    public void actionPerformed(ActionEvent e) {
        if (path.compareTo("") != 0) {
            ...
            while (newFile.exists()) {
                imagePath = "." + imageFile.separator + "Images" + imageFile.separator + imageName.substring(0, imageName.lastIndexOf('.')) + "_underscore" + i + imageName.substring(imageName.lastIndexOf('.'), imageName.length());
                newFile = new File(imagePath);
                i++;
            }
            imagePathThumb = (imagePath.substring(0, imagePath.lastIndexOf("."))).concat("_underscoreth.jpg");
            ...
            if (imageFile != null) {
                ...
                if (image.getImageLoadStatus() == MediaTracker.ERRORED) {
                    int choosenOption = JOptionPane.NO_underscoreOPTION;
                    choosenOption = JOptionPane.showConfirmDialog(null, TLanguage.getString("TIGInsertImageAction.MESSAGE"), TLanguage.getString("TIGInsertImageAction.NAME"), JOptionPane.CLOSED_underscoreOPTION, JOptionPane.ERROR_underscoreMESSAGE);
                } else {
                    try {
                        int thumbWidth = PREVIEW_underscoreWIDTH;
                        int thumbHeight = PREVIEW_underscoreHEIGHT;
                        ...
                        BufferedImage thumbImage = new BufferedImage(thumbWidth, thumbHeight, BufferedImage.TYPE_underscoreINT_underscoreRGB);
                        Graphics2D graphics2D = thumbImage.createGraphics();
                        graphics2D.setRenderingHint(RenderingHints.KEY_underscoreINTERPOLATION, RenderingHints.VALUE_underscoreINTERPOLATION_underscoreBILINEAR);
