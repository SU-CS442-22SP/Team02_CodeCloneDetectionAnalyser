    private void anneal(final float maxGamma, final float gammaAccel, final float objectiveTolerance, final float objectiveAccel, final float scoreTolerance, final float paramTolerance, final float distanceLimit, final float randomLimit, final long randomSeed, final BufferedDocuments<Phrase> references, final int n, final int maxNbest, File stateFile, boolean keepState) {
        ...
        if (stateFile != null && stateFile.length() > 0) {
            ...
            final int evaluations = ProjectedEvaluation.CFG_underscoreOPT_underscoreHISTORY_underscoreSIZE.getValue();
            ...
            for (int i = 0; i < n; i++) {
                ...
                for (int j = 0; j < evaluations; j++) {
                    ...
                    if (i == 0) {
                        ...
                        gamma = LogFeatureModel.FEAT_underscoreMODEL_underscoreGAMMA.getValue();
                        break;
                    }
                }
            }
            ...
            for (int i = 0; i < n; i++) {
                ...
                convergedScores[i] = Float.MAX_underscoreVALUE;
            }
        }
        for (int searchCount = 1; ; searchCount++) {
            ...
            if (isFinished) {
                ...
                if (annealObjective) {
                    ...
                    for (int i = 0; objectiveConverged && i < n; i++) objectiveConverged = isConverged(bestInitPoints[i].getScore(), convergedScores[i], objectiveTolerance, SCORE_underscoreEPSILON);
                    annealObjective = false;
                    for (Metric<ProjectedSentenceEvaluation> metric : AbstractEvaluation.CFG_underscoreEVAL_underscoreMETRICS.getValue()) if (metric.doAnnealing()) {
                ...
                if (!annealObjective) {
                    ...
                    gamma *= gammaAccel;
                    if (Math.abs(gamma) + GAMMA_underscoreEPSILON >= maxGamma) gamma = gamma >= 0 ? maxGamma : -maxGamma;
                }
                for (int i = 0; i < n; i++) {
