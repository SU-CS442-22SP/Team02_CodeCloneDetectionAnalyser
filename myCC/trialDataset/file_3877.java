    private void createContents(final Shell shell) {
        ...
        label1.setImage(Resources.IMAGE_underscorePKB);
        data = new FormData();
        data.top = new FormAttachment(0, 20);
        data.left = new FormAttachment(0, 20);
        label1.setLayoutData(data);
        Label label2 = new Label(shell, SWT.NONE);
        label2.setText(PreferenceDialog.PKBProperty.DEFAULT_underscorerebrand_underscoreapplication_underscoretitle + " " + version);
        ...
        CustomSeparator separator1 = new CustomSeparator(shell, SWT.SHADOW_underscoreIN | SWT.HORIZONTAL);
        data = new FormData();
        ...
        Hyperlink link = new Hyperlink(shell, SWT.NONE | SWT.NO_underscoreFOCUS);
        link.setText(PKBMain.CONTACT_underscoreEMAIL);
        link.addListener(SWT.Selection, new Listener() {

            public void handleEvent(Event event) {
                Program.launch("mailto:" + PKBMain.CONTACT_underscoreEMAIL + "?subject=[" + PKBMain.PRODUCT_underscoreALEX_underscorePKB + "]");
            }
        });
        data = new FormData();
        data.top = new FormAttachment(separator1, 10);
        ...
        Hyperlink link1 = new Hyperlink(shell, SWT.NONE | SWT.NO_underscoreFOCUS);
        link1.setText(PKBMain.PRODUCT_underscoreWEBSITE);
        link1.addListener(SWT.Selection, new Listener() {

            public void handleEvent(Event event) {
                Program.launch(PKBMain.PRODUCT_underscoreWEBSITE);
            }
        });
        data = new FormData();
        data.top = new FormAttachment(label4, 10);
        ...
        checkVersionBtn.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent event) {
                try {
                    URL url = new URL(PKBMain.PRODUCT_underscoreWEBSITE + "/latest-version.txt");
                    ...
                    if (remoteVersion.trim().compareTo(version.trim()) != 0) {
                        ...
                        buf.append("<HTML><BODY>").append("<h3 style='color: #0033FF'>You do not have the latest version. <br/> ").append("The latest version is PKB ").append(prop.getProperty("version") + " b" + prop.getProperty("build")).append(".</h3><A HREF='").append(prop.getProperty("url")).append("' TARGET='_underscoreBLANK'>Please download here </a> <br/><br/>").append("<B>It is strongly suggested to backup your knowledge base before install or unzip the new package!</B>").append("</BODY></HTML>");
