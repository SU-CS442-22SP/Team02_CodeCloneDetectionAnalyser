    public void process(String dir) {
        ...
        if (open_underscoreas_underscorestack || only_underscoreimages) {
            boolean sorted = false;
            while (!sorted) {
                sorted = true;
                for (int i = 0; i < n - 1; i++) {
            ...
            double min = Double.MAX_underscoreVALUE;
            double max = -Double.MAX_underscoreVALUE;
            int k = 0;
            try {
                for (int i = 0; i < n; i++) {
                    String path = (dir + list[i]);
