    public static String sendScripts(Session session) {
        ...
        try {
            ...
            {
                InputStream rsyncHelperContentInput = Thread.currentThread().getContextClassLoader().getResourceAsStream("scripts/" + RSYNC_underscoreHELPER_underscoreSCRIPT);
                channel = session.openChannel("exec");
                final String command = "cat > " + tempDirectory + "/" + RSYNC_underscoreHELPER_underscoreSCRIPT;
                ((ChannelExec) channel).setCommand(command);
            ...
            {
                channel = session.openChannel("exec");
                final String command = "chmod 700 " + tempDirectory + "/" + RSYNC_underscoreHELPER_underscoreSCRIPT;
                ((ChannelExec) channel).setCommand(command);
                InputStream in = channel.getInputStream();
                channel.connect();
                String[] result = inputStreamToString(in, channel);
            ...
            {
                InputStream askPassContentInput = Thread.currentThread().getContextClassLoader().getResourceAsStream("scripts/" + RSYNC_underscoreASKPASS_underscoreSCRIPT);
                channel = session.openChannel("exec");
                final String command = "cat > " + tempDirectory + "/" + RSYNC_underscoreASKPASS_underscoreSCRIPT;
                ((ChannelExec) channel).setCommand(command);
                OutputStream out = channel.getOutputStream();
                channel.connect();
            ...
            {
                channel = session.openChannel("exec");
                final String command = "chmod 700 " + tempDirectory + "/" + RSYNC_underscoreASKPASS_underscoreSCRIPT;
                ((ChannelExec) channel).setCommand(command);
                InputStream in = channel.getInputStream();
                channel.connect();
                String[] result = inputStreamToString(in, channel);
