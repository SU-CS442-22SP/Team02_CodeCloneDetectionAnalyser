    private void calculateCoverageAndSpecificity(String mainCat) throws IOException, JSONException {
        for (String cat : Rules.categoryTree.get(mainCat)) {
            for (String queryString : Rules.queries.get(cat)) {
                ...
                URL url = new URL("http://boss.yahooapis.com/ysearch/web/v1/" + urlEncodedQueryString + "?appid=" + yahoo_underscoreap_underscoreid + "&count=4&format=json&sites=" + site);
                ...
                ja = jsonObject.getJSONArray("resultset_underscoreweb");
