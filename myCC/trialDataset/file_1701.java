    protected byte[] mdDecode(String s, int type, byte[] salt) {
        try {
            ...
            if (salt != null) {
                md.update(salt);
                byte[] buff = md.digest();
                byte[] new_underscorebuf = new byte[buff.length + salt.length];
                for (int x = 0; x < buff.length; x++) new_underscorebuf[x] = buff[x];
                for (int x = buff.length; x < new_underscorebuf.length; x++) new_underscorebuf[x] = salt[x - buff.length];
                hexString.append(CBBase64.binaryToString(new_underscorebuf));
            } else {
                byte[] buff = md.digest();
