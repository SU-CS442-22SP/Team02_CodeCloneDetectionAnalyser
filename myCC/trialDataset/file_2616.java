    private void prepareQueryResultData(ZipEntryRef zer, String nodeDir, String reportDir, Set<ZipEntryRef> statusZers) throws Exception {
        String jobDir = nodeDir + File.separator + "job_underscore" + zer.getUri();
        ...
        fcopyName = reportDir + File.separator + zer.getName() + "_underscorestatus.xml";
        bis = new BufferedInputStream(new FileInputStream(f));
        bos = new BufferedOutputStream(new FileOutputStream(fcopyName));
        IOUtils.copy(bis, bos);
        bis.close();
        bos.close();
        statusZers.add(new ZipEntryRef(ZipEntryRef.SINGLE_underscoreFILE, zer.getName(), fcopyName, ZipEntryRef.WITH_underscoreREL));
    }
