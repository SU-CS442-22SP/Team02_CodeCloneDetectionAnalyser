    public void doWrite() {
        ...
        try {
            ...
            if (null != method.getResource()) {
                ifile = method.getJavaProject().getResource().getProject().getFile(Statics.CFG_underscoreDIR + Statics.SEPARATOR + path.substring(path.lastIndexOf(Statics.SEPARATOR)));
            }
            file = new File(path);
            if (file.exists()) {
