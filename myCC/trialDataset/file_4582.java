        ...
        try {
            ...
            if (documents != null && !documents.isEmpty()) {
                for (Iterator<Document> iter = documents.iterator(); iter.hasNext(); ) {
                    ...
                    if (AppConstants.STATUS_underscoreACTIVE.equals(document.getStatus())) {
                        try {
                            ...
                            if (isDeleted) {
                                document.setStatus(AppConstants.STATUS_underscorePROCESSING);
                                int uploadCount = 0;
                        ...
                        } catch (Exception add_underscoreex) {
                            add_underscoreex.printStackTrace();
                        }
                    } else if (AppConstants.STATUS_underscorePROCESSING.equals(document.getStatus())) {
                    ...
                    } else if (AppConstants.STATUS_underscoreINACTIVE.equals(document.getStatus())) {
                        document.setFixFlag(AppConstants.FLAG_underscoreNO);
                        newDocuments.add(document);
                    }
                }
            }
