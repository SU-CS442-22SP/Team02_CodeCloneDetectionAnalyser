    public boolean setSchedule(Schedule s) {
        ...
        try {
            conn = getConnection();
            pst1 = conn.prepareStatement("INSERT INTO timetable (recipe_underscoreid, time, meal) VALUES (?, ?, ?);");
            pst2 = conn.prepareStatement("SELECT * FROM timetable WHERE time BETWEEN ? AND ?");
            pst3 = conn.prepareStatement("DELETE FROM timetable WHERE time = ? AND meal = ? AND recipe_underscoreid = ?");
            ...
            while (rs2.next()) {
                ...
                pst3.setInt(3, rs2.getInt("recipe_underscoreid"));
                pst3.executeUpdate();
            }
