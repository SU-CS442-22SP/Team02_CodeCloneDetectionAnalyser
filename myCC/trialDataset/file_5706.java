    public static void reconstruct(final List files, final Map properties, final OutputStream fout, final String base_underscoreurl, final String producer, final PageSize[] size, final List hf) throws CConvertException {
        ...
        try {
            ...
            oldOut = out;
            if ("true".equals(properties.get(IHtmlToPdfTransformer.USE_underscorePDF_underscoreSIGNING))) {
                signed = true;
            ...
            for (int i = 0; i < files.size(); i++) {
                ...
                if (first) {
                    ...
                    writer.setPdfVersion(PdfWriter.VERSION_underscore1_underscore3);
                    writer.setFullCompression();
                    if ("true".equals(properties.get(IHtmlToPdfTransformer.USE_underscorePDF_underscoreENCRYPTION))) {
                        final String password = (String) properties.get(IHtmlToPdfTransformer.PDF_underscoreENCRYPTION_underscorePASSWORD);
                    ...
                    final String title = (String) properties.get(IHtmlToPdfTransformer.PDF_underscoreTITLE);
                    if (title != null) {
                        document.addTitle(title);
                    } else if (base_underscoreurl != null) {
                        document.addTitle(base_underscoreurl);
                    }
                    final String creator = (String) properties.get(IHtmlToPdfTransformer.PDF_underscoreCREATOR);
                    if (creator != null) {
                        document.addCreator(creator);
                    } else {
                        document.addCreator(IHtmlToPdfTransformer.VERSION);
                    }
                    final String author = (String) properties.get(IHtmlToPdfTransformer.PDF_underscoreAUTHOR);
                    if (author != null) {
                        document.addAuthor(author);
                    }
                    final String sproducer = (String) properties.get(IHtmlToPdfTransformer.PDF_underscorePRODUCER);
                    if (sproducer != null) {
            ...
            out.close();
            {
                ...
                while (i < n) {
                    ...
                    for (final Iterator it = hf.iterator(); it.hasNext(); ) {
                        ...
                        if (chf.getSfor().equals(CHeaderFooter.ODD_underscorePAGES) && (i % 2 == 0)) {
                            continue;
                        } else if (chf.getSfor().equals(CHeaderFooter.EVEN_underscorePAGES) && (i % 2 != 0)) {
                            continue;
                        }
                        ...
                        if (chf.getType().equals(CHeaderFooter.HEADER)) {
                            trans.transform(new ByteArrayInputStream(text.getBytes("utf-8")), base_underscoreurl, new PageSize(dsize[0] - (dmargin[0] + dmargin[1]), dmargin[3]), new ArrayList(), properties, bbout);
                        } else if (chf.getType().equals(CHeaderFooter.FOOTER)) {
                            trans.transform(new ByteArrayInputStream(text.getBytes("utf-8")), base_underscoreurl, new PageSize(dsize[0] - (dmargin[0] + dmargin[1]), dmargin[2]), new ArrayList(), properties, bbout);
                        }
            ...
            if (signed) {
                final String keypassword = (String) properties.get(IHtmlToPdfTransformer.PDF_underscoreSIGNING_underscorePRIVATE_underscoreKEY_underscorePASSWORD);
                final String password = (String) properties.get(IHtmlToPdfTransformer.PDF_underscoreENCRYPTION_underscorePASSWORD);
                final String keyStorepassword = (String) properties.get(IHtmlToPdfTransformer.PDF_underscoreSIGNING_underscoreKEYSTORE_underscorePASSWORD);
                final String privateKeyFile = (String) properties.get(IHtmlToPdfTransformer.PDF_underscoreSIGNING_underscorePRIVATE_underscoreKEY_underscoreFILE);
                final String reason = (String) properties.get(IHtmlToPdfTransformer.PDF_underscoreSIGNING_underscoreREASON);
                final String location = (String) properties.get(IHtmlToPdfTransformer.PDF_underscoreSIGNING_underscoreLOCATION);
                final boolean selfSigned = !"false".equals(properties.get(IHtmlToPdfTransformer.USE_underscorePDF_underscoreSELF_underscoreSIGNING));
                ...
                if (selfSigned) {
                    sap.setCrypto(key, chain, null, PdfSignatureAppearance.SELF_underscoreSIGNED);
                } else {
                    sap.setCrypto(key, chain, null, PdfSignatureAppearance.WINCER_underscoreSIGNED);
                }
                if (reason != null) {
