    public long create(long mimeTypeId, long sanId) throws SQLException {
        ...
        try {
            ...
            stmt.execute(NEXT_underscoreFILE_underscoreID);
            ResultSet rs = stmt.getResultSet();
            while (rs.next()) {
                fileId = rs.getLong(NEXTVAL);
            }
            PreparedStatement pstmt = conn.prepareStatement(INSERT_underscoreFILE);
            pstmt.setLong(1, fileId);
            pstmt.setLong(2, mimeTypeId);
            pstmt.setLong(3, sanId);
            pstmt.setLong(4, WORKFLOW_underscoreATTENTE_underscoreVALIDATION);
