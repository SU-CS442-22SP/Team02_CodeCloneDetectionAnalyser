    ...
    public void saveStructure(long userId, TreeStructureInfo info, List<TreeStructureNode> structure) throws DatabaseException {
        ...
        try {
            insertInfoSt = getConnection().prepareStatement(INSERT_underscoreINFO);
            ...
            PreparedStatement seqSt = getConnection().prepareStatement(INFO_underscoreVALUE);
            ...
            for (TreeStructureNode node : structure) {
                insSt = getConnection().prepareStatement(INSERT_underscoreNODE);
                insSt.setLong(1, key);
            ...
            if (total != structure.size()) {
                getConnection().rollback();
                throw new DatabaseException("Unable to insert _underscoreALL_underscore nodes: " + total + " nodes were inserted of " + structure.size());
            }
