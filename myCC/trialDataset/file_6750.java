    public HttpURLConnection execute(S3Bucket pBucket, S3Object pObject, S3OperationParameters pOpParams) throws S3Exception {
        ...
        switch(mStyle) {
            case Path:
                opParams.addHostHeader(BASE_underscoreDOMAIN);
                break;
            case Subdomain:
                if (pBucket == null) opParams.addHostHeader(BASE_underscoreDOMAIN); else opParams.addHostHeader(pBucket.getName() + "." + BASE_underscoreDOMAIN);
                break;
            case VirtualHost:
                if (pBucket == null) opParams.addHostHeader(BASE_underscoreDOMAIN); else opParams.addHostHeader(pBucket.getName());
                break;
        }
        if (opParams.isSign()) {
            ...
            try {
                sb.append("/");
                if (pBucket != null) {
                    sb.append(URLEncoder.encode(pBucket.getName(), URL_underscoreENCODING));
                    sb.append("/");
                    if (pObject != null) {
                        sb.append(URLEncoder.encode(pObject.getKey(), URL_underscoreENCODING));
                    }
                }
                ...
                throw new S3Exception("URL encoding not supported: " + URL_underscoreENCODING, e);
            }
        }
        try {
            killHostVerifier();
