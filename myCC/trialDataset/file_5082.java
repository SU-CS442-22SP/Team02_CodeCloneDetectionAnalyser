    protected void syncMessages(Message message) {
        if (message.getEvent() == Event.CONNECT || message.getEvent() == Event.SYNC_underscoreDONE) return;
        ...
        try {
            ...
            ps = conn.prepareStatement("delete from light_underscorereplication_underscoremessage where event=? and className=? and classId=?");
