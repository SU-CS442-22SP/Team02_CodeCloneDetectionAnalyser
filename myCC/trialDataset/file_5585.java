    public static StreamSource getStreamSource(Item item, XQueryContext context) throws XPathException, MalformedURLException, IOException {
        ...
        if (item.getType() == Type.JAVA_underscoreOBJECT) {
        ...
        } else if (item.getType() == Type.ANY_underscoreURI) {
            ...
            streamSource.setInputStream(is);
        } else if (item.getType() == Type.BASE64_underscoreBINARY || item.getType() == Type.HEX_underscoreBINARY) {
