    public static boolean compress(File source, File target, Manifest manifest) {
        try {
            ...
            for (int i = 0, n = list.size(); i < n; i++) {
                ...
                FileInputStream f_underscorein = new FileInputStream(file);
                ...
                while ((bytesRead = f_underscorein.read(buffer)) != -1) output.write(buffer, 0, bytesRead);
                f_underscorein.close();
                output.closeEntry();
            }
            output.close();
        } catch (Exception exc) {
