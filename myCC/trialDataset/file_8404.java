    void IconmenuItem5_underscoreactionPerformed(ActionEvent e) {
        ...
        if (JFileChooser.APPROVE_underscoreOPTION == jFileChooser1.showOpenDialog(this.getFatherFrame())) {
            ...
            try {
                ...
                JOptionPane.showMessageDialog(null, ex.getMessage().substring(0, Math.min(ex.getMessage().length(), getFatherPanel().MAX_underscoreDIALOG_underscoreMSG_underscoreSZ)) + "-" + getClass(), "", JOptionPane.ERROR_underscoreMESSAGE);
            }
            setPath(newPath);
            if (getDefaultPath() == null || getDefaultPath().equals("")) {
                ...
                if (JOptionPane.showConfirmDialog(null, msgString.substring(0, Math.min(msgString.length(), getFatherPanel().MAX_underscoreDIALOG_underscoreMSG_underscoreSZ)), "choose one", JOptionPane.YES_underscoreNO_underscoreOPTION) == JOptionPane.YES_underscoreOPTION) {
