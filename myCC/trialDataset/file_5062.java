    public void addRegisterInfo(HttpServletRequest request) throws ApplicationException {
        ...
        try {
            ...
            for (String pid : pids) {
                ...
                pd.productName = StrFun.getString(request, "productName_underscore" + pid);
                pd.regAmount = StrFun.getInt(request, "regAmount_underscore" + pid);
                pd.regPrice = StrFun.getInt(request, "regPrice_underscore" + pid);
                pd.regSalePrice = StrFun.getInt(request, "regSalePrice_underscore" + pid);
                pd.userNo = ServerUtil.getUserFromSession(request).userNo;
                if (pd.regAmount <= 0) throw new ApplicationException("�����������Ϊ��");
                String sql = "insert into SS_underscoreRegisterInfo " + "(pid, rid, productName, regAmount, regPrice, regSalePrice, userNo) " + "values(" + "'" + pd.pid + "', " + "'" + pd.rid + "', " + "'" + pd.productName + "', " + "'" + pd.regAmount + "', " + "'" + pd.regPrice + "', " + "'" + pd.regSalePrice + "', " + "'" + pd.userNo + "' " + ")";
