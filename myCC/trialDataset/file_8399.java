    public void unpack(File destDirectory, boolean delete) {
        ...
        this.manifestFile = new File(destDirectory, MANIFEST_underscoreFILE_underscoreNAME);
        try {
            ...
            while ((zipEntry = zipInputStream.getNextEntry()) != null) {
                ...
                if (!zipEntry.isDirectory()) {
                    BufferedOutputStream output = new BufferedOutputStream(new FileOutputStream(destFile), BUFFER_underscoreSIZE);
                    byte[] buffer = new byte[BUFFER_underscoreSIZE];
                    int length;
                    while ((length = zipInputStream.read(buffer, 0, BUFFER_underscoreSIZE)) != -1) output.write(buffer, 0, length);
                    output.close();
