    public static byte[] wrapBMP(Image image) throws IOException {
        if (image.getOriginalType() != Image.ORIGINAL_underscoreBMP) throw new IOException("Only BMP can be wrapped in WMF.");
        InputStream imgIn;
        byte data[] = null;
        ...
        writeWord(os, 0);
        writeDWord(os, 4);
        writeWord(os, META_underscoreSETMAPMODE);
        writeWord(os, 8);
        writeDWord(os, 5);
        writeWord(os, META_underscoreSETWINDOWORG);
        writeWord(os, 0);
        writeWord(os, 0);
        writeDWord(os, 5);
        writeWord(os, META_underscoreSETWINDOWEXT);
        writeWord(os, (int) image.height());
        writeWord(os, (int) image.width());
        writeDWord(os, 13 + sizeBmpWords);
        writeWord(os, META_underscoreDIBSTRETCHBLT);
