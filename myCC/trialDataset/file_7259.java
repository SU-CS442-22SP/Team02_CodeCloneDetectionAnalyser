    public void run() {
        try {
            ...
            while (total_underscoreread < CL && (bytes_underscoreread = sockWebInputStream.read(buffer)) != -1) {
                if (!resourceExists) {
                    if (10000000000L == CL) {
                        String s = new String(buffer, 0, bytes_underscoreread);
                    ...
                    if (bytes_underscoreread >= 7) {
                        byte end[] = new byte[7];
                        System.arraycopy(buffer, bytes_underscoreread - 7, end, 0, 7);
                ...
                baos.write(buffer, 0, bytes_underscoreread);
                total_underscoreread += bytes_underscoreread;
            }
            if (inMemory) {
                baos.close();
