    public void appendMessage(MimeMessage oMsg) throws FolderClosedException, StoreClosedException, MessagingException {
        ...
        String gu_underscoremimemsg;
        if (oMsg.getClass().getName().equals("com.knowgate.hipermail.DBMimeMessage")) {
            gu_underscoremimemsg = ((DBMimeMessage) oMsg).getMessageGuid();
            if (((DBMimeMessage) oMsg).getFolder() == null) ((DBMimeMessage) oMsg).setFolder(this);
        } else {
            gu_underscoremimemsg = Gadgets.generateUUID();
        }
        String gu_underscoreworkarea = ((DBStore) getStore()).getUser().getString(DB.gu_underscoreworkarea);
        ...
        if (sMessageID == null) sMessageID = gu_underscoremimemsg; else if (sMessageID.length() == 0) sMessageID = gu_underscoremimemsg;
        Timestamp tsSent;
        ...
        if ((iOpenMode & MODE_underscoreMBOX) != 0) {
            try {
                if (DebugFile.trace) DebugFile.writeln("new File(" + Gadgets.chomp(sFolderDir, File.separator) + oCatg.getStringNull(DB.nm_underscorecategory, "null") + ".mbox)");
                oFile = getFile();
                ...
                sMsgCharSeq = byOutStrm.toString("ISO8859_underscore1");
                byOutStrm.close();
        ...
        String sSQL = "INSERT INTO " + DB.k_underscoremime_underscoremsgs + "(gu_underscoremimemsg,gu_underscoreworkarea,gu_underscorecategory,id_underscoretype,id_underscorecontent,id_underscoremessage,id_underscoredisposition,len_underscoremimemsg,tx_underscoremd5,de_underscoremimemsg,file_underscorename,tx_underscoreencoding,tx_underscoresubject,dt_underscoresent,dt_underscorereceived,tx_underscoreemail_underscorefrom,nm_underscorefrom,tx_underscoreemail_underscorereply,nm_underscoreto,id_underscorepriority,bo_underscoreanswered,bo_underscoredeleted,bo_underscoredraft,bo_underscoreflagged,bo_underscorerecent,bo_underscoreseen,bo_underscorespam,pg_underscoremessage,nu_underscoreposition,by_underscorecontent) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        if (DebugFile.trace) DebugFile.writeln("Connection.prepareStatement(" + sSQL + ")");
        PreparedStatement oStmt = null;
        try {
            oStmt = oConn.prepareStatement(sSQL);
            oStmt.setString(1, gu_underscoremimemsg);
            oStmt.setString(2, gu_underscoreworkarea);
            if (oCatg.isNull(DB.gu_underscorecategory)) oStmt.setNull(3, Types.CHAR); else oStmt.setString(3, oCatg.getString(DB.gu_underscorecategory));
            ...
            if (oConn.getDataBaseProduct() == JDCConnection.DBMS_underscoreORACLE) {
                ...
                if ((iOpenMode & MODE_underscoreMBOX) != 0) oStmt.setBigDecimal(29, new BigDecimal(lPosition)); else oStmt.setNull(29, Types.NUMERIC);
            ...
            throw new MessagingException(DB.k_underscoremime_underscoremsgs + " " + sqle.getMessage(), sqle);
        }
        if ((iOpenMode & MODE_underscoreBLOB) != 0) {
            try {
                byOutStrm.close();
        ...
        try {
            ...
            if (oContent instanceof MimeMultipart) {
                try {
                    saveMimeParts(oMsg, sMsgCharSeq, sBoundary, gu_underscoremimemsg, sMessageID, dPgMessage.intValue(), 0);
                } catch (MessagingException msge) {
                    try {
                        if (oMBox != null) oMBox.close();
                    } catch (Exception ignore) {
        ...
        sSQL = "SELECT " + DB.gu_underscorecontact + "," + DB.gu_underscorecompany + "," + DB.tx_underscorename + "," + DB.tx_underscoresurname + "," + DB.tx_underscoresurname + " FROM " + DB.k_underscoremember_underscoreaddress + " WHERE " + DB.tx_underscoreemail + "=? AND " + DB.gu_underscoreworkarea + "=? UNION SELECT " + DB.gu_underscoreuser + ",'****************************USER'," + DB.nm_underscoreuser + "," + DB.tx_underscoresurname1 + "," + DB.tx_underscoresurname2 + " FROM " + DB.k_underscoreusers + " WHERE (" + DB.tx_underscoremain_underscoreemail + "=? OR " + DB.tx_underscorealt_underscoreemail + "=?) AND " + DB.gu_underscoreworkarea + "=?";
        if (DebugFile.trace) DebugFile.writeln("Connection.prepareStatement(" + sSQL + ")");
        PreparedStatement oAddr = null;
        try {
            oAddr = oConn.prepareStatement(sSQL, ResultSet.TYPE_underscoreFORWARD_underscoreONLY, ResultSet.CONCUR_underscoreREAD_underscoreONLY);
            ResultSet oRSet;
            ...
            if (oFrom != null) {
                ...
                oAddr.setString(2, gu_underscoreworkarea);
                oAddr.setString(3, oFrom.getAddress());
                oAddr.setString(4, oFrom.getAddress());
                oAddr.setString(5, gu_underscoreworkarea);
            ...
            oAddr.close();
            sSQL = "INSERT INTO " + DB.k_underscoreinet_underscoreaddrs + " (gu_underscoremimemsg,id_underscoremessage,tx_underscoreemail,tp_underscorerecipient,gu_underscoreuser,gu_underscorecontact,gu_underscorecompany,tx_underscorepersonal) VALUES ('" + gu_underscoremimemsg + "','" + sMessageID + "',?,?,?,?,?,?)";
            if (DebugFile.trace) DebugFile.writeln("Connection.prepareStatement(" + sSQL + ")");
            oStmt = oConn.prepareStatement(sSQL);
            ...
            oStmt.close();
            oStmt = null;
            oStmt = oConn.prepareStatement("UPDATE " + DB.k_underscorecategories + " SET " + DB.len_underscoresize + "=" + DB.len_underscoresize + "+" + String.valueOf(iSize) + " WHERE " + DB.gu_underscorecategory + "=?");
        ...
        if (DebugFile.trace) {
            DebugFile.decIdent();
            DebugFile.writeln("End DBFolder.appendMessage() : " + gu_underscoremimemsg);
        }
    }
