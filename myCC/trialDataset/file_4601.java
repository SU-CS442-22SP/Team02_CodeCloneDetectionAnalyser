    public boolean deleteRoleType(int id, int namespaceId, boolean removeReferencesInRoleTypes, DTSPermission permit) throws SQLException, PermissionException, DTSValidationException {
        ...
        String sqlRightId = getDAO().getStatement(ROLE_underscoreTYPE_underscoreTABLE_underscoreKEY, "DELETE_underscoreRIGHT_underscoreIDENTITY_underscoreREF");
        String sqlParent = getDAO().getStatement(ROLE_underscoreTYPE_underscoreTABLE_underscoreKEY, "DELETE_underscorePARENT_underscoreREF");
        String sql = getDAO().getStatement(ROLE_underscoreTYPE_underscoreTABLE_underscoreKEY, "DELETE");
        PreparedStatement pstmt = null;
        boolean success = false;
        long typeGid = getGID(namespaceId, id);
        conn.setAutoCommit(false);
        int defaultLevel = conn.getTransactionIsolation();
        conn.setTransactionIsolation(Connection.TRANSACTION_underscoreREAD_underscoreCOMMITTED);
        try {
