                ...
                if (execConsulta.equalsIgnoreCase("delete")) {
                    ...
                    try {
                        ...
                        if (transactionIsolation == 1 || transactionIsolation == 2 || transactionIsolation == 4 || transactionIsolation == 8) {
                            ...
                            throw new IllegalArgumentException("Valor invalido sobre TransactionIsolation,\n TRANSACTION_underscoreNONE no es soportado por MySQL");
                        }
                        st = (Statement) conexion.createStatement(ResultSetImpl.TYPE_underscoreSCROLL_underscoreSENSITIVE, ResultSetImpl.CONCUR_underscoreUPDATABLE);
                        conexion.setReadOnly(false);
                        filasDelete = st.executeUpdate(consulta.trim(), Statement.RETURN_underscoreGENERATED_underscoreKEYS);
                        if (filasDelete > -1) {
                            if (autocommit == false) {
                                conexion.commit();
                            }
