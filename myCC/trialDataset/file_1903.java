    private static Long statusSWGCraftTime() {
        ...
        if (current < (previousStatusCheck + SWGCraft.STATUS_underscoreCHECK_underscoreDELAY)) return previousStatusTime;
        URL url = null;
        try {
            synchronized (previousStatusTime) {
                if (current >= previousStatusCheck + SWGCraft.STATUS_underscoreCHECK_underscoreDELAY) {
