    protected boolean writeFile(Interest outstandingInterest) throws IOException {
        ...
        ContentName versionedName = VersioningProfile.addVersion(new ContentName(_underscoreprefix, outstandingInterest.name().postfix(_underscoreprefix).components()), modificationTime);
        CCNFileOutputStream ccnout = new CCNFileOutputStream(versionedName, _underscorehandle);
        ccnout.addOutstandingInterest(outstandingInterest);
        byte[] buffer = new byte[BUF_underscoreSIZE];
