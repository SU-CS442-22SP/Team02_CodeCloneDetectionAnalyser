    private void generateArchetype(final IProject project, final IDataModel model, final IProgressMonitor monitor, final boolean offline) throws CoreException, InterruptedException, IOException {
        if (getArchetypeArtifactId(model) != null) {
            ...
            String artifact = (String) model.getProperty(IMavenFacetInstallDataModelProperties.PROJECT_underscoreARTIFACT_underscoreID);
            if (artifact == null || artifact.trim().length() == 0) {
                artifact = project.getName();
            }
            properties.put("artifactId", artifact);
            String group = (String) model.getProperty(IMavenFacetInstallDataModelProperties.PROJECT_underscoreGROUP_underscoreID);
            if (group == null || group.trim().length() == 0) {
                group = project.getName();
            }
            properties.put("groupId", group);
            properties.put("version", model.getProperty(IMavenFacetInstallDataModelProperties.PROJECT_underscoreVERSION));
            ...
            final ILaunchConfigurationType launchConfigurationType = launchManager.getLaunchConfigurationType(LAUNCH_underscoreCONFIGURATION_underscoreTYPE_underscoreID);
            ...
            try {
                ...
                workingCopy.setAttribute(ATTR_underscorePOM_underscoreDIR, archetypePomDirectory.getAbsolutePath());
                workingCopy.setAttribute(ATTR_underscorePROPERTIES, convertPropertiesToList(properties));
                String goalName = "archetype:create";
                if (offline) {
                    goalName = new StringBuffer(goalName).append(" -o").toString();
                }
                goalName = updateGoal(goalName);
                workingCopy.setAttribute(ATTR_underscoreGOALS, goalName);
