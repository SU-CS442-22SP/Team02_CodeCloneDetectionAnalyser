        private void checkSites() {
            Log.d(LOG_underscoreTAG, "Updating sites: " + sitesToCheck.keySet().toString());
            for (Map.Entry<Site, Item> entry : sitesToCheck.entrySet()) {
                ...
                try {
                    ...
                    if (response.getStatusLine().getStatusCode() == HttpStatus.SC_underscoreOK) {
                        ...
                        Log.e(LOG_underscoreTAG, "HTTP request for " + site.name() + " failed: " + response.getStatusLine().toString());
                    }
                } catch (Throwable e) {
                    Log.e(LOG_underscoreTAG, e.getMessage(), e);
                }
            }
        }
