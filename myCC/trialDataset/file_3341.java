    protected void doUpload(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String newcommunity = paramMap.getValue(HttpParams.UP2P_underscoreCOMMUNITY);
        ...
        uploadFiles.clear();
        Iterator<String> uploadedFileIter = paramMap.getValues(HttpParams.UP2P_underscoreFILENAME);
        String filename = "";
        try {
        ...
        String editResourceId = paramMap.getValue(HttpParams.UP2P_underscoreEDIT_underscoreRESOURCE);
        ...
        String batchUploadString = paramMap.getValue(HttpParams.UP2P_underscoreBATCH);
        ...
        boolean pushUpload = paramMap.getValue(HttpParams.UP2P_underscorePUSH) != null && paramMap.getValue(HttpParams.UP2P_underscorePUSH).length() > 0;
        String id = "";
        ...
        for (int i = 0; i < uploadFiles.size(); i++) {
            ...
            try {
                ...
                if (batchUpload) {
                    ...
                    uploadFilesCleanup(uploadFiles, tempUploadDir);
                    String respondWithXml = paramMap.getValue(HttpParams.UP2P_underscoreFETCH_underscoreXML);
        ...
        uploadFilesCleanup(uploadFiles, tempUploadDir);
        String ajaxRequest = paramMap.getValue(HttpParams.UP2P_underscoreXMLHTTP);
        String respondWithXml = paramMap.getValue(HttpParams.UP2P_underscoreFETCH_underscoreXML);
        if (respondWithXml != null && respondWithXml.length() > 0) {
            ...
            PrintWriter out = response.getWriter();
            out.println("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
            out.print("<resource id=\"" + request.getParameter(HttpParams.UP2P_underscoreRESOURCE) + "\" ");
            if (request.getParameter(HttpParams.UP2P_underscorePEERID) != null) {
                out.print("peerid=\"" + request.getParameter(HttpParams.UP2P_underscorePEERID) + "\" ");
            }
            out.println("/>");
        } else if (batchUpload) {
