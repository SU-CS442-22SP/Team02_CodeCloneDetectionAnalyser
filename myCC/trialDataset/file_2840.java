    private void trySend(Primitive p) throws IOException {
        ...
        try {
            ...
            mTxManager.notifyErrorResponse(p.getTransactionID(), ImErrorInfo.SERIALIZER_underscoreERROR, "Internal serializer error, primitive: " + p.getType());
            out.close();
            return;
        }
        ...
        if (Log.isLoggable(ImpsLog.PACKET_underscoreTAG, Log.DEBUG)) {
        ...
        if (Log.isLoggable(ImpsLog.PACKET_underscoreTAG, Log.DEBUG)) {
            Log.d(ImpsLog.PACKET_underscoreTAG, statusLine.toString());
            Header[] headers = res.getAllHeaders();
            for (Header h : headers) {
                Log.d(ImpsLog.PACKET_underscoreTAG, h.toString());
            }
        ...
        try {
            if (statusLine.getStatusCode() != HttpURLConnection.HTTP_underscoreOK) {
                mTxManager.notifyErrorResponse(p.getTransactionID(), statusLine.getStatusCode(), statusLine.getReasonPhrase());
                return;
            }
            if (resEntity.getContentLength() == 0) {
                if ((p.getTransactionMode() != TransactionMode.Response) && !p.getType().equals(ImpsTags.Polling_underscoreRequest)) {
                    mTxManager.notifyErrorResponse(p.getTransactionID(), ImErrorInfo.ILLEGAL_underscoreSERVER_underscoreRESPONSE, "bad response from server");
                }
                return;
            }
            ...
            if (!mReceiveQueue.offer(response)) {
                mTxManager.notifyErrorResponse(p.getTransactionID(), ImErrorInfo.UNKNOWN_underscoreERROR, "receiving queue full");
            }
        } catch (ParserException e) {
            ImpsLog.logError(e);
            mTxManager.notifyErrorResponse(p.getTransactionID(), ImErrorInfo.PARSER_underscoreERROR, "Parser error, received a bad response from server");
        } finally {
