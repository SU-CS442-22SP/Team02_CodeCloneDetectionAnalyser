    ...
    public void aggregate() {
        ...
        try {
            ...
            if (StringUtils.isBlank(tableExistsResult)) {
                ...
                createTableSql = createTableSql.replaceAll("auto_underscoreincrement", "");
                createTableSql = createTableSql.replaceAll("AUTO_underscoreINCREMENT", "");
            ...
            prestm = connection.prepareStatement("select max(d_underscoreinsDate) from " + destination);
            rs = prestm.executeQuery();
            Date from = null;
            ...
            if (currentRows > 0) {
                ...
                if (from != null && fromStr != null) {
                    prestm = connection.prepareStatement("INSERT INTO " + destination + " SELECT * FROM " + origin + " WHERE d_underscoreinsDate > '" + fromStr + "'");
                    if (logger.isDebugEnabled()) logger.debug("Query: INSERT INTO " + destination + " SELECT * FROM " + origin + " WHERE d_underscoreinsDate > '" + fromStr + "'");
                } else {
