    public Resource readResource(URL url, ResourceManager resourceManager) throws NAFException {
        ...
        try {
            ...
            for (Node n = doc.getFirstChild(); n != null; n = n.getNextSibling()) {
                if (n.getNodeType() == Node.PROCESSING_underscoreINSTRUCTION_underscoreNODE && "xml-stylesheet".equals(n.getNodeName())) {
            ...
            if (trList != null) {
                for (Transformer t : trList) {
                    ...
                    if (LOGGER_underscoreDUMP.isDebugEnabled()) {
                        StringWriter swr = new StringWriter();
                        DOMUtil.dumpNode(doc, swr);
                        LOGGER_underscoreDUMP.debug("Transformed instance:\n" + swr + "\n");
                    }
                }
            }
            Element rootE = doc.getDocumentElement();
            if (!NAF_underscoreNAMESPACE_underscoreURI.equals(rootE.getNamespaceURI())) throw new NAFException("Root element does not use the NAF namespace");
