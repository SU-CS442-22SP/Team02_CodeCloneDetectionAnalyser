    public static void main(final String[] args) {
        if ((args.length < 1)) {
            String sUsage = "1. _underscoreBIZVIEW=<filename>or<XAware Bizview name> \r\n";
            sUsage += "2. _underscoreOUTPUT=DISPLAY|NONE|FILE \r\n";
            sUsage += "3. _underscoreGEN_underscoreOUT=<output filename> \r\n";
            sUsage += "4. _underscorePOST or _underscoreXADATA=<xml data file> \r\n";
            sUsage += "5. _underscoreUID<=user id> \r\n";
            sUsage += "6. _underscorePWD=<password> \r\n";
            sUsage += "7. _underscoreURL=<appserver url> \r\n";
        ...
        for (int i = 1; i < args.length; i++) {
            ...
            if (args[i].startsWith(GEN_underscoreOUT)) {
                sGenOut = args[i].substring(GEN_underscoreOUT.length() + 1);
                continue;
            }
        ...
        String responseXML = "";
        try {
            sServletUrl += "?_underscoreBIZVIEW=" + filename;
            for (int i = 1; i < args.length; i++) {
                if (args[i].charAt(0) != '_underscore') {
