    public static FileChannel newFileChannel(File file, String rw, boolean enableException) throws IOException {
        ...
        rw = rw.toLowerCase();
        if (rw.equals(MODE_underscoreREAD)) {
        ...
        } else if (rw.equals(MODE_underscoreWRITE)) {
            FileOutputStream fos = new FileOutputStream(file);
            FileChannel ch = fos.getChannel();
            setObjectMap(ch.hashCode(), fos, FOS_underscoreW);
            return ch;
        } else if (rw.equals(MODE_underscoreAPPEND)) {
            if (FileUtil.exists(file, enableException)) {
                RandomAccessFile raf = new RandomAccessFile(file, "rw");
                FileChannel ch = raf.getChannel();
                ch.position(ch.size());
                setObjectMap(ch.hashCode(), raf, FOS_underscoreA);
                return ch;
            }
        } else if (rw.equals(MODE_underscoreREAD_underscoreWRITE)) {
            if (FileUtil.exists(file, enableException)) {
                RandomAccessFile raf = new RandomAccessFile(file, rw);
                FileChannel ch = raf.getChannel();
