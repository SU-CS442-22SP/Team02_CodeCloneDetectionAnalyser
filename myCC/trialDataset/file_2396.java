    protected void createValueListAnnotation(IProgressMonitor monitor, IPackageFragment pack, Map model) throws CoreException {
        ...
        QualifiedName qn = new QualifiedName(JstActivator.PLUGIN_underscoreID, JstActivator.PACKAGE_underscoreINFO_underscoreLOCATION);
        String location = pj.getPersistentProperty(qn);
        if (location != null) {
            IFolder javaFolder = pj.getFolder(new Path(NexOpenFacetInstallDataModelProvider.WEB_underscoreSRC_underscoreMAIN_underscoreJAVA));
            ...
            try {
                ...
                IStatus status = new Status(IStatus.ERROR, JeeServiceComponentUIPlugin.PLUGIN_underscoreID, IStatus.OK, "Velocity exception", e);
                throw new CoreException(status);
            } finally {
                try {
                    baos.close();
