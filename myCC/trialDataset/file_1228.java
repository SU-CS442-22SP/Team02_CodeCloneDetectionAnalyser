        ...
        try {
            for (String path : resourceLocations) {
                if (path.startsWith("${") && path.endsWith("}")) {
                    ...
                    if (key.equals(ApplicationConstants.RESOURCE_underscoreSQL_underscoreLOCATION_underscorePROP_underscoreNAME)) path = AdminHelper.getRepository().getURI("sql"); else path = AdminHelper.getRepository().getSetupApplicationProperties().get(key);
                    log.debug(key + "=" + path);
                }
