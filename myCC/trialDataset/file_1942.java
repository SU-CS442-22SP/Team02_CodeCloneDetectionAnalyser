    Library(MainFrame mf, boolean newLibrary, String templateName, String newFileName) throws Exception {
        ...
        if (newLibrary) {
            if (templateName == null) {
                Object resp = JOptionPane.showInputDialog(mainFrame.frame, "Please select a template.", "Select Type of Library", JOptionPane.WARNING_underscoreMESSAGE, null, options, null);
                if (resp == null) return;
            ...
            if (newFileName == null) {
                ...
                fileChooser.setFileSelectionMode(JFileChooser.FILES_underscoreONLY);
                ...
                fileChooser.setDialogType(JFileChooser.SAVE_underscoreDIALOG);
                int returnVal = fileChooser.showSaveDialog(mainFrame.frame);
                if (returnVal == fileChooser.APPROVE_underscoreOPTION) {
            ...
            if (propFile.exists()) {
                if (JOptionPane.showConfirmDialog(mainFrame.frame, "File " + propFileName + " already exists. Do you want to overwrite it ?", "Warning", JOptionPane.YES_underscoreNO_underscoreOPTION) != JOptionPane.YES_underscoreOPTION) return;
            }
