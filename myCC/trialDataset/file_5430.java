    public ProcessorOutput createOutput(String name) {
        ProcessorOutput output = new ProcessorImpl.CacheableTransformerOutputImpl(getClass(), name) {
            ...
            protected void readImpl(org.orbeon.oxf.pipeline.api.PipelineContext context, final ContentHandler contentHandler) {
                ProcessorInput i = getInputByName(INPUT_underscoreDATA);
                try {
                    Grammar grammar = (Grammar) readCacheInputAsObject(context, getInputByName(INPUT_underscoreCONFIG), new CacheableInputReader() {
                        ...
                        private void generateErrorElement(ValidationException ve) throws SAXException {
                            if (decorateOutput && ve != null) {
                                if (!stopDecorating) {
                                    ...
                                    a.addAttribute("", ValidationProcessor.MESSAGE_underscoreATTRIBUTE, ValidationProcessor.MESSAGE_underscoreATTRIBUTE, "CDATA", ve.getSimpleMessage());
                                    a.addAttribute("", ValidationProcessor.SYSTEMID_underscoreATTRIBUTE, ValidationProcessor.SYSTEMID_underscoreATTRIBUTE, "CDATA", ve.getLocationData().getSystemID());
                                    a.addAttribute("", ValidationProcessor.LINE_underscoreATTRIBUTE, ValidationProcessor.LINE_underscoreATTRIBUTE, "CDATA", Integer.toString(ve.getLocationData().getLine()));
                                    a.addAttribute("", ValidationProcessor.COLUMN_underscoreATTRIBUTE, ValidationProcessor.COLUMN_underscoreATTRIBUTE, "CDATA", Integer.toString(ve.getLocationData().getCol()));
                                    contentHandler.startElement(ValidationProcessor.ORBEON_underscoreERROR_underscoreNS, ValidationProcessor.ERROR_underscoreELEMENT, ValidationProcessor.ORBEON_underscoreERROR_underscorePREFIX + ":" + ValidationProcessor.ERROR_underscoreELEMENT, a);
                                    contentHandler.endElement(ValidationProcessor.ORBEON_underscoreERROR_underscoreNS, ValidationProcessor.ERROR_underscoreELEMENT, ValidationProcessor.ORBEON_underscoreERROR_underscorePREFIX + ":" + ValidationProcessor.ERROR_underscoreELEMENT);
                                    stopDecorating = true;
                                }
                            } else {
                                throw ve;
                    ...
                    readInputAsSAX(context, getInputByName(INPUT_underscoreDATA), verifier);
                } catch (Exception e) {
