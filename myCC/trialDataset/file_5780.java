        ...
        try {
            ...
            while (zipEntries.hasMoreElements()) {
                ...
                try {
                    ...
                    if (WEB_underscoreINF_underscoreWEB_underscoreXML.equals(target)) {
                        System.out.println("Found web.xml");
                        webXml = parseXml(source);
                    } else if (WEB_underscoreINF_underscorePORTLET_underscoreXML.equals(target)) {
                        System.out.println("Found WEB-INF/portlet.xml");
                        portletXml = parseXml(source);
                    } else if (META_underscoreINF_underscoreCONTEXT_underscoreXML.equals(target)) {
                ...
                try {
                    ...
                    if (stripLoggers && target.endsWith(".jar") && (target.startsWith("WEB-INF/lib/commons-logging") || target.startsWith("WEB-INF/lib/log4j"))) {
                        ...
                        if (webRewriter.getRealPortletTldFile() != null && fullTarget.equals(webRewriter.getRealPortletTldFile())) {
                            ...
                            continue;
                        } else if (target.equals(WEB_underscoreINF_underscoreWEB_underscoreXML)) {
                            System.out.println("Skip web.xml file " + target);
                            continue;
                        } else if (target.equals(WEB_underscoreINF_underscorePORTLET_underscoreXML)) {
                            System.out.println("Skip portlet.xml file " + target);
                            continue;
                        } else if (target.equals(META_underscoreINF_underscoreCONTEXT_underscoreXML)) {
            ...
            addFile(WEB_underscoreINF_underscoreWEB_underscoreXML, webXml, jout);
            addFile(WEB_underscoreINF_underscorePORTLET_underscoreXML, portletXml, jout);
            addFile(META_underscoreINF_underscoreCONTEXT_underscoreXML, contextXml, jout);
