    public void save(File selectedFile) throws IOException {
        if (storeEntriesInFiles) {
            ...
            for (int i = 0; i < tempFiles.size(); i++) {
                File newFile = new File(selectedFile.getAbsolutePath() + "_underscore" + Integer.toString(i) + ".zettmp");
        ...
        XStream xml_underscoreconvert = new XStream();
        xml_underscoreconvert.setMode(XStream.ID_underscoreREFERENCES);
        xml_underscoreconvert.toXML(this, output);
        output.flush();
        output.close();
    }
