    public final void provisionGSLAM() {
        try {
            ...
            for (SLA s : slas) {
                ...
                for (Party p : parties) {
                    System.out.println("Printing gslam_underscoreepr value for Party" + p.getId() + "--" + p.getAgreementRole());
                    System.out.println(parties[0].getPropertyValue(new STND("gslam_underscoreepr")));
                }
            }
            sslamContext.getSLARegistry().getIRegister().register(slas[0], slaUUID, SLAState.OBSERVED);
            String xmlFile2Send = System.getenv("SLASOI_underscoreHOME") + System.getProperty("file.separator") + "Integration" + System.getProperty("file.separator") + "soap" + System.getProperty("file.separator") + "provision.xml";
            String soapAction = "";
            URL url;
