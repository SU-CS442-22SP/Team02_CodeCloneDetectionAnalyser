    public void readPage(String search) {
        ...
        try {
            ...
            while (scan.hasNextLine()) {
                temp = (scan.nextLine());
                if (temp.contains("<td style='padding-top:4px;' class='ergebnisse_underscoreres'>")) {
                    int anfang = temp.indexOf("-->") + 3;
                    temp = temp.substring(anfang);
                    temp = temp.substring(0, temp.indexOf("<!--"));
                    translate = temp.trim();
                } else if (temp.contains("<td style='' class='ergebnisse_underscoreart'>") || temp.contains("<td style='' class='ergebnisse_underscoreart_underscoredif'>") || temp.contains("<td style='padding-top:4px;' class='ergebnisse_underscoreart'>")) {
                    if (searchEnglish == false && searchGerman == false) {
                        searchEnglish = temp.contains("<td style='' class='ergebnisse_underscoreart'>");
                        searchGerman = temp.contains("<td style='' class='ergebnisse_underscoreart_underscoredif'>");
                    }
