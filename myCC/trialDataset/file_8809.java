    public void downSync(Vector v) throws SQLException {
        try {
            ...
            PreparedStatement update = con.prepareStatement("update cal_underscoreEvent set owner=?,subject=?,text=?,place=?," + "contactperson=?,startdate=?,enddate=?,starttime=?,endtime=?,allday=?," + "syncstatus=?,dirtybits=? where OId=? and syncstatus=?");
            PreparedStatement insert = con.prepareStatement("insert into cal_underscoreEvent (owner,subject,text,place," + "contactperson,startdate,enddate,starttime,endtime,allday,syncstatus," + "dirtybits) values(?,?,?,?,?,?,?,?,?,?,?,?)");
            PreparedStatement insert1 = con.prepareStatement(DBUtil.getQueryCurrentOID(con, "cal_underscoreEvent", "newoid"));
            PreparedStatement delete1 = con.prepareStatement("delete from  cal_underscoreEvent_underscoreRemind where event=?");
            PreparedStatement delete2 = con.prepareStatement("delete from  cal_underscoreEvent where OId=? " + "and (syncstatus=? or syncstatus=?)");
