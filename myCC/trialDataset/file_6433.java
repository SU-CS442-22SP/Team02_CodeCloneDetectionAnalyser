    public static void saveDigraph(mainFrame parentFrame, DigraphView digraphView, File tobeSaved) {
        ...
        try {
            if (!DigraphFile.DIGRAPH_underscoreFILE_underscoreEXTENSION.equals(getExtension(tobeSaved))) {
                tobeSaved = new File(tobeSaved.getPath() + "." + DigraphFile.DIGRAPH_underscoreFILE_underscoreEXTENSION);
            }
            File dtdFile = new File(tobeSaved.getParent() + "/" + DigraphFile.DTD_underscoreFILE);
            if (!dtdFile.exists()) {
            ...
            JOptionPane.showMessageDialog(parentFrame, "Error Saving File:\n" + exep.getMessage(), "Save Error", JOptionPane.ERROR_underscoreMESSAGE);
