    public ActionForward sendTrackback(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) throws RollerException {
        ...
        try {
            ...
            if (entryid == null) {
                entryid = request.getParameter(RollerRequest.WEBLOGENTRYID_underscoreKEY);
            }
            ...
            if (rses.isUserAuthorizedToAuthor(entry.getWebsite())) {
                ...
                String blog_underscorename = entry.getWebsite().getName();
                if (form.getTrackbackUrl() != null) {
                    ...
                    if (!allowTrackback) {
                        errors.add(ActionErrors.GLOBAL_underscoreERROR, new ActionError("error.trackbackNotAllowed"));
                    } else {
                        try {
                            ...
                            data += ("&" + URLEncoder.encode("blog_underscorename", "UTF-8") + "=" + URLEncoder.encode(blog_underscorename, "UTF-8"));
                            ...
                            try {
                                ...
                                if (inputAvailable) {
                                    ...
                                    resultMsg.add(ActionMessages.GLOBAL_underscoreMESSAGE, new ActionMessage("weblogEdit.trackbackResults", resultBuff));
                                }
                                if (conn.getResponseCode() > 399) {
                                    errors.add(ActionErrors.GLOBAL_underscoreERROR, new ActionError("weblogEdit.trackbackStatusCodeBad", new Integer(conn.getResponseCode())));
                                } else {
                                    resultMsg.add(ActionMessages.GLOBAL_underscoreMESSAGE, new ActionMessage("weblogEdit.trackbackStatusCodeGood", new Integer(conn.getResponseCode())));
                                }
                            } finally {
                                if (wr != null) wr.close();
                                if (rd != null) rd.close();
                            ...
                            errors.add(ActionErrors.GLOBAL_underscoreERROR, new ActionError("error.trackback", e));
                        }
                    }
                } else {
                    errors.add(ActionErrors.GLOBAL_underscoreERROR, new ActionError("error.noTrackbackUrlSpecified"));
                }
            ...
            errors.add(ActionErrors.GLOBAL_underscoreERROR, new ActionError("error.general", msg));
        }
        saveErrors(request, errors);
        saveMessages(request, resultMsg);
        request.setAttribute("model", new WeblogEntryPageModel(request, response, mapping, (WeblogEntryFormEx) actionForm, WeblogEntryPageModel.EDIT_underscoreMODE));
        return forward;
    }
