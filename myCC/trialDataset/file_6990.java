    private Map getBlackHoleData() throws Exception {
        File dataFile = new File(Kit.getDataDir() + BLACK_underscoreHOLE);
        if (dataFile.exists() && daysOld(dataFile) < 1) {
            return getStoredData(dataFile);
        }
        InputStream stream = null;
        try {
            String bh_underscoreurl = "http://www.critique.org/users/critters/blackholes/sightdata.html";
            URL url = new URL(bh_underscoreurl);
