    private boolean setupDatabase() {
        if (DoInstallationTasks.DEBUG_underscoreDB) {
        ...
        if (havePachy20) {
            ...
            int result = JOptionPane.showOptionDialog(this.installer, Pachyderm21Installer.ISTRINGS.getString("dit.pachy20msg"), Pachyderm21Installer.ISTRINGS.getString("dit.pachy20title"), JOptionPane.DEFAULT_underscoreOPTION, JOptionPane.QUESTION_underscoreMESSAGE, null, options, options[0]);
            if (result == 0) {
            ...
            int result = JOptionPane.showOptionDialog(this.installer, Pachyderm21Installer.ISTRINGS.getString("dit.nopachy20msg"), Pachyderm21Installer.ISTRINGS.getString("dit.nopachy20title"), JOptionPane.DEFAULT_underscoreOPTION, JOptionPane.QUESTION_underscoreMESSAGE, null, options, options[0]);
            if (result == 1) {
                System.out.println("Aborted by user before doing database");
                return false;
            }
        ...
        if (havePachy20) {
            try {
                ...
                stmt = conn.createStatement();
                stmt.executeUpdate("RENAME TABLE APDEFAULT TO APDEFAULT_underscore2_underscore0_underscore" + d.getTime());
            } catch (SQLException sqle) {
                System.err.println("Error doing check for presentation, " + "means 2.0 doesn't exist");
                System.out.println("SQLException: " + sqle.getMessage());
                System.out.println("SQLState: " + sqle.getSQLState());
        ...
        try {
            ...
            _underscoremd.update(adminPassword.getBytes("UTF-8"));
            byte[] md5 = _underscoremd.digest();
            ...
            if (v.size() > 0) {
                ...
                ps2 = conn.prepareStatement("INSERT INTO `AUTHMAP` " + "(external_underscoreid,external_underscorerealm," + "map_underscoreid,person_underscoreid) " + "VALUES (?,'pachyderm',?,?)");
                ps3 = conn.prepareStatement("INSERT INTO `GROUPPERSONJOIN` " + "(group_underscoreid,person_underscoreid) " + "VALUES(1, ?)");
            }
            for (int i = 0; i < v.size(); ++i) {
                AdminData ad = (AdminData) v.elementAt(i);
                _underscoremd = MessageDigest.getInstance("MD5");
                _underscoremd.update(ad.getPassword().getBytes("UTF-8"));
                md5 = _underscoremd.digest();
