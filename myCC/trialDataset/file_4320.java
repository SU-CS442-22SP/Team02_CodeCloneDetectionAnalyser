    public void run() {
        ...
        if (getCommand() instanceof ListCommand) {
            try {
                ...
                setStatus(ReplyWorker.ERROR_underscoreIO_underscoreEXCEPTION);
            }
            return;
        } else if (getCommand() instanceof RetrieveCommand) {
            RetrieveCommand retrieveCommand = (RetrieveCommand) getCommand();
            if (retrieveCommand.getFromFile().getTransferType().intern() == Command.TYPE_underscoreI || retrieveCommand.getFromFile().getTransferType().intern() == Command.TYPE_underscoreA) {
                try {
                    log.debug("Download file: " + retrieveCommand.getFromFile().toString());
                    FileOutputStream out = null;
                    FileChannel channel = null;
                    if (getDownloadMethod() == RetrieveCommand.FILE_underscoreBASED) {
                    ...
                    } else if (getDownloadMethod() == RetrieveCommand.BYTEBUFFER_underscoreBASED) {
                    }
                    int amount;
                    try {
                        while ((amount = getSocketProvider().read(buffer)) != -1) {
                            ...
                            while (buffer.hasRemaining()) {
                                if (getDownloadMethod() == RetrieveCommand.STREAM_underscoreBASED) {
                    ...
                    setStatus(ReplyWorker.ERROR_underscoreFILE_underscoreNOT_underscoreFOUND);
                }
            ...
            if (storeCommand.getToFile().getTransferType().intern() == Command.TYPE_underscoreI || storeCommand.getToFile().getTransferType().intern() == Command.TYPE_underscoreA) {
                try {
