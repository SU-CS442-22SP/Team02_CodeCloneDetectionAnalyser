    public void buildSiteForEdit(String sBasePath, String sOutputPath, String sCtrlPath, String sMenuPath, String sIntegradorPath, String sSelPageOptions, Properties oEnvironmentProps, Properties oUserProps) throws IOException, DOMException, TransformerException, TransformerConfigurationException, MalformedURLException {
        ...
        if (oContainers == null) {
            ...
            throw new DOMException(DOMException.NOT_underscoreFOUND_underscoreERR, "<containers> node not found");
        }
        ...
        for (int c = 0; c < vPages.size(); c++) {
            ...
            try {
                ...
                sMedia = oTransformer.getOutputProperty(OutputKeys.MEDIA_underscoreTYPE);
                if (DebugFile.trace) DebugFile.writeln(OutputKeys.MEDIA_underscoreTYPE + "=" + sMedia);
                ...
                if (DebugFile.trace) DebugFile.writeln("Page.filePath(" + sOutputPath + oCurrentPage.getTitle().replace(' ', '_underscore') + "." + sMedia + ")");
                oCurrentPage.filePath(sOutputPath + oCurrentPage.getTitle().replace(' ', '_underscore') + "." + sMedia);
                StylesheetCache.setParameters(oTransformer, oEnvironmentProps);
                StylesheetCache.setParameters(oTransformer, oUserProps);
                oTransformer.setParameter("param_underscorepage", ((Page) (vPages.get(c))).getTitle());
            ...
            if (DebugFile.trace) DebugFile.writeln("new FileWriter(" + sOutputPath + oCurrentPage.getTitle().replace(' ', '_underscore') + "_underscore." + sMedia + ")");
            if (sSelPageOptions.length() == 0) oFS.writefilestr(sOutputPath + oCurrentPage.getTitle().replace(' ', '_underscore') + "_underscore." + sMedia, oPostTransform.toString(), "UTF-8"); else try {
                oFS.writefilestr(sOutputPath + oCurrentPage.getTitle().replace(' ', '_underscore') + "_underscore." + sMedia, Gadgets.replace(oPostTransform.toString(), ":selPageOptions", sSelPageOptions), "UTF-8");
            } catch (Exception e) {
            }
            oPostTransform = null;
            sTransformed = null;
