    private boolean confirmAndModify(MDPRArchiveAccessor archiveAccessor) {
        ...
        MessageFormat format = new MessageFormat(AUTO_underscoreMOD_underscoreMESSAGE);
        String message = format.format(new String[] { backupName });
        boolean ok = MessageDialog.openQuestion(new Shell(Display.getDefault()), AUTO_underscoreMOD_underscoreTITLE, message);
        if (ok) {
