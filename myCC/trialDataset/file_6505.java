    protected static byte[] httpConnection(Context context, long token, String url, byte[] pdu, int method, boolean isProxySet, String proxyHost, int proxyPort) throws IOException {
        ...
        if (LOCAL_underscoreLOGV) {
            Log.v(TAG, "httpConnection: params list");
            Log.v(TAG, "\ttoken\t\t= " + token);
            Log.v(TAG, "\turl\t\t= " + url);
            Log.v(TAG, "\tUser-Agent\t\t=" + mUserAgent);
            Log.v(TAG, "\tmethod\t\t= " + ((method == HTTP_underscorePOST_underscoreMETHOD) ? "POST" : ((method == HTTP_underscoreGET_underscoreMETHOD) ? "GET" : "UNKNOWN")));
        ...
        try {
            ...
            HttpHost target = new HttpHost(hostUrl.getHost(), hostUrl.getPort(), HttpHost.DEFAULT_underscoreSCHEME_underscoreNAME);
            client = createHttpClient(context);
            HttpRequest req = null;
            switch(method) {
                case HTTP_underscorePOST_underscoreMETHOD:
                    ...
                    req = post;
                    break;
                case HTTP_underscoreGET_underscoreMETHOD:
                    req = new HttpGet(url);
                    break;
                default:
                    Log.e(TAG, "Unknown HTTP method: " + method + ". Must be one of POST[" + HTTP_underscorePOST_underscoreMETHOD + "] or GET[" + HTTP_underscoreGET_underscoreMETHOD + "].");
                    return null;
            }
            ...
            req.addHeader(HDR_underscoreKEY_underscoreACCEPT, HDR_underscoreVALUE_underscoreACCEPT);
            {
            ...
            if (extraHttpParams != null) {
                String line1Number = ((TelephonyManager) context.getSystemService(Context.TELEPHONY_underscoreSERVICE)).getLine1Number();
            ...
            req.addHeader(HDR_underscoreKEY_underscoreACCEPT_underscoreLANGUAGE, HDR_underscoreVALUE_underscoreACCEPT_underscoreLANGUAGE);
