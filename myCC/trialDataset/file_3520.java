    ...
    protected int run(CmdLineParser parser) {
        ...
        conf.setStrings(INPUT_underscorePATHS_underscorePROP, strInputs.toArray(new String[0]));
        conf.set(SortOutputFormat.OUTPUT_underscoreNAME_underscorePROP, intermediateOutName);
        ...
        if (out != null) try {
            ...
            final FileStatus[] parts = srcFS.globStatus(new Path(wrkDir, conf.get(SortOutputFormat.OUTPUT_underscoreNAME_underscorePROP) + "-[0-9][0-9][0-9][0-9][0-9][0-9]*"));
            {
                int i = 0;
            ...
            outs.write(BlockCompressedStreamConstants.EMPTY_underscoreGZIP_underscoreBLOCK);
            outs.close();
