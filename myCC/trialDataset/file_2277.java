    public Boolean compress(String sSourceDir, ArrayList<String> aFiles, String sDestinationFilename) {
        ...
        try {
            ...
            aData = new byte[BUFFER_underscoreSIZE];
            ...
            while (oIterator.hasNext()) {
                try {
                    ...
                    oOrigin = new BufferedInputStream(fisInput, BUFFER_underscoreSIZE);
                    ZipEntry oEntry = new ZipEntry(sFilename.replace('\\', '/'));
                    oOutput.putNextEntry(oEntry);
                    int iCount;
                    while ((iCount = oOrigin.read(aData, 0, BUFFER_underscoreSIZE)) != -1) oOutput.write(aData, 0, iCount);
                } finally {
