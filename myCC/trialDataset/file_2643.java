    public void setPilot(PilotData pilotData) throws UsernameNotValidException {
        try {
            ...
            if (pilotData.id == 0) {
                psta = jdbc.prepareStatement("INSERT INTO pilot " + "(name, address1, address2, zip, city, state, country, birthdate, " + "pft_underscoretheory, pft, medical, passenger, instructor, loc_underscorelanguage, " + "loc_underscorecountry, loc_underscorevariant, username, password, id) " + "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                pilotData.id = Sequence.nextVal("pilot_underscoreid", jdbc);
            } else {
                psta = jdbc.prepareStatement("UPDATE pilot SET " + "name = ?, address1 = ?, address2 = ?, " + "zip = ?, city = ?, state = ?, country = ?, birthdate = ?, pft_underscoretheory = ?," + "pft = ?, medical = ?, passenger = ?, instructor = ?, loc_underscorelanguage = ?, " + "loc_underscorecountry = ?, loc_underscorevariant = ?, username = ?, password = ? " + "WHERE id = ?");
            }
            ...
            if (pilotData.pft_underscoretheory != null) psta.setLong(9, pilotData.pft_underscoretheory.getTime()); else psta.setNull(9, java.sql.Types.INTEGER);
            ...
            psta.setString(14, pilotData.loc_underscorelanguage);
            psta.setString(15, pilotData.loc_underscorecountry);
            psta.setString(16, pilotData.loc_underscorevariant);
