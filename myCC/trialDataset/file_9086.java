    private static void readIzvestiyaArticles() throws IOException {
        CsvReader reader = new CsvReader(new InputStreamReader(IzvestiyaUtil.class.getClassLoader().getResourceAsStream("mathnet_underscoreizvestiya.csv")), ';');
        ...
        for (Map.Entry<String, String> entry : id2filename.entrySet()) {
            String filepath = String.format("%s/%s", INPUT_underscoreDIR, entry.getValue());
            filepath = new File(filepath).exists() ? filepath : filepath.replace(".tex", ".TEX");
            if (new File(filepath).exists()) {
                InputStream in = new FileInputStream(filepath);
                FileOutputStream out = new FileOutputStream(String.format("%s/%s.tex", OUTPUT_underscoreDIR, entry.getKey()), false);
                try {
