    public void updateMetaList() {
        ...
        serverCache.expire(EXPIRE_underscoreINTERVAL * 1000);
        final Map<String, MetaserverEntry> oldEntries = serverCache.getAll();
        final MetaserverEntry localhostMetaserverEntry = MetaserverEntryParser.parseEntry(ServerCache.DEFAULT_underscoreENTRY_underscoreLOCALHOST);
        ...
        try {
            final URL url = new URL(METASERVER_underscoreURL);
            final String httpProxy = System.getenv("http_underscoreproxy");
            if (httpProxy != null && httpProxy.length() > 0) {
                if (httpProxy.regionMatches(true, 0, "http://", 0, 7)) {
                    ...
                    System.err.println("Warning: unsupported http_underscoreproxy protocol: " + httpProxy);
                }
            }
            ...
            try {
                ...
                conn.connect();
                if (conn.getResponseCode() == HttpURLConnection.HTTP_underscoreOK) {
