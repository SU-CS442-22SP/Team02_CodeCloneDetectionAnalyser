    private boolean downloadBlacklist() {
        boolean blacklist_underscoreupdated = false;
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_underscoreNOT_underscoreMODIFIED) {
                mLogger.debug("MT blacklist site says we are current");
                return false;
            }
            long lastModifiedLong = connection.getHeaderFieldDate("Last-Modified", -1);
            if (responseCode == HttpURLConnection.HTTP_underscoreOK && (this.lastModified == null || this.lastModified.getTime() < lastModifiedLong)) {
                ...
                outstream.close();
                instream.close();
                blacklist_underscoreupdated = true;
        ...
        return blacklist_underscoreupdated;
    }
