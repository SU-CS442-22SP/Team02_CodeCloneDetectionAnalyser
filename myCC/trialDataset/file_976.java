    public HttpResponse executeWithoutRewriting(HttpUriRequest request, HttpContext context) throws IOException {
        ...
        try {
            ...
            try {
                ...
                values.put(Checkin.Stats.TAG, Checkin.Stats.Tag.HTTP_underscoreREQUEST + ":" + mAppName);
                mResolver.insert(Checkin.Stats.CONTENT_underscoreURI, values);
                if (mConnectionAllocated.get() == null && code >= 0) {
                    values.put(Checkin.Stats.TAG, Checkin.Stats.Tag.HTTP_underscoreREUSED + ":" + mAppName);
                    mResolver.insert(Checkin.Stats.CONTENT_underscoreURI, values);
                }
                String status = code < 0 ? "IOException" : Integer.toString(code);
                values.put(Checkin.Stats.TAG, Checkin.Stats.Tag.HTTP_underscoreSTATUS + ":" + mAppName + ":" + status);
                mResolver.insert(Checkin.Stats.CONTENT_underscoreURI, values);
            } catch (Exception e) {
