    public static void downloadXrefTask(String url, String file) {
        ...
        try {
            ...
            byte buffer[] = new byte[XREF_underscoreDOWNLOAD_underscoreBUFFER_underscoreSIZE];
            progressi = 0;
            progressn = con.getContentLength();
            n = 1;
            while (n >= 0) {
                n = ii.read(buffer, 0, XREF_underscoreDOWNLOAD_underscoreBUFFER_underscoreSIZE);
                if (n > 0) {
                    oo.write(buffer, 0, n);
                    progressi += n;
                }
            ...
            JOptionPane.showMessageDialog(null, e.toString() + "\nWhile downloading " + url + ".\nMaybe wrong proxy configuration?", "Xrefactory Error", JOptionPane.ERROR_underscoreMESSAGE);
        }
        progress.setVisible(false);
    }
