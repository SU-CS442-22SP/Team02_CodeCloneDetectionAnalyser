    public AssessmentItemType getAssessmentItemType(String filename) {
        if (filename.contains(" ") && (System.getProperty("os.name").contains("Windows"))) {
            ...
            try {
                ...
                long size = in.size();
                MappedByteBuffer buf = in.map(FileChannel.MapMode.READ_underscoreONLY, 0, size);
                out.write(buf);
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
        ...
        try {
            ...
            ChangeNamespace convertfromv2p0tov2p1 = new ChangeNamespace(reader, "http://www.imsglobal.org/xsd/imsqti_underscorev2p0", "http://www.imsglobal.org/xsd/imsqti_underscorev2p1");
