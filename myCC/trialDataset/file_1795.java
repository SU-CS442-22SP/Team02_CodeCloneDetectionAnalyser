    public static void parseString(String str, String name) {
        ...
        try {
            ...
            while (zeile != null) {
                if (zeile.length() > 0) {
                    ...
                    if (firstL) {
                        firstL = false;
                        l_underscoreb = lambda;
                    }
                    curve.put(lambda, intens);
                }
                zeile = reader.readLine();
            }
            l_underscoree = lambda;
        } catch (IOException e) {
        ...
        try {
            ...
            bw.write("# COLOR: " + name + " Auto generated File: 02/09/2009; From " + l_underscoreb + " to " + l_underscoree);
            bw.newLine();
            bw.write(l_underscoreb + "");
            bw.newLine();
            for (int i = l_underscoreb; i <= l_underscoree; i++) {
