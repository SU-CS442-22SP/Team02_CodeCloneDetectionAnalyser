    public static IProject createSimplemodelEnabledJavaProject() throws CoreException {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            newNatures[natures.length] = JavaCore.NATURE_underscoreID;
            newNatures[natures.length + 1] = SimplemodelNature.NATURE_underscoreID;
            description.setNatureIds(newNatures);
            project.setDescription(description, null);
            IJavaProject javaProject = JavaCore.create(project);
            Set<IClasspathEntry> entries = new HashSet<IClasspathEntry>();
            IVMInstall vmInstall = JavaRuntime.getDefaultVMInstall();
            Path containerPath = new Path(JavaRuntime.JRE_underscoreCONTAINER);
