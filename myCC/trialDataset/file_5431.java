    public void createZipCopy(IUIContext ui, final String zipFileName, final File[] filesToZip, final FilenameFilter fileFilter, Timestamp timestamp) {
        ...
        nameCopy = nameCopy + "_underscore" + timestamp.toString() + ".zip";
        ...
        IWorkspaceRunnable noResourceChangedEventsRunner = new IWorkspaceRunnable() {
            ...
            public void run(IProgressMonitor runnerMonitor) throws CoreException {
                try {
                    ...
                    try {
                        out.setLevel(Deflater.DEFAULT_underscoreCOMPRESSION);
                    ...
                    Status error = new Status(Status.ERROR, PlatformActivator.PLUGIN_underscoreID, Status.ERROR, e.getLocalizedMessage(), e);
        ...
        try {
            ...
            workspace.run(noResourceChangedEventsRunner, workspace.getRoot(), IWorkspace.AVOID_underscoreUPDATE, new NullProgressMonitor());
