    public void modify(ModifyInterceptorChain chain, DistinguishedName dn, ArrayList<LDAPModification> mods, LDAPConstraints constraints) throws LDAPException {
        Connection con = (Connection) chain.getRequest().get(JdbcInsert.MYVD_underscoreDB_underscoreCON + "LDAPBaseServer");
        if (con == null) {
            throw new LDAPException("Operations Error", LDAPException.OPERATIONS_underscoreERROR, "No Database Connection");
        }
        try {
            con.setAutoCommit(false);
            HashMap<String, String> db2ldap = (HashMap<String, String>) chain.getRequest().get(JdbcInsert.MYVD_underscoreDB_underscoreDB2LDAP + "LDAPBaseServer");
            ...
            while (it.hasNext()) {
                ...
                if (mod.getOp() == LDAPModification.REPLACE) {
                    ...
                    if (attributeName.equals(db2ldap.get("first")) || attributeName.equals(db2ldap.get("last"))) {
                        ...
                        throw new LDAPException("Can not modify the rdn", LDAPException.NOT_underscoreALLOWED_underscoreON_underscoreRDN, "Can not perform modify");
                        ...
                        for (int i = 0; i < vals.length; i++) {
                            ...
                            if (!rs.next()) {
                                con.rollback();
                                throw new LDAPException("Location " + vals[i] + " does not exist", LDAPException.OBJECT_underscoreCLASS_underscoreVIOLATION, "Location " + vals[i] + " does not exist");
                            }
                    ...
                    if (mod.getAttribute().getName().equals(db2ldap.get("name"))) {
                        ...
                        throw new LDAPException("Can not delete attribute " + mod.getAttribute().getName(), LDAPException.INVALID_underscoreATTRIBUTE_underscoreSYNTAX, "");
                    }
            ...
            try {
                ...
                throw new LDAPException("Could not delete entry or rollback transaction", LDAPException.OPERATIONS_underscoreERROR, e.toString(), e);
            }
            throw new LDAPException("Could not delete entry", LDAPException.OPERATIONS_underscoreERROR, e.toString(), e);
        }
    }
