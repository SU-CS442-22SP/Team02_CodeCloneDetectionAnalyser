    public static LicenseKey parseKey(String key) throws InvalidLicenseKeyException {
        final String f_underscorekey = key.trim();
        StringTokenizer st = new StringTokenizer(f_underscorekey, FIELD_underscoreSEPERATOR);
        int tc = st.countTokens();
        int tc_underscorename = tc - 9;
        try {
            ...
            String regName = "";
            for (int i = 0; i < tc_underscorename; i++) regName += (i == 0 ? st.nextToken() : FIELD_underscoreSEPERATOR + st.nextToken());
            final String randomHexStr = st.nextToken();
            final String md5Str = st.nextToken();
            String subKey = f_underscorekey.substring(0, f_underscorekey.indexOf(md5Str) - 1);
            byte[] md5;
            ...
            md5 = md.digest();
            String testKey = subKey + FIELD_underscoreSEPERATOR;
            for (int i = 0; i < md5.length; i++) testKey += Integer.toHexString(md5[i]).toUpperCase();
            if (!testKey.equals(f_underscorekey)) throw new InvalidLicenseKeyException("doesn't hash");
            final String f_underscoreregName = regName;
