    Bundle install(String location, InputStream is) throws BundleException {
        synchronized (bundlesLock) {
            ...
            try {
                ...
                Version version = Version.parseVersion((String) headers.get(Constants.BUNDLE_underscoreVERSION));
                File cache = createNewCache(bundleId, version);
                File manifestFile = new File(cache, BUNDLE_underscoreMANIFEST_underscoreFILE);
                os = new FileOutputStream(manifestFile);
                ManifestUtil.storeManifest(headers, os);
                os.close();
                if (isBundleInstalled((String) headers.get(Constants.BUNDLE_underscoreSYMBOLICNAME))) {
                    throw new BundleException(new StringBuilder("Bundle(location=").append(location).append(") already installed.").toString());
                }
                ManifestEntry[] entries = ManifestEntry.parse(headers.get(Constants.FRAGMENT_underscoreHOST));
                if (entries != null) {
                    if (entries[0].hasAttribute("extension")) {
                        ...
                        if (extension.equals("bootclasspath")) {
                            ...
                            if (!symbolicName.equals(framework.getSymbolicName()) && !symbolicName.equals(Constants.SYSTEM_underscoreBUNDLE_underscoreSYMBOLICNAME)) {
                ...
                String requiredEE = (String) headers.get(Constants.BUNDLE_underscoreREQUIREDEXECUTIONENVIRONMENT);
                if (requiredEE != null) {
                    BundleContext context = framework.getBundleContext();
                    String ee = context.getProperty(Constants.FRAMEWORK_underscoreEXECUTIONENVIRONMENT);
                ...
                File bundlefile = new File(cache, Storage.BUNDLE_underscoreFILE);
