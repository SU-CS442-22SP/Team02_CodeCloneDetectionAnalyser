                    ...
                    for (StudentType student : students) {
                        if (student.getOsobniCislo().equals(osobniCisloZeSlozky)) {
                            ...
                            for (int i = (pokusyDirectories.length - 1); i >= 0; i--) {
                                if ((pokusyDirectories[i].isDirectory()) && (pzp < Integer.parseInt(pokusyDirectories[i].getName().split("_underscore")[1].trim()))) {
                                    File testedFile = pokusyDirectories[i].listFiles()[0];
                                    if ((testedFile.exists()) && (testedFile.isFile())) {
                                        String[] partsOfFilename = testedFile.getName().split("_underscore");
                                        ...
                                        if (testedFile.getName().startsWith(samostatnaPraceNazev)) {
                                            ...
                                            if (partsOfFilename.length != 4) {
                                                ...
                                                lst.add(st);
                                                continue;
                                            } else if (!testedFile.getName().startsWith("Ppa1_underscorecv")) {
                                                SouborType st = new SouborType();
                                                st.setJmeno(testedFile.getName());
                                    ...
                                    if (zipFile.exists()) {
                                        String[] s = zipFile.getName().split("_underscore");
                                        if (s.length >= 3) {
                                    ...
                                    if (shodaJmenaSouboru && !samostatnaPrace) {
                                        ...
                                        String cisloCviceniS = testedFile.getName().split("_underscore")[1].substring(2);
                                        int cisloCviceniI = Integer.parseInt(cisloCviceniS);
                                        String cisloUlohyS = testedFile.getName().split("_underscore")[2];
                                        ...
                                        for (CviceniType ct : lct) {
                                            if (ct.getCislo().intValue() == cisloCviceniI) {
                                                ...
                                                XMLGregorianCalendar xmlGCDate = DatatypeFactory.newInstance().newXMLGregorianCalendarDate(Integer.parseInt(datumSouboru[0]), Integer.parseInt(datumSouboru[1]), Integer.parseInt(datumSouboru[2]), DatatypeConstants.FIELD_underscoreUNDEFINED);
                                                XMLGregorianCalendar xmlGCTime = DatatypeFactory.newInstance().newXMLGregorianCalendarTime(Integer.parseInt(casSouboru[0]), Integer.parseInt(casSouboru[1]), Integer.parseInt(casSouboru[2]), DatatypeConstants.FIELD_underscoreUNDEFINED);
                                                    ...
                                                    if (ut.getCislo().intValue() == cisloUlohyI) {
                                                        ...
                                                        if (vt.isVysledek()) {
                                                            ...
                                                            if (!(new File(test).exists())) {
                                                                LogManager.getInstance().log("Nebyla provedena p��prava soubor�. Chyb� slo�ka " + test.substring(Ppa1Cviceni.USER_underscoreDIR.length()) + ".");
                                                                return;
                                                            } else {
                            ...
                            String[] slozkaPoslednihoPokusu = pokusyDirectories[pokusyDirectories.length - 1].getName().split("_underscore");
