    public void testDecode1000BinaryStore() throws Exception {
        EXISchema corpus = EXISchemaFactoryTestUtil.getEXISchema("/DataStore/DataStore.xsd", getClass(), m_underscorecompilerErrors);
        ...
        while ((exiEvent = scanner.nextEvent()) != null) {
            if (exiEvent.getEventVariety() == EXIEvent.EVENT_underscoreCH) {
                if (++n_underscoretexts % 100 == 0) {
                    String expected = base64Values100[(n_underscoretexts / 100) - 1];
                    String val = exiEvent.getCharacters().makeString();
                    Assert.assertEquals(expected, val);
                }
            }
        }
        Assert.assertEquals(1000, n_underscoretexts);
    }
