    public static boolean copyDataToNewTable(EboContext p_underscoreeboctx, String srcTableName, String destTableName, String where, boolean log, int mode) throws boRuntimeException {
        ...
        try {
            ...
            PreparedStatement pstm = cn.prepareStatement("SELECT TABLE_underscoreNAME FROM INFORMATION_underscoreSCHEMA.TABLES WHERE upper(TABLE_underscoreNAME)=?  AND TABLE_underscoreSCHEMA=database()");
            ...
            if (!destexists) {
                rslt.close();
                pstm.close();
                pstm = cn.prepareStatement("SELECT TABLE_underscoreNAME FROM INFORMATION_underscoreSCHEMA.VIEWS WHERE upper(TABLE_underscoreNAME)=? AND TABLE_underscoreSCHEMA=database()");
                pstm.setString(1, destTableName);
                rslt = pstm.executeQuery();
                if (rslt.next()) {
            ...
            if (srcexists && !destexists) {
                ...
                PreparedStatement pstm2 = cn.prepareStatement("SELECT COLUMN_underscoreNAME FROM INFORMATION_underscoreSCHEMA.COLUMNS WHERE upper(TABLE_underscoreNAME) = ? AND TABLE_underscoreSCHEMA=database()");
                pstm2.setString(1, destTableName);
                ResultSet rslt2 = pstm2.executeQuery();
                StringBuffer fields = new StringBuffer();
                PreparedStatement pstm3 = cn.prepareStatement("SELECT COLUMN_underscoreNAME FROM INFORMATION_underscoreSCHEMA.COLUMNS WHERE upper(TABLE_underscoreNAME) = ? and upper(COLUMN_underscoreNAME)=? AND TABLE_underscoreSCHEMA=database()");
                while (rslt2.next()) {
