    public void initResources() throws XAwareException {
        ...
        if (!initialized) {
            String host = channelObject.getProperty(XAwareConstants.BIZDRIVER_underscoreHOST);
            if (host == null || host.trim().length() == 0) {
                throw new XAwareException("xa:host must be specified.");
            }
            String portString = channelObject.getProperty(XAwareConstants.BIZDRIVER_underscorePORT);
            ...
            String remoteVerification = channelObject.getProperty(XAwareConstants.XAWARE_underscoreFTP_underscoreREMOTE_underscoreVERIFICATION);
            String userName = channelObject.getProperty(XAwareConstants.BIZDRIVER_underscoreUSER);
            String password = channelObject.getProperty(XAwareConstants.BIZDRIVER_underscorePWD);
            String proxyUser = channelObject.getProperty(XAwareConstants.BIZCOMPONENT_underscoreATTR_underscorePROXYUSER);
            String proxyPassword = channelObject.getProperty(XAwareConstants.BIZCOMPONENT_underscoreATTR_underscorePROXYPASSWORD);
            ftpClient = new FTPClient();
            logger.finest("Connecting to host:" + host + " Port:" + port, className, methodName);
            try {
                ftpClient.connect(host, port);
                if (remoteVerification != null && remoteVerification.trim().equals(XAwareConstants.XAWARE_underscoreYES)) {
