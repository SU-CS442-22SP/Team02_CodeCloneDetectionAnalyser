  ...
  private void compress(File src) throws IOException
  {
    ...
    try
    {
      ...
      bzOut       = new BZip2OutputStream(
        ...
        Math.min(
          (this.blockSize == -1) ? BZip2OutputStream.MAX_underscoreBLOCK_underscoreSIZE : this.blockSize,
      ...
      while (true)
      {
        final long maxStep = showProgress ? Math.max(8192, (inSize - pos) / MAX_underscorePROGRESS) : (inSize - pos);
        if (maxStep <= 0)
        {
          if (showProgress)
          {
            for (int i = progress; i < MAX_underscorePROGRESS; i++)
              this.log.print('#');
        ...
        else
        {
          ...
          if (showProgress)
          {
            ...
            final int     newProgress = (int) (MAX_underscorePROGRESS * p);
