    public void processAction(ActionMapping mapping, ActionForm form, PortletConfig config, ActionRequest req, ActionResponse res) throws Exception {
        ...
        req.setAttribute(ViewReportsAction.REPORT_underscoreEDITOR_underscoreOR_underscoreADMIN, false);
        User user = _underscoregetUser(req);
        ...
        for (Role role : roles) {
            if (role.getName().equals("Report Administrator") || role.getName().equals("Report Editor") || role.getName().equals("CMS Administrator")) {
                req.setAttribute(ViewReportsAction.REPORT_underscoreEDITOR_underscoreOR_underscoreADMIN, true);
                editor = true;
                break;
            }
        }
        ...
        for (String dataSource : ds) {
            ...
            if (dataSource.equals(com.dotmarketing.util.Constants.DATABASE_underscoreDEFAULT_underscoreDATASOURCE)) {
        ...
        if (reportId > 0) {
            ...
            adminRoles.add(com.dotmarketing.util.Constants.ROLE_underscoreREPORT_underscoreADMINISTRATOR);
            if (user.getUserId().equals(report.getOwner())) {
                _underscorecheckWritePermissions(report, user, httpReq, adminRoles);
            }
            ...
            report = new Report();
            report.setOwner(_underscoregetUser(req).getUserId());
            newReport = true;
        }
        req.setAttribute(WebKeys.PERMISSION_underscoreINODE_underscoreEDIT, report);
        if ((cmd != null) && cmd.equals(Constants.EDIT)) {
            if (Validator.validate(req, form, mapping)) {
                ...
                try {
                    ...
                    _underscoreapplyPermissions(req, report);
                    if (!rfm.isWebFormReport()) {
                        if (UtilMethods.isSet(Config.getStringProperty("ASSET_underscoreREAL_underscorePATH"))) {
                            jrxmlPath = Config.getStringProperty("ASSET_underscoreREAL_underscorePATH") + File.separator + Config.getStringProperty("REPORT_underscorePATH") + File.separator + report.getInode() + ".jrxml";
                            jasperPath = Config.getStringProperty("ASSET_underscoreREAL_underscorePATH") + File.separator + Config.getStringProperty("REPORT_underscorePATH") + File.separator + report.getInode() + ".jasper";
                        } else {
                            jrxmlPath = Config.CONTEXT.getRealPath(File.separator + Config.getStringProperty("REPORT_underscorePATH") + File.separator + report.getInode() + ".jrxml");
                            jasperPath = Config.CONTEXT.getRealPath(File.separator + Config.getStringProperty("REPORT_underscorePATH") + File.separator + report.getInode() + ".jasper");
                        }
                        ...
                        if (importFile.exists()) {
                            ...
                            try {
                                ...
                                setForward(req, "portlet.ext.report.edit_underscorereport");
                                return;
                            }
                            ...
                            _underscoreloadReportParameters(jasperReport.getParameters());
                    ...
                    params.put("struts_underscoreaction", new String[] { "/ext/report/view_underscorereports" });
                    ...
                    _underscoresendToReferral(req, res, referrer);
                    return;
                } catch (Exception ex) {
                    HibernateUtil.rollbackTransaction();
