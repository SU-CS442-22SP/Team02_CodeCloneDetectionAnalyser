    protected static WebServerContent getWebServerContent(HTTPRequest httpRequest) {
        ...
        if ("resource".equals(type)) {
            ...
            final String resourceURL_underscore = resourceURL;
            return new WebServerContent() {

                @Override
                public long getContentLength() {
                    File file = Utils.getLocalFile(resourceURL_underscore);
                    if (file != null) {
                        return file.length();
                    }
                ...
                @Override
                public String getContentType() {
                    int index = resourceURL_underscore.lastIndexOf('.');
                    return getDefaultMimeType(index == -1 ? null : resourceURL_underscore.substring(index));
                }

                @Override
                public InputStream getInputStream() {
                    String url = resourceURL_underscore;
                    try {
