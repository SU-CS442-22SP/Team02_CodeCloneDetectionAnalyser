    private synchronized void executePrebuildCache() {
        ...
        for (int i = 0; i < paths.length; i++) {
            try {
                ...
                logger.warn("Is CACHE_underscoreSERVER dead? escape prebuild cache!");
                logger.error("[****Prebuild cache interrupted! Please check the CACHE_underscoreSERVER's status.****]");
            } catch (Exception e) {
        ...
        if (CacheManager.getPreCacheMap() == null || CacheManager.getPreCacheMap().size() == 0) {
            logger.warn("[****Something wrong in prebuild cache process is CACHE_underscoreSERVER dead? escape sending cache to CLIENT****]");
            return;
        } else {
