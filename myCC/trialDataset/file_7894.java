    public void moveMessage(DBMimeMessage oSrcMsg) throws MessagingException {
        ...
        try {
            ...
            oStmt = oConn.prepareStatement("SELECT " + DB.pg_underscoremessage + "," + DB.nu_underscoreposition + "," + DB.len_underscoremimemsg + " FROM " + DB.k_underscoremime_underscoremsgs + " WHERE " + DB.gu_underscoremimemsg + "=?");
            ...
            oStmt = oConn.prepareStatement("UPDATE " + DB.k_underscorecategories + " SET " + DB.len_underscoresize + "=" + DB.len_underscoresize + "-" + String.valueOf(iLen) + " WHERE " + DB.gu_underscorecategory + "=?");
            oStmt.setString(1, ((DBFolder) (oSrcMsg.getFolder())).getCategory().getString(DB.gu_underscorecategory));
            oStmt.executeUpdate();
            oStmt.close();
            oStmt = null;
            oStmt = oConn.prepareStatement("UPDATE " + DB.k_underscorecategories + " SET " + DB.len_underscoresize + "=" + DB.len_underscoresize + "+" + String.valueOf(iLen) + " WHERE " + DB.gu_underscorecategory + "=?");
        ...
        try {
            oMboxSrc = new MboxFile(oSrcFldr.getFile(), MboxFile.READ_underscoreWRITE);
            oMboxThis = new MboxFile(oSrcFldr.getFile(), MboxFile.READ_underscoreWRITE);
        ...
        try {
            ...
            String sCatGuid = getCategory().getString(DB.gu_underscorecategory);
            oStmt = oConn.prepareStatement("UPDATE " + DB.k_underscoremime_underscoremsgs + " SET " + DB.gu_underscorecategory + "=?," + DB.pg_underscoremessage + "=? WHERE " + DB.gu_underscoremimemsg + "=?");
