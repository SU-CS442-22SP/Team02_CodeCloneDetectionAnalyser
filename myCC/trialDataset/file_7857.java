    ...
    public void testSecondary() throws Exception {
        ...
        try {
            ...
            stm.executeUpdate("drop table if exists first_underscorechanges");
            stm.executeUpdate("drop table if exists second");
            stm.executeUpdate("drop table if exists second_underscorechanges");
            stm.executeUpdate("create table first (a integer, b varchar, c integer, d date)");
            stm.executeUpdate("create table first_underscorechanges (a integer, b varchar, c integer, d date, ct varchar, cd integer)");
            stm.executeUpdate("create table second (a integer, b varchar, c integer, d date)");
            stm.executeUpdate("create table second_underscorechanges (a integer, b varchar, c integer, d date, ct varchar, cd integer)");
            ...
            stm.executeUpdate("insert into second_underscorechanges (a,b,c,d,ct,cd) values (2,'f',60,date '2007-02-02', 'r', 2)");
            ...
            stm.executeUpdate("drop table first_underscorechanges");
            stm.executeUpdate("drop table second");
            stm.executeUpdate("drop table second_underscorechanges");
            stm.close();
            store.close();
            conn.commit();
        } catch (Exception e) {
