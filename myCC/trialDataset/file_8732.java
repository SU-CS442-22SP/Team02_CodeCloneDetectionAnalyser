    private void handleInterfaceDown(long eventID, long nodeID, String ipAddr, String eventTime) {
        ...
        if (eventID == -1 || nodeID == -1 || ipAddr == null) {
            log.warn(EventConstants.INTERFACE_underscoreDOWN_underscoreEVENT_underscoreUEI + " ignored - info incomplete - eventid/nodeid/ip: " + eventID + "/" + nodeID + "/" + ipAddr);
            return;
        }
        ...
        try {
            ...
            PreparedStatement activeSvcsStmt = dbConn.prepareStatement(OutageConstants.DB_underscoreGET_underscoreACTIVE_underscoreSERVICES_underscoreFOR_underscoreINTERFACE);
            PreparedStatement openStmt = dbConn.prepareStatement(OutageConstants.DB_underscoreOPEN_underscoreRECORD);
            PreparedStatement newOutageWriter = dbConn.prepareStatement(OutageConstants.DB_underscoreINS_underscoreNEW_underscoreOUTAGE);
            PreparedStatement getNextOutageIdStmt = dbConn.prepareStatement(OutageManagerConfigFactory.getInstance().getGetNextOutageID());
            newOutageWriter = dbConn.prepareStatement(OutageConstants.DB_underscoreINS_underscoreNEW_underscoreOUTAGE);
