    private void makeDailyBackup() throws CacheOperationException, ConfigurationException {
        final int MAX_underscoreDAILY_underscoreBACKUPS = 5;
        File cacheFolder = getBackupFolder();
        cacheLog.debug("Making a daily backup of current Beehive archive...");
        try {
            File oldestDaily = new File(DAILY_underscoreBACKUP_underscorePREFIX + "." + MAX_underscoreDAILY_underscoreBACKUPS);
            if (oldestDaily.exists()) {
                moveToWeeklyBackup(oldestDaily);
            }
            for (int index = MAX_underscoreDAILY_underscoreBACKUPS - 1; index > 0; index--) {
                File daily = new File(cacheFolder, DAILY_underscoreBACKUP_underscorePREFIX + "." + index);
                File target = new File(cacheFolder, DAILY_underscoreBACKUP_underscorePREFIX + "." + (index + 1));
                if (!daily.exists()) {
        ...
        File tempBackupArchive = new File(cacheFolder, BEEHIVE_underscoreARCHIVE_underscoreNAME + ".tmp");
        ...
        try {
            ...
            while ((len = archiveReader.read(buffer, 0, BUFFER_underscoreSIZE)) != -1) {
