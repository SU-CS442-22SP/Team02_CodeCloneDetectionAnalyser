    private void unzip(File filename) throws ZipException, IOException {
        ...
        ZipEntry entry = null;
        boolean first_underscoreentry = true;
        while ((entry = in.getNextEntry()) != null) {
            if (first_underscoreentry) {
                if (!entry.isDirectory()) {
                    File subdir = new File(dir + File.separator + filename.getName().substring(0, filename.getName().length() - SUFFIX_underscoreZIP.length()));
                    if (!subdir.exists()) {
                        subdir.mkdir();
                        dir = subdir;
                    }
                }
                first_underscoreentry = false;
            }
            if (entry.isDirectory()) {
