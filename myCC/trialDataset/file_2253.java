    void createAgentProgram(PrintStream ps, String agentName, String agentID) throws IOException, ConfigException {
        ...
        double agent_underscoretimeout_underscoreseconds = engine.getTimeout();
        String ii = "\"";
        ...
        for (String taskName : taskNames) {
            ...
            if (embedScripts) {
                ps.println();
                ps.println("cat > " + task.getFile() + " << " + ii + "EOF_underscore" + task.getFile() + ii);
                ps.println(script);
                ps.println("EOF_underscore" + task.getFile());
            } else {
        ...
        }
        {
            ...
            if (embedScripts) {
                ps.println();
                ps.println("cat > " + serviceScript + " << " + ii + "EOF_underscore" + serviceScript + ii);
                ss = ps;
            } else {
            ...
            if (embedScripts) ps.println("EOF_underscore" + serviceScript); else ss.close();
        }
        {
            ...
            if (embedScripts) {
                ps.println();
                ps.println("cat > " + agentScript + " << " + ii + "EOF_underscore" + agentScript + ii);
                as = ps;
            } else {
            ...
            as.println(tab[i] + "timeout=" + ii + agent_underscoretimeout_underscoreseconds + ii);
            as.println(tab[i] + "stop_underscoreflag=0");
            as.println(tab[i] + "kill_underscoreflag=0");
            as.println();
            ...
            as.println();
            as.println(tab[i] + "def create_underscorenamespace(service) :");
            ...
            for (VarAttributes va : attributesMap.values()) {
                ...
                if (Constants.VAR_underscoreSTDOUT.equals(varname)) continue;
            ...
            for (String taskName : taskNames) {
                ...
                as.println(tab[i] + "def task_underscore" + taskName + "():");
                {
                    i++;
                    as.println(tab[i] + "try :");
                    if (background) {
                        as.println(tab[i] + "    # as ZSIWrapper is not task save," + " create a new one");
                        as.println(tab[i] + "    port_underscoretask=ZSIWrapper(URL, agentID, " + "instanceID, instanceKey, timeout)");
                        as.println(tab[i] + "    service = Service(port_underscoretask)");
                    } else {
                        as.println(tab[i] + "    # this is foreground, " + "use port0 from current task");
                        as.println(tab[i] + "    service = Service(port0)");
                    }
                    as.println(tab[i] + "    globals = create_underscorenamespace(service)");
                    ...
                    as.println(tab[i] + "    if kill_underscoreflag :");
                    as.println(tab[i] + "        task_underscoreexit_underscorestatus = " + Status.FAILED.ordinal());
                    as.println(tab[i] + "    elif stop_underscoreflag :");
                    as.println(tab[i] + "        task_underscoreexit_underscorestatus = " + Status.TERMINATED.ordinal());
                    as.println(tab[i] + "    else :");
                    as.println(tab[i] + "        task_underscoreexit_underscorestatus = " + Status.FINISHED.ordinal());
                    as.println(tab[i] + "    port0.setTaskStatus('" + taskName + "', task_underscoreexit_underscorestatus )");
                    as.println();
                    i--;
                }
            }
            ...
            as.println(tab[i] + "while action != '" + Agent.STOP + "' and action != '" + Agent.KILL + "' and action != '" + Agent.FINISH + "' :");
            {
                ...
                as.println(tab[i] + "to_underscorebe_underscoreremoved = []");
                as.println(tab[i] + "for tr2 in tasklist:");
                as.println(tab[i] + "    if (not tr2.isAlive()):");
                as.println(tab[i] + "        to_underscorebe_underscoreremoved.append(tr2)");
                as.println(tab[i] + "for tr3 in to_underscorebe_underscoreremoved:");
                ...
                for (String taskName : taskNames) {
                    ...
                    String function = "task_underscore" + taskName;
                    if (background) {
            ...
            as.println(tab[i] + "else :");
            as.println(tab[i] + "    if kill_underscoreflag :");
            as.println(tab[i] + "        agent_underscoreexit_underscorestatus = " + Status.FAILED.ordinal());
            as.println(tab[i] + "    elif stop_underscoreflag :");
            as.println(tab[i] + "        agent_underscoreexit_underscorestatus = " + Status.TERMINATED.ordinal());
            as.println(tab[i] + "    else :");
            as.println(tab[i] + "        agent_underscoreexit_underscorestatus = " + Status.FINISHED.ordinal());
            as.println(tab[i] + "    port0.setAgentStatus( agent_underscoreexit_underscorestatus )");
            if (embedScripts) ps.println("EOF_underscore" + agentScript); else as.close();
        }
        for (String agentFileName : agent.getFilesSet()) {
            ...
            ps.println("cat > " + agentFileName + " << " + ii + "EOF_underscore" + agentFileName + ii);
            File file = new File(scriptDir, agentFileName);
            byte bytes[] = FileUtils.readFileToByteArray(file);
            ps.write(bytes);
            ps.println();
            ps.println("EOF_underscore" + agentFileName);
        }
        if (embedScripts) {
            ps.println("SKIP=`awk '/^_underscore_underscoreARCHIVE_underscoreFOLLOWS_underscore_underscore/ { print NR + 1; exit 0; }' $0`");
            ps.println("tail -n +$SKIP $0 | gzip -dc | tar x ");
            ps.println("python -u " + agentScript);
            ps.println("exit 0");
            ps.println("_underscore_underscoreARCHIVE_underscoreFOLLOWS_underscore_underscore");
