    public void execute() {
        try {
            ...
            for (Iterator<Resource> i = classFiles.iterator(); i.hasNext(); ) {
                ...
                if (!classFile.getName().equals(CALLSTACK_underscoreFILE)) {
                    try {
            ...
            for (Iterator<Resource> i = resources.iterator(); i.hasNext(); ) {
                ...
                try {
                    InputStream inputStream = classFile.getInputStream();
                    ClassReader reader = new ClassReader(inputStream);
                    ClassWriter writer = new ClassWriter(reader, ClassWriter.COMPUTE_underscoreMAXS);
                    if (!classFile.getName().startsWith(CALLSTACK_underscoreFILE.substring(0, CALLSTACK_underscoreFILE.indexOf('.')))) {
            ...
            File classFile = new File(classFiles.getDir(), CALLSTACK_underscoreFILE);
            if (!classFile.exists()) {
