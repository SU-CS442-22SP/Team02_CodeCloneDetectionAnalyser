    private void zipAndSendFiles(File[] logFiles) {
        ...
        try {
            File globalZipFile = new File("XeptoTileServer_underscoreStatistics_underscore" + sdf.format(new Date()) + ".zip");
            ...
            for (File logFile : logFiles) {
                ...
                for (i = 0; i < usernames.length; i++) {
                    if (logFile.getName().substring(27, 28 + usernames[i].length()).equalsIgnoreCase(usernames[i] + "_underscore")) break;
                }
                ...
                if (i < usernames.length && emails[i] != null) {
                    if (zipFiles[i] == null) {
                        zipFiles[i] = new File("XeptoTileServer_underscoreStatistics_underscore" + usernames[i] + "_underscore" + sdf.format(new Date()) + ".zip");
            ...
            globalOut.close();
            emailSender.send(java.util.ResourceBundle.getBundle("logsender_underscoreconf").getString("e-mail"), globalZipFile);
