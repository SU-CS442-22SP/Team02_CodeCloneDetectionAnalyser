    private static void loadCommandList() {
        final URL url;
        try {
            url = IOUtils.getResource(null, PYTHON_underscoreMENU_underscoreFILE);
        } catch (final FileNotFoundException ex) {
            log.error("File '" + PYTHON_underscoreMENU_underscoreFILE + "': " + ex.getMessage());
            return;
        }
        ...
        try {
            ...
            try {
                final Reader reader = new InputStreamReader(inputStream, IOUtils.MAP_underscoreENCODING);
                try {
                    ...
                    try {
                        ...
                        Collections.sort(cmdList, String.CASE_underscoreINSENSITIVE_underscoreORDER);
                        if (!cmdList.isEmpty()) {
