    private void fetchAvailable(ProgressObserver po) {
        ...
        try {
            ...
            String enc = null;
            if (!skipDownload) {
                URL available_underscoreurl = new URL(jEdit.getProperty(OOO_underscoreDICTS_underscorePROP) + "available.lst");
                URLConnection connect = available_underscoreurl.openConnection();
                connect.connect();
                ...
                if (!copied) {
                    Log.log(Log.ERROR, HunspellDictsManager.class, "Unable to download " + available_underscoreurl.toString());
                    GUIUtilities.error(null, "spell-check-hunspell-error-fetch", new String[] { "Unable to download file " + available_underscoreurl.toString() });
                    availables = null;
