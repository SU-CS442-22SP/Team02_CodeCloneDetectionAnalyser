    ...
    protected String determineDataType() {
        ...
        if (spec.endsWith("=jpeg")) {
            ...
            return mimeType;
        } else if (spec != SPEC_underscoreUNSET && spec.startsWith("http") && spec.contains("fedora")) {
            if (spec.endsWith("bdef:AssetDef/getFullView/")) {
                return "html";
            } else {
                String type = getProperty(CONTENT_underscoreTYPE);
                if (type == null || type.length() < 1) {
                    try {
                        ...
                        if (type != null && type.length() > 0) setProperty(CONTENT_underscoreTYPE, type);
                    } catch (Throwable t) {
