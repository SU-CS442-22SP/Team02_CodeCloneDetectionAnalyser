    ...
    public void launch(final ILaunchConfiguration configuration, final String mode, final ILaunch launch, final IProgressMonitor monitor) throws CoreException {
        ...
        final String projectName = configuration.getAttribute(INexOpenLaunchConfigurationConstants.NEXOPEN_underscorePROJECT_underscoreNAME, "");
        ...
        if (NexOpenProjectUtils.isNexOpenProject(prj)) {
            ...
            try {
                ...
                final String dialectName = configuration.getAttribute(INexOpenLaunchConfigurationConstants.HIBERNATE_underscoreDIALECT, "MySQL5InnoDB");
                props.setProperty("hibernate.dialect", support.getDialectClass(dialectName));
                props.setProperty("hibernate.connection.driver_underscoreclass", configuration.getAttribute(INexOpenLaunchConfigurationConstants.JDBC_underscoreDRIVER, "com.mysql.jdbc.Driver"));
                props.setProperty("hibernate.connection.url", configuration.getAttribute(INexOpenLaunchConfigurationConstants.JDBC_underscoreURL, "jdbc:mysql://<host><:port>/<database>"));
                props.setProperty("hibernate.connection.username", configuration.getAttribute(INexOpenLaunchConfigurationConstants.JDBC_underscoreUSERNAME, "sa"));
                props.setProperty("hibernate.connection.password", configuration.getAttribute(INexOpenLaunchConfigurationConstants.JDBC_underscorePASSWORD, ""));
                props.store(output, "hibernate properties for code generation using NexOpen Tools 1.0.0");
                final IFile props_underscorefile = prj.getFile("business/src/test/resources/hibernate.properties");
                if (!props_underscorefile.exists()) {
                    props_underscorefile.create(new ByteArrayInputStream(output.toByteArray()), true, monitor);
                } else {
                    props_underscorefile.setContents(new ByteArrayInputStream(output.toByteArray()), true, false, monitor);
                }
                    ...
                    public void processHandle(final Document doc) {
                        ...
                        for (final Element bean : beans) {
                            ...
                            if ("valueListAdapterResolver".equals(id)) {
                                try {
                                    final String pkgName = configuration.getAttribute(INexOpenLaunchConfigurationConstants.NEXOPEN_underscorePACKAGE, "");
                ...
                if (support.isReverseEngineeringFileNeeded(dialectName)) {
                    try {
                        ...
                        if (!revengFile.exists()) {
                            ...
                            str = str.replace("${schema}", configuration.getAttribute(INexOpenLaunchConfigurationConstants.JDBC_underscoreUSERNAME, "sa"));
