    public ActionForward uploadFile(ActionMapping mapping, ActionForm actForm, HttpServletRequest request, HttpServletResponse in_underscoreresponse) {
        ...
        try {
            ...
            String realXLSPath;
            if (request != null) {
                proposalCode = (String) request.getSession().getAttribute(Constants.PROPOSAL_underscoreCODE);
                proposalNumber = String.valueOf(request.getSession().getAttribute(Constants.PROPOSAL_underscoreNUMBER));
                proposalName = proposalCode + proposalNumber.toString();
                uploadedFileName = form.getRequestFile().getFileName();
                String fileName = proposalName + "_underscore" + uploadedFileName;
            ...
            if (shippingId != null) {
                ...
                double nbSamplesContainers = DBAccess_underscoreEJB.DeleteAllSamplesAndContainersForShipping(shippingId);
            ...
            Hashtable<String, Hashtable<String, Integer>> listProteinAcronym_underscoreSampleName = new Hashtable<String, Hashtable<String, Integer>>();
            ...
            for (int p = 0; p < listProteins.size(); p++) {
                ...
                for (int c = 0; c < listCrystals.length; c++) {
                    CrystalLightValue _underscorextal = (CrystalLightValue) listCrystals[c];
                    CrystalValue xtal = crystal.findByPrimaryKey(_underscorextal.getPrimaryKey());
                ...
                listProteinAcronym_underscoreSampleName.put(prot.getAcronym(), listSampleName);
            }
            parser.validate(inFile, listProteinAcronym_underscoreSampleName, targetProposal.getProposalId());
            ...
            while (errIt.hasNext()) {
                ...
                errors.add(ActionMessages.GLOBAL_underscoreMESSAGE, new ActionMessage("message.free", xlsEx.getMessage() + " ---> " + xlsEx.getSuggestedFix()));
            }
            try {
            ...
            while (warnIt.hasNext()) {
                ...
                messages.add(ActionMessages.GLOBAL_underscoreMESSAGE, new ActionMessage("message.free", xlsEx.getMessage() + " ---> " + xlsEx.getSuggestedFix()));
            }
            try {
            ...
            ServerLogger.Log4Stat("XLS_underscoreUPLOAD", proposalName, uploadedFileName);
        } catch (XlsUploadException e) {
            resetCounts(shippingId);
            errors.add(ActionMessages.GLOBAL_underscoreMESSAGE, new ActionMessage("errors.detail", e.getMessage()));
            ClientLogger.getInstance().error(e.toString());
            saveErrors(request, errors);
            return mapping.findForward("error");
        } catch (Exception e) {
            resetCounts(shippingId);
            errors.add(ActionMessages.GLOBAL_underscoreMESSAGE, new ActionMessage("errors.detail", e.toString()));
            ClientLogger.getInstance().error(e.toString());
            e.printStackTrace();
            saveErrors(request, errors);
            return mapping.findForward("error");
