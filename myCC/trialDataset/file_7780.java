    public void loadScripts() {
        ...
        NodeList scripts = document.getElementsByTagNameNS(SVGConstants.SVG_underscoreNAMESPACE_underscoreURI, SVGConstants.SVG_underscoreSCRIPT_underscoreTAG);
        ...
        for (int i = 0; i < len; i++) {
            ...
            String type = script.getAttributeNS(null, SVGConstants.SVG_underscoreTYPE_underscoreATTRIBUTE);
            if (type.length() == 0) {
                type = SVGConstants.SVG_underscoreSCRIPT_underscoreTYPE_underscoreDEFAULT_underscoreVALUE;
            }
            if (type.equals(SVGConstants.SVG_underscoreSCRIPT_underscoreTYPE_underscoreJAVA)) {
                try {
            ...
            try {
                ...
                if (href.length() > 0) {
                    ...
                    desc = Messages.formatMessage(INLINE_underscoreSCRIPT_underscoreDESCRIPTION, new Object[] { d.getURL(), "<" + script.getNodeName() + ">", new Integer(line) });
                    Node n = script.getFirstChild();
                    if (n != null) {
                        StringBuffer sb = new StringBuffer();
                        while (n != null) {
                            if (n.getNodeType() == Node.CDATA_underscoreSECTION_underscoreNODE || n.getNodeType() == Node.TEXT_underscoreNODE) sb.append(n.getNodeValue());
                            n = n.getNextSibling();
                        }
