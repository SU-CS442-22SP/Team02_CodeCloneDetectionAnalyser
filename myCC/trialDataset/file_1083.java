    final void importSelectedEntity() {
        ...
        if (userObject instanceof ProjectSet) {
            ...
            if (result == JFileChooser.APPROVE_underscoreOPTION) {
                ...
                if (Arrays.asList(workspace.getProjectNames()).contains(projectName)) {
                    confirmLoad = JOptionPane.showConfirmDialog(mainWindow.getApplicativeFrame(), "Overwrite project \"" + projectName + "\"" + "?", "Overwrite project", JOptionPane.OK_underscoreCANCEL_underscoreOPTION) == JOptionPane.OK_underscoreOPTION;
                    if (confirmLoad) {
