    public static String md5Crypt(final byte[] key, final byte[] salt) throws NoSuchAlgorithmException {
        ...
        final MessageDigest _underscoremd = MessageDigest.getInstance("MD5");
        _underscoremd.update(key);
        _underscoremd.update(MAGIC.getBytes());
        _underscoremd.update(salt);
        ...
        for (int j = 0, i = key.length; i != 0; i >>>= 1) {
            if ((i & 1) == 1) _underscoremd.update(abyFinal, j, 1); else _underscoremd.update(key, j, 1);
        }
