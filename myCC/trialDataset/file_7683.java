    public void run() {
        ...
        try {
            ...
            try {
                if (target instanceof File) {
                    final byte[] byteBuffer = new byte[BUFFER_underscoreSIZE];
                    while (true) {
                        checkState();
                        final int byteCount = bis.read(byteBuffer, 0, BUFFER_underscoreSIZE);
                        if (byteCount == -1) {
                            break;
                        }
                    ...
                    final char[] charBuffer = new char[BUFFER_underscoreSIZE];
                    final StringBuilder sb = (StringBuilder) target;
                    while (true) {
                        checkState();
                        final int charCount = br.read(charBuffer, 0, BUFFER_underscoreSIZE);
                        if (charCount == -1) {
                            break;
                        }
