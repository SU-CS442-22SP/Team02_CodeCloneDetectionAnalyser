    public static void main(String[] args) throws Throwable {
        ...
        options.addOption(new CommandLineOptionBuilder("prefix", "file prefix for all generated files ( default " + DEFAULT_underscorePREFIX + " )").build());
        ...
        options.addOption(new CommandLineOptionBuilder("chromat_underscoredir", "directory of chromatograms to be converted into phd " + "(it is assumed the read data for these chromatograms are in a fasta file which the .cas file knows about").build());
        options.addOption(new CommandLineOptionBuilder("s", "cache size ( default " + DEFAULT_underscoreCACHE_underscoreSIZE + " )").longName("cache_underscoresize").build());
        ...
        try {
            ...
            int cacheSize = commandLine.hasOption("s") ? Integer.parseInt(commandLine.getOptionValue("s")) : DEFAULT_underscoreCACHE_underscoreSIZE;
            ...
            if (commandLine.hasOption("trim")) {
                ...
                trimDatastore = TrimDataStoreUtil.EMPTY_underscoreDATASTORE;
            }
            ...
            if (commandLine.hasOption("chromat_underscoredir")) {
                sourceChromatogramFileServer = DirectoryFileServer.createReadOnlyDirectoryFileServer(new File(commandLine.getOptionValue("chromat_underscoredir")));
            ...
            if (!commandLine.hasOption("tempDir")) {
                tempDir = DirectoryFileServer.createTemporaryDirectoryFileServer(DEFAULT_underscoreTEMP_underscoreDIR);
            } else {
            ...
            try {
                if (!outputDir.contains("chromat_underscoredir")) {
                    outputDir.createNewDir("chromat_underscoredir");
                }
                if (sourceChromatogramFileServer != null) {
                    for (File f : sourceChromatogramFileServer) {
                        String name = f.getName();
                        OutputStream out = new FileOutputStream(outputDir.createNewFile("chromat_underscoredir/" + name));
                ...
                for (File traceFile : casAssembly.getNuceotideFiles()) {
                    ...
                    if (name.contains("fastq")) {
                        if (!outputDir.contains("solexa_underscoredir")) {
                            outputDir.createNewDir("solexa_underscoredir");
                        }
                        if (outputDir.contains("solexa_underscoredir/" + name)) {
                            IOUtil.delete(outputDir.getFile("solexa_underscoredir/" + name));
                        }
                        outputDir.createNewSymLink(traceFile.getAbsolutePath(), "solexa_underscoredir/" + name);
                    } else if ("sff".equals(extension)) {
                        if (!outputDir.contains("sff_underscoredir")) {
                            outputDir.createNewDir("sff_underscoredir");
                        }
                        if (outputDir.contains("sff_underscoredir/" + name)) {
                            IOUtil.delete(outputDir.getFile("sff_underscoredir/" + name));
                        }
                        outputDir.createNewSymLink(traceFile.getAbsolutePath(), "sff_underscoredir/" + name);
                    }
                }
                ...
                WholeAssemblyAceTag pathToPhd = new DefaultWholeAssemblyAceTag("phdball", "cas2consed", new Date(DateTimeUtils.currentTimeMillis()), "../phd_underscoredir/" + prefix + ".phd.ball");
