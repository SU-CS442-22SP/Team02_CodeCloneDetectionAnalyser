                protected IStatus run(IProgressMonitor monitor) {
                    try {
                        ...
                        if (MAC.equals("not_underscorefound") || serial.equals("not_underscorefound") || !serial.startsWith(distributor)) {
                            try {
                                MAC = getFromFile("[SPECIFICINFO]", "MAC", getAppPath() + "/register.ini");
                                serial = getFromFile("[SPECIFICINFO]", "serial", getAppPath() + "/register.ini");
                            } catch (Exception e) {
                            ...
                            if (htmlMessage.contains("error")) {
                                ...
                                setProperty(IProgressConstants.ICON_underscorePROPERTY, IconImg.liveUpIco);
                                if (isModal(this)) {
                                    showResults2();
                                } else {
                                    setProperty(IProgressConstants.KEEP_underscorePROPERTY, Boolean.TRUE);
                                    setProperty(IProgressConstants.ACTION_underscorePROPERTY, getUpdateCompletedAction2());
                                }
                            } else {
                                if (!getDBVersion().equals(latestVersion)) {
                                    try {
                                        ...
                                        try {
                                            ...
                                            while (status) {
                                                ...
                                                if (monitor.isCanceled()) return Status.CANCEL_underscoreSTATUS;
                                            }
                                            if (in != null) {
                                                in.close();
                                            }
                                            ...
                                            try {
                                                ...
                                                if (isModal(this)) {
                                                    ...
                                                    setProperty(IProgressConstants.KEEP_underscorePROPERTY, Boolean.TRUE);
                                                    setProperty(IProgressConstants.ACTION_underscorePROPERTY, getUpdateCompletedAction());
                                                }
                                            } catch (Exception e) {
                                                setProperty(IProgressConstants.ICON_underscorePROPERTY, IconImg.liveUpIco);
                                                if (isModal(this)) {
                                    ...
                                    if (isModal(this)) {
                                        ...
                                        setProperty(IProgressConstants.KEEP_underscorePROPERTY, Boolean.TRUE);
                                        setProperty(IProgressConstants.ACTION_underscorePROPERTY, getUpdateCompletedAction1());
                                    }
                                }
                            }
                        }
                        ...
                        lock.release();
                        updateRunning = false;
                        if (getValue("AUTO_underscoreUPDATE")) schedule(10800000);
                    }
                }
