    public static String getFileContents(String path) {
        ...
        if (path.indexOf("://") != -1) {
            ...
            if (toRead.exists() && !toRead.isDirectory()) {
                String _underscorepath = toRead.getAbsolutePath();
                try {
                    String encoding = Characters.getDeclaredXMLEncoding(URLTools.createValidURL(_underscorepath));
                    stream = new FileInputStream(_underscorepath);
                    buffReader = new BufferedReader(new InputStreamReader(stream, encoding));
                } catch (IOException e) {
                    LOGGER.warn(String.format("I/O error trying to read \"%s\"", _underscorepath));
                    return null;
                }
            } else {
